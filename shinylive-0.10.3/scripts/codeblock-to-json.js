...
// src/utils.ts
function engineSwitch(engine2, rValue, pythonValue) {
  switch (engine2) {
    case "r":
      return rValue;
...
function parseCodeBlock(codeblock, engine2) {
  if (!Array.isArray(codeblock)) {
    codeblock = codeblock.split("\n");
  }
  const { lines: lines2, quartoArgs: quartoArgsParsed } = processQuartoArgs(codeblock);
  ...
  if (quartoArgs.components.includes("viewer")) {
    ...
    defaultFilename = engineSwitch(engine2, "app.R", "app.py");
  } else {
    ...
    defaultFilename = engineSwitch(engine2, "code.R", "code.py");
  }
  const files = parseFileContents(lines2, defaultFilename);
  return { files, quartoArgs };
}
function processQuartoArgs(lines2) {
  let i = 0;
  ...
  const argCommentLines = lines2.slice(0, i).map((line) => line.replace(rgxQuartoComment, ""));
  const quartoArgs = load(
    argCommentLines.join("\n")
  );
  return {
    lines: lines2.slice(i),
    quartoArgs
  };
}
function parseFileContents(lines2, defaultFilename) {
  const files = [];
  let currentFile = {
...
var engine = args.length > 0 && args[0] == "r" ? "r" : "python";
var content = parseCodeBlock(lines, engine);
