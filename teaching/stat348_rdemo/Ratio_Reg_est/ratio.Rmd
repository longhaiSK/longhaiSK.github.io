...
# Domain and Post-stratification with agsrs.csv example
## Domain Analysis
```{r}
...
cbind(ybarh, sh, nh, prop_obs=nh/sum(nh), Nh, nh_post, prop_post = nh_post/sum(nh_post))
...
# Domain and Post-stratification with teacher example

## Reading Data
```{r}
teacher <- read.csv ("data/college_teacher.csv")
table(teacher)
prop.table(table(teacher),2)
```
...
rbind(
    ...
    male = srs_mean_est(subset(teacher, gender==2)$teacher, N=3000)
)
```

## Post-stratified Analysis
```{r}
Nh <- c(3000, 1000)
ph_post <- Nh/sum (Nh); ph_post
```

**Create nh proportional to Nh, instead of observed nh**
