...
* $\mu|\tau \sim N(\mu_0, 1/(\tau_0\tau))$
    $$\pi(\mu|\tau) \propto \tau^{1/2} e^{-\frac{\tau_0\tau}{2}(\mu-\mu_0)^2}$$

The joint prior is:
$$
\pi(\mu, \tau) \propto \tau^{\alpha - 1/2} \exp\left\{ -\tau \left( \beta + \frac{\tau_0}{2}(\mu - \mu_0)^2 \right) \right\}
$$

**Derivation of the Likelihood:**

The likelihood is the product of the individual densities:
$$
\begin{aligned}
L(\mu, \tau) &= \prod_{i=1}^n \frac{\tau^{1/2}}{\sqrt{2\pi}} \exp\left\{ -\frac{\tau}{2}(x_i - \mu)^2 \right\} \\
&\propto \tau^{n/2} \exp\left\{ -\frac{\tau}{2} \sum_{i=1}^n (x_i - \mu)^2 \right\}
\end{aligned}
$$

Using the identity $\sum (x_i - \mu)^2 = S_{xx} + n(\bar{x}-\mu)^2$, where $S_{xx} = \sum (x_i - \bar{x})^2$:
$$
L(\mu, \tau) \propto \tau^{n/2} \exp\left\{ -\frac{\tau}{2} \left[ S_{xx} + n(\bar{x}-\mu)^2 \right] \right\}
$$
...
$$
\begin{aligned}
\pi(\mu, \tau | x) &\propto \underbrace{\tau^{\alpha + n/2 - 1} \exp\left\{ -\tau \left[ \beta + \frac{S_{xx}}{2} + \frac{n\tau_0}{2(n+\tau_0)}(\bar{x}-\mu_0)^2 \right] \right\}}_{\text{Marginal of } \tau} \\
...
We define the posterior parameters $\tau' = \tau_0 + n$ and $\mu' = \frac{\tau_0\mu_0 + n\bar{x}}{\tau_0 + n}$.

* **Conditional Posterior of $\mu|\tau, x$**:
    $$\mu|\tau, x \sim N(\mu', 1/(\tau'\tau))$$
    $$E[\mu|\tau, x] = \mu' = \frac{\tau_0\mu_0 + n\bar{x}}{\tau_0 + n}$$

* The marginal posterior of the precision is $\tau|x \sim \text{Gamma}(\alpha', \beta')$. While the shape parameter is always $\alpha' = \frac{\alpha_0 + n}{2}$, the rate parameter $\beta'$ can be expressed in this form:
   $$
   \beta' = \frac{1}{2} \left[ \alpha_0 w_0 + \sum_{i=1}^n (x_i - \mu_0)^2 - \frac{n^2}{n+\tau_0}(\bar{x}-\mu_0)^2 \right]
   $$
   
   We introduce the prior parameters $\alpha=\alpha_0/2$ and $\beta = \alpha_0 w_0/2$, and the MLE variance $\hat{\sigma}^2_{MLE} = S_{xx}/n$.
   The posterior is $\tau|x \sim \text{Gamma}(\alpha', \beta')$ where:

   $$
   \begin{aligned}
   \alpha' &= \frac{\alpha_0 + n}{2} \\
   \beta' &= \frac{\alpha_0 w_0 + n\hat{\sigma}^2_{MLE} + \frac{n\tau_0}{n+\tau_0}(\bar{x}-\mu_0)^2}{2}
   \end{aligned}
   $$

   **Approximate Expectation of $\sigma^2$**:
   Using the approximation $E[\sigma^2|x] \approx 1/E[\tau|x] = \beta'/\alpha'$:

   $$
   E[\sigma^2|x] \approx \frac{\alpha_0 w_0 + n\hat{\sigma}^2_{MLE} + \frac{n\tau_0}{n+\tau_0}(\bar{x}-\mu_0)^2}{\alpha_0 + n}
   $$

* **Conditional Posterior of $\tau|\mu, x$**:
   If $\mu$ is considered known (or conditioned upon), we combine the terms involving $\tau$ from the joint prior and likelihood, treating $\mu$ as a fixed parameter. Note that the prior term $\pi(\mu|\tau)$ contributes an extra $\tau^{1/2}$.
   $$
   \pi(\tau|\mu, x) \propto \tau^{\alpha + \frac{n+1}{2} - 1} \exp\left\{ -\tau \left[ \beta + \frac{1}{2}\sum_{i=1}^n (x_i-\mu)^2 + \frac{\tau_0}{2}(\mu-\mu_0)^2 \right] \right\}
   $$
   Thus, $\tau|\mu, x \sim \text{Gamma}(\alpha'', \beta'')$ with shape $\alpha'' = \frac{\alpha_0 + n + 1}{2}$ and rate $\beta'' = \frac{\alpha_0 w_0 + \sum(x_i-\mu)^2 + \tau_0(\mu-\mu_0)^2}{2}$.

   The approximate expectation of the variance is:
   $$
   E[\sigma^2|\mu, x] \approx \frac{\beta''}{\alpha''} = \frac{\alpha_0 w_0 + \sum_{i=1}^n (x_i-\mu)^2 + \tau_0(\mu-\mu_0)^2}{\alpha_0 + n + 1}
   $$
:::
...
$$R(\theta, d) = \int_{X} \mathcal{L}(\theta, d(x)) f(x;\theta) dx$$
...
$$
d(x) = \underset{a}{\arg\min} \ E_{\theta|x} [ \mathcal{L}(\theta, a) ]
$$
:::
...
$$
\int_{\Theta} \mathcal{L}(\theta, d(x)) \pi(\theta|x) d\theta = E_{\theta|x} [ \mathcal{L}(\theta, d(x)) ]
$$
...
:::{.callout-important}
Therefore, to minimize the Bayes risk, one just need to  choose $d(x)$ to minimize the **posterior expected loss** for each data set $x$.
:::
...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \node [block, below of=posterior, node distance=3cm] (exp_loss) {Compute Posterior Expected Loss \\ $E_{\theta|x}[\mathcal{L}(\theta, d)]$};
...
To find the Bayes rule, we minimize the **posterior expected loss** for a given $x$, denoted as $E_{\theta|x}[\mathcal{L}(\theta, a)]$.

* **Expected Loss for choosing $a=0$ (Accept $H_0$):**
    $$
    E_{\theta|x}[\mathcal{L}(\theta, 0)] = 0 \cdot P(\theta \in \Theta_0|x) + 1 \cdot P(\theta \in \Theta_1|x) = P(\theta \in \Theta_1|x)
    $$

* **Expected Loss for choosing $a=1$ (Reject $H_0$):**
    $$
    E_{\theta|x}[\mathcal{L}(\theta, 1)] = 1 \cdot P(\theta \in \Theta_0|x) + 0 \cdot P(\theta \in \Theta_1|x) = P(\theta \in \Theta_0|x)
    $$
...
* **Expected Loss for $a=0$:**
    $$E[\mathcal{L}(\theta, 0)|x] = 0 \cdot P(\Theta_0|x) + 1 \cdot P(\Theta_1|x) = P(\Theta_1|x)$$

* **Expected Loss for $a=1$:**
    $$E[\mathcal{L}(\theta, 1)|x] = c \cdot P(\Theta_0|x) + 0 \cdot P(\Theta_1|x) = c P(\Theta_0|x)$$
...
With asymmetric costs, we accept $H_1$ only if the posterior probability of the null hypothesis is sufficiently small (below the threshold $\frac{1}{1+c}$). If the cost of false rejection $c$ is high, we require stronger evidence against $H_0$. The optimal Bayes decision rule $d(x)$ is given by:
...
$$L(y, \hat{y}) = \begin{cases} 0 & \text{if } \hat{y} = y \ (\text{Correct Classification}) \\ 1 & \text{if } \hat{y} \neq y \ (\text{Misclassification}) \end{cases}$$
...
The Bayes rule for classification is to predict the class with the highest posterior probability. While this is technically the **Maximum A Posteriori (MAP)** estimator, in the context of machine learning and pattern recognition, this decision rule is known as the **Bayes Optimal Classifier**.
...
$$
E[\mathcal{L}(\theta, d)|x] = 1 \cdot P(|\theta - d| > \delta | x) = 1 - P(d - \delta \le \theta \le d + \delta | x)
$$
...
$$
\frac{d}{dd} \left( \int_{d-\delta}^{d+\delta} \pi(\theta|x) \, d\theta \right) = \pi(d+\delta|x) - \pi(d-\delta|x) = 0
$$
...
$$
L(\theta|x) = \prod_{i=1}^n \frac{1}{\theta} e^{-x_i/\theta} = \theta^{-n} e^{-\frac{1}{\theta}\sum x_i}
$$
...
$$
\frac{\partial \ell}{\partial \theta} = -\frac{n}{\theta} + \frac{\sum x_i}{\theta^2} = 0 \implies \hat{\theta}_{MLE} = \frac{\sum x_i}{n} = \bar{x}
$$

**Minimax Estimation**

We propose the estimator $d_0(x) = \frac{\sum x_i}{n+1}$. Note that this shrinks the MLE slightly. Consider a sequence of priors $\pi_k \sim \text{Inverse-Gamma}(\alpha_k, \beta_k)$ for $\theta$, where the density is given by $\pi(\theta) \propto \theta^{-\alpha_k - 1} e^{-\beta_k / \theta}$.

We will show that $d_0$ is a minimax estimator by deriving the Bayes estimator for this sequence and comparing the risks.
...
The posterior expected loss is $E[ \mathcal{L}(\theta, d) | x ] = d^2 E[\theta^{-2} | x] - 2d E[\theta^{-1} | x] + 1$.

**Derivation of the Bayes Estimator**

Minimizing the posterior expected loss with respect to $d$ yields $d_{\pi_k} = \frac{E[\theta^{-1} | x]}{E[\theta^{-2} | x]}$. Substituting the moments derived above:

$$
d_{\pi_k}(x) = \frac{ \frac{n + \alpha_k}{T + \beta_k} }{ \frac{(n + \alpha_k)(n + \alpha_k + 1)}{(T + \beta_k)^2} } = \frac{T + \beta_k}{n + \alpha_k + 1}
$$

**Bayes Risk Limit**

The Bayes risk $r(\pi_k)$ is the expected value of the minimum posterior loss. Substituting $d_{\pi_k}$ back into the loss equation:
...
Finally, we calculate the frequentist risk for the candidate estimator $d_0(x) = \frac{T}{n+1}$. Let $Y = T/\theta \sim \text{Gamma}(n, 1)$.

$$
R(\theta, d_0) = E \left[ \left( \frac{Y}{n+1} - 1 \right)^2 \right] = \text{Var}\left(\frac{Y}{n+1}\right) + \left(E\left[\frac{Y}{n+1}\right]-1\right)^2
$$

With $E[Y]=n$ and $\text{Var}(Y)=n$, the risk is:

$$
R(\theta, d_0) = \frac{n}{(n+1)^2} + \left( \frac{n}{n+1} - 1 \right)^2 = \frac{1}{n+1}
$$

Since the risk of $d_0$ is constant and equals the limit of the Bayes risks ($\frac{1}{n+1}$), $d_0$ is a **minimax estimator**.
:::
...
* **Posterior Precision (inverse variance):**
    ...
    $$
    \frac{1}{v_{post}} = \frac{1}{\sigma^2} + \frac{1}{1} = \frac{1 + \sigma^2}{\sigma^2}
    $$
    Therefore, the posterior variance is:
    $$
    v_{post} = \frac{\sigma^2}{1 + \sigma^2}
    $$
...
* **Posterior Mean (Bayes Estimator):**
    ...
    $$
    \begin{aligned}
    E[\theta_i | X_i] &= v_{post} \left( \frac{0}{\sigma^2} + \frac{X_i}{1} \right) \\
...
$$
d^{Bayes}(X) = \left( 1 - \frac{1}{1 + \sigma^2} \right) X
$$
...
$$
E [ (\theta_i - d^{Bayes}(X_i))^2 | X_i ] = v_{post} = \frac{\sigma^2}{1 + \sigma^2}
$$

The total Bayes risk is the sum of these variances over $p$ components:

$$
r(\pi, d^{Bayes}) = \sum_{i=1}^p \frac{\sigma^2}{1 + \sigma^2} = \frac{p\sigma^2}{1 + \sigma^2}
$$
...
To prove it is minimax, we show that $p$ is the limit of the Bayes risks for a sequence of conjugate priors $\theta_i \sim N(0, \sigma^2)$. As derived above, the Bayes risk for the optimal Bayes estimator $d^{Bayes}$ is:

$$
r(\pi_{\sigma^2}, d^{Bayes}) = \frac{p\sigma^2}{1 + \sigma^2}
$$
...
Let $X \sim N_p(\theta, I)$ be a multivariate normal random vector, and let $g: \mathbb{R}^p \to \mathbb{R}^p$ be a continuously differentiable function such that $E[| \partial g_i / \partial X_i |] < \infty$. Then:
...
$$
E_\theta [ (X_i - \theta_i) g_i(X) ] = \int_{\mathbb{R}^p} (x_i - \theta_i) g_i(x) \left( \prod_{j=1}^p \phi(x_j - \theta_j) \right) dx
$$
...
Let $X \sim N_p(\theta, I)$ and consider a radial vector field of the form $g(X) = c(\|X\|^2)X$, where $c: \mathbb{R} \to \mathbb{R}$ is a differentiable scalar function. Then:
...
$$
\begin{aligned}
\nabla \cdot g(X) &= p \cdot c(\|X\|^2) + X^T (2 c'(\|X\|^2) X) \\
&= p \cdot c(\|X\|^2) + 2 c'(\|X\|^2) \|X\|^2
\end{aligned}
$$
...
* **The James-Stein Weight:** If we set $c(\|X\|^2) = \frac{1}{\|X\|^2}$, then $c'(z) = -\frac{1}{z^2}$.
    $$
    \nabla \cdot g(X) = \frac{p}{\|X\|^2} + 2 \|X\|^2 \left( -\frac{1}{\|X\|^4} \right) = \frac{p-2}{\|X\|^2}
    $$
...
$$
E_\theta \left[ (X-\theta) g(X) \right] = E_\theta \left[ g'(X) \right]
$$
...
$$
\text{RHS} = E_\theta [g'(X)] = E_\theta [2X] = 2 E_\theta [X] = 2\theta
$$
...
$$
\begin{aligned}
\text{LHS} &= E_\theta \left[ (X-\theta) X^2 \right] \\
&= E \left[ Z (Z + \theta)^2 \right] \quad \text{where } Z \sim N(0,1) \\
&= E \left[ Z (Z^2 + 2\theta Z + \theta^2) \right] \\
&= E [Z^3] + 2\theta E [Z^2] + \theta^2 E [Z]
\end{aligned}
$$
...
Let $X \sim N_p(\theta, I)$ and $g(X) = \|X\|^2 X$. We verify the Radial Field Lemma:
$$E_\theta \left[ (X - \theta)^T g(X) \right] = E_\theta \left[ p \cdot c(\|X\|^2) + 2 \|X\|^2 \cdot c'(\|X\|^2) \right]$$
...
$$
\begin{aligned}
\nabla \cdot g(X) &= p(\|X\|^2) + 2\|X\|^2(1) \\
&= (p+2)\|X\|^2
\end{aligned}
$$
The expectation is $(p+2)E_\theta[\|X\|^2]$. Since $\|X\|^2$ is a non-central $\chi^2_p$ with non-centrality parameter $\|\theta\|^2$, we know $E_\theta[\|X\|^2] = p + \|\theta\|^2$.
Thus, $\text{RHS} = (p+2)(p + \|\theta\|^2)$.


**LHS (Alignment):**
$$
\begin{aligned}
E_\theta \left[ (X-\theta)^T (\|X\|^2 X) \right] &= E_\theta \left[ \|X\|^2 (X^T X - \theta^T X) \right] \\
&= E_\theta \left[ \|X\|^4 - \theta^T X \|X\|^2 \right]
\end{aligned}
$$
...
$E[\|X\|^4] = p(p+2) + 2(p+2)\|\theta\|^2 + \|\theta\|^4$.
For the cross term $E[\theta^T X \|X\|^2]$, we find it equals $\|\theta\|^4 + (p+2)\|\theta\|^2$.

Subtracting these:
$$
\begin{aligned}
\text{LHS} &= [p(p+2) + 2(p+2)\|\theta\|^2 + \|\theta\|^4] - [\|\theta\|^4 + (p+2)\|\theta\|^2] \\
&= p(p+2) + (p+2)\|\theta\|^2 \\
&= (p+2)(p + \|\theta\|^2)
\end{aligned}
$$
...
$$
\begin{aligned}
R(\theta, d^{JS}) &= E_\theta \left[ || (X - \theta) - g(X) ||^2 \right] \\
&= E_\theta \left[ ||X - \theta||^2 \right] - 2 E_\theta \left[ (X-\theta)^T g(X) \right] + E_\theta \left[ ||g(X)||^2 \right]
\end{aligned}
$$
...
$$
\begin{aligned}
R(\theta, d^{JS}) &= p - 2(p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right] + (p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right] \\
&= p - (p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right]
\end{aligned}
$$
...
The term $(p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right]$ represents the reduction in risk provided by the James-Stein estimator.

* When the observed signal $\|X\|^2$ is large (implying $\|\theta\|^2$ is likely large), the term $1/\|X\|^2$ is small. Consequently, the risk of the James-Stein estimator approaches that of the MLE ($p$), and the improvement is negligible.

* When the observed signal $\|X\|^2$ is small (implying $\|\theta\|^2$ is likely near zero), the term $1/\|X\|^2$ is large. In this scenario, the James-Stein estimator shrinks the estimate aggressively toward zero, resulting in a risk substantially lower than $p$.

   **Risk Ratio near the Origin**:
   As the true parameter vector shrinks to zero ($\|\theta\| \to 0$), the ratio of the risks converges to a constant fraction. We substitute the expectation $E[1/\|X\|^2] = 1/(p-2)$ directly into the numerator:
   
   $$
   \lim_{\|\theta\| \to 0} \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = \frac{p - (p-2)^2 \left( \frac{1}{p-2} \right)}{p} = \frac{p - (p-2)}{p} = \frac{2}{p}
   $$
...
Because $R(\theta, d^{JS}) < p$ for all $\theta$ and $\lim_{\|\theta\| \to \infty} R(\theta, d^{JS}) = p$, the maximum risk of the James-Stein estimator is exactly $p$. Therefore, $d^{JS}$ achieves the minimax risk level and is a minimax estimator.
:::


```{r}
#| label: fig-js-risk-ggplot
#| fig-cap: "Risk comparison of James-Stein vs MLE (p=10) with approximation."
#| warning: false
...
# Scale Factor for Dual Axis (Ratio 0-1 needs to stretch to Risk 0-10)
scale_factor <- p

# Define Legend Labels using latex2exp::TeX
# We use raw strings r'(...)' to avoid escaping backslashes
labels_map <- c(
  "MLE" = TeX(r'($R(\theta, d^{MLE}) = p$)'),
  "JS" = TeX(r'($R(\theta, d^{JS}) \approx p - \frac{(p-2)^2}{p-2 + \|\theta\|^2}$)'),
...
ggplot(df, aes(x = theta_sq)) +
  ...
  geom_line(aes(y = risk_js, color = "JS", linetype = "JS"), size = 1) +
  
  # 3. Ratio Line (Green Dot-Dash) - Scaled up
  ...
  scale_y_continuous(
    name = "Risk",
    limits = c(0, p + 2),
    # Secondary Axis definition
    sec.axis = sec_axis(~ . / scale_factor, name = "Risk Ratio")
  ) +
  
  # Manual Scale for Colors and Linetypes
  scale_color_manual(
    name = NULL,
  ...
  # Theming
  labs(
    title = TeX(r'(Risk Comparison of Estimators ($p=10$))'),
...
$$
R(\theta, d^{MLE}) = E_\theta \left[ \frac{\|X - \theta\|^2}{\|\theta\|^2} \right] = \frac{1}{\|\theta\|^2} E_\theta \left[ \|X - \theta\|^2 \right]
$$
Since $X \sim N_p(\theta, I)$, we have $E[\|X - \theta\|^2] = p$.
$$
R(\theta, d^{MLE}) = \frac{p}{\|\theta\|^2}
$$
...
$$
E_\theta [\|d^{JS} - \theta\|^2] = p - (p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right]
$$
The risk under the normalized loss is simply this term scaled by $1/\|\theta\|^2$:
$$
R(\theta, d^{JS}) = \frac{1}{\|\theta\|^2} \left( p - (p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right] \right)
$$
...
$$
R(\theta, d^{MLE}) - R(\theta, d^{JS}) = \frac{1}{\|\theta\|^2} (p-2)^2 E_\theta \left[ \frac{1}{\|X\|^2} \right]
$$
...
* **Behavior near $\theta \approx 0$:**
    As $\|\theta\| \to 0$, both risks diverge to infinity. We analyze their relative performance by examining the ratio of the risks:
    
    $$
    \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = \frac{p - (p-2)^2 E_\theta [1/\|X\|^2]}{p} = 1 - \frac{(p-2)^2}{p} E_\theta \left[ \frac{1}{\|X\|^2} \right]
    $$

    At $\theta = 0$, $\|X\|^2 \sim \chi^2_p$, and the expectation is $E[1/\|X\|^2] = 1/(p-2)$. Substituting this into the ratio:

    $$
    \lim_{\|\theta\| \to 0} \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = 1 - \frac{(p-2)^2}{p(p-2)} = 1 - \frac{p-2}{p} = \frac{2}{p}
    $$
...
$$
r(\pi, d) = E_m [ E_{\theta|x} [ \mathcal{L}(\theta, d(x)) ] ]
$$
...
* **Squared Bias term:**
    Let $B = \frac{1}{1+\sigma^2}$. Then $E[\theta|x] = (1-B)x$.
...
* **Expectation of Bias term:**
    ...
    $$
    \begin{aligned}
    E_m \left[ \frac{(p-2)^2}{||X||^2} - \frac{2(p-2)}{1+\sigma^2} + \frac{||X||^2}{(1+\sigma^2)^2} \right] &= (p-2)^2 \frac{1}{(p-2)(1+\sigma^2)} - \frac{2(p-2)}{1+\sigma^2} + \frac{p(1+\sigma^2)}{(1+\sigma^2)^2} \\
...
$$E(\mu_i|x_i, \sigma^2) = \frac{\sigma^2}{1+\sigma^2} x_i = \left( 1 - \frac{1}{1+\sigma^2} \right) x_i$$
...
We need an estimator for $B = \frac{1}{1+\sigma^2}$. From the properties of the inverse Chi-square distribution, we know $E[1/\chi^2_p] = \frac{1}{p-2}$ for $p > 2$. Therefore:
$$E \left[ \frac{p-2}{S} \right] = \frac{p-2}{1+\sigma^2} E\left[\frac{1}{\chi^2_p}\right] = \frac{p-2}{1+\sigma^2} \cdot \frac{1}{p-2} = \frac{1}{1+\sigma^2} = B$$
