...
- $L(\theta, a)$: loss function measuring the loss incurred when the true parameter is $\theta$ and action $a$ is taken
...
$$
E[L(\theta, d)|x] = 1 \cdot P(|\theta - d| > \delta | x) = 1 - P(d - \delta \le \theta \le d + \delta | x)
$$
...
Let $X \sim N_p(\theta, I)$. Under the squared error loss function $L(\theta, d) = ||\theta - d||^2$, the standard Maximum Likelihood Estimator $d^0(X) = X$ is a **minimax rule**. That is, it minimizes the maximum possible risk over the parameter space:
