...
$$L(\theta, d) = \frac{|\theta - d|}{\theta}$$

This loss function penalizes errors relative to the magnitude of the true parameter $\theta$. It is particularly relevant when the scale of $\theta$ varies significantly, making a fixed absolute error (e.g., off by 5) negligible for large $\theta$ but disastrous for small $\theta$.

To find the Bayes rule, we minimize the posterior expected loss. Let $\pi(\theta|x)$ denote the posterior density.

$$
\psi(d) = E_{\theta|x} \left[ \frac{|\theta - d|}{\theta} \right] = \int \frac{|\theta - d|}{\theta} \pi(\theta|x) \, d\theta
$$

Let $w(\theta) = 1/\theta$. We can view this integral as an expectation with respect to a **weighted posterior density** $\pi^*(\theta|x)$:

$$
\pi^*(\theta|x) \propto w(\theta) \pi(\theta|x) = \frac{1}{\theta} \pi(\theta|x)
$$

Minimizing the original expected loss is equivalent to minimizing $E_{\theta|x^*} [ |\theta - d| ]$, where the expectation is taken over the weighted density $\pi^*$.

**Result:** By analogy to the absolute error case, the Bayes rule under relative absolute error loss is the **median** of the weighted posterior distribution $\pi^*(\theta|x)$.

**General Case (Weighted Median)**

For a general weight function $w(\theta)$ (where loss is $L(\theta, d) = w(\theta)|\theta - d|$), the Bayes rule is the median of the distribution with density proportional to $w(\theta)\pi(\theta|x)$.

Computational estimation of this median using samples from the standard posterior $\pi(\theta|x)$ is straightforward via **Importance Sampling**:

**Algorithm:**

1.  **Sample**: Draw $M$ independent samples $\theta_1, \dots, \theta_M$ from the standard posterior $\pi(\theta | x)$.
2.  **Weight**: Compute the importance weight for each sample:
    $$ W_i = w(\theta_i) $$
    *(For relative error, $W_i = 1/\theta_i$.)*
3.  **Sort**: Reorder the samples such that $\theta_{(1)} \le \theta_{(2)} \le \dots \le \theta_{(M)}$. Arrange the corresponding weights $W_{(1)}, \dots, W_{(M)}$ to match this order.
4.  **Accumulate**: Compute the cumulative weights $S_k = \sum_{j=1}^k W_{(j)}$.
5.  **Select**: The estimated weighted median is the value $\theta_{(k)}$ where the cumulative weight reaches half of the total weight:
    $$ \min \{ k : S_k \ge 0.5 \times S_M \} $$
...
# Assume 'theta_samples' are drawn from the standard posterior
# Define the weight function (e.g., relative error)
weight_func <- function(theta) { 1 / theta }

# 1. Compute Weights
W <- weight_func(theta_samples)

# 2. Sort samples and weights simultaneously
ord <- order(theta_samples)
theta_sorted <- theta_samples[ord]
W_sorted     <- W[ord]

# 3. Find the Weighted Median
W_cumsum <- cumsum(W_sorted)
W_total  <- sum(W_sorted)

# Find the first index where cumulative sum exceeds 50%
idx <- which(W_cumsum >= 0.5 * W_total)[1]
weighted_median <- theta_sorted[idx]
```
...
In practice, this means that to find a minimax estimator, one can often search for a "least favorable prior" that results in a Bayes estimator with a flat risk profile.
