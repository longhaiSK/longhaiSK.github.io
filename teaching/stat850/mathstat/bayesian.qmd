...
\begin{tikzpicture}[node distance=2cm, auto]
    ...
    \tikzstyle{input} = [trapezium, trapezium left angle=70, trapezium right angle=110, draw, fill=green!10, text width=5em, text centered, minimum height=3em]
    \tikzstyle{line} = [draw, -latex, thick]

    % Nodes
    \node [input] (inputs) {Prior $\pi(\theta)$ \\ Likelihood $f(x|\theta)$ \\ Data $x$};
    \node [block, below of=inputs, node distance=2.5cm] (posterior) {Compute Posterior \\ $\pi(\theta|x)$};
    \node [input, right of=posterior, node distance=4cm] (loss) {Loss Function \\ $L(\theta, d)$};
    \node [block, below of=posterior, node distance=2.5cm] (exp_loss) {Compute Posterior Expected Loss \\ $E_{\theta|x}[L(\theta, d)]$};
    \node [block, below of=exp_loss, node distance=2.5cm] (minimize) {Minimize w.r.t $d$ \\ $\arg\min_d E[L]$};
    \node [block, below of=minimize, node distance=2.5cm, fill=red!10] (bayesrule) {Bayes Rule \\ $d(x)$};
...
The **Highest Posterior Density (HPD)** interval is defined as the set of values where the posterior density exceeds a certain threshold $c$:
...
1.  **Equal-Tailed Interval:** We construct the interval $[L, U]$ such that the probability of $\theta$ being below $L$ or above $U$ is exactly $\alpha/2$.
    $$P(\theta < L | x) = \alpha/2 \quad \text{and} \quad P(\theta > U | x) = \alpha/2$$

2.  **Normal Approximation:** If the posterior is approximately normal (which is often true for large samples), we can use the posterior mean $\mu$ and standard deviation $\sigma$:
    $$(\mu - z_{\alpha/2}\sigma, \mu + z_{\alpha/2}\sigma)$$
    where $z_{\alpha/2}$ is the upper-$\alpha$ point of the standard normal distribution.
...
## Finding Minimax Estimators with Bayes Rules
