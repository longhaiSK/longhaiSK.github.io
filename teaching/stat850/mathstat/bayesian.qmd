...
We first derive the Bayes rule with respect to a specific conjugate prior. Consider the standard multivariate normal model:

$$
X | \theta \sim N_p(\theta, I)
$$

We place a centered normal prior on the unknown parameter $\theta$:

$$
\pi(\theta) \sim N_p(0, \sigma^2 I)
$$

Under the squared error loss function $L(\theta, d) = ||\theta - d||^2$, the Bayes rule $d^{Bayes}(X)$ is the expected value of the posterior distribution, $E[\theta | X]$.

To find the posterior, we examine the kernel of the posterior density $p(\theta | x) \propto f(x | \theta)\pi(\theta)$:

$$
\begin{aligned}
p(\theta | x) &\propto \exp \left\{ -\frac{1}{2} ||x - \theta||^2 \right\} \exp \left\{ -\frac{1}{2\sigma^2} ||\theta||^2 \right\} \\
&= \exp \left\{ -\frac{1}{2} \left( (x - \theta)^T(x - \theta) + \frac{1}{\sigma^2}\theta^T\theta \right) \right\}
\end{aligned}
$$

Expanding the terms in the exponent (ignoring terms not involving $\theta$):

$$
\begin{aligned}
-\frac{1}{2} \left( \theta^T \theta - 2x^T \theta + \frac{1}{\sigma^2}\theta^T \theta \right) &= -\frac{1}{2} \left( \left(1 + \frac{1}{\sigma^2}\right)\theta^T \theta - 2x^T \theta \right) \\
&= -\frac{1}{2} \left( \frac{\sigma^2 + 1}{\sigma^2} \right) \left( ||\theta||^2 - \frac{2\sigma^2}{\sigma^2 + 1} x^T \theta \right)
\end{aligned}
$$

Completing the square with respect to $\theta$, we identify this as the kernel of a normal distribution $N_p(\mu_{post}, \Sigma_{post})$ where:

$$
\mu_{post} = \frac{\sigma^2}{\sigma^2 + 1} X
$$

Thus, the Bayes rule is the posterior mean:

$$
d^{Bayes}(X) = \frac{\sigma^2}{\sigma^2 + 1} X = \left( 1 - \frac{1}{\sigma^2 + 1} \right) X
$$

This estimator shrinks the observation $X$ toward the prior mean (zero) by a factor determined by the signal-to-noise ratio.
...
To prove it is minimax, we show that $p$ is the limit of the Bayes risks for a sequence of conjugate priors $\theta \sim N(0, \sigma^2 I)$. The Bayes risk for the optimal Bayes estimator $d^{Bayes}$ is:

$$
r(\pi_{\sigma^2}, d^{Bayes}) = \frac{p\sigma^2}{\sigma^2 + 1}
$$
