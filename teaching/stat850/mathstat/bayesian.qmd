...
A Hierarchical Model can be visualized as a Directed Acyclic Graph (DAG). The diagram below illustrates the flow of information:
* $\theta$ is the global **hyperparameter** (top level).
* $\mu_i$ are the group-specific **parameters** (middle level), which depend on $\theta$.
* $X_i$ are the observed **data** (bottom level), which depend on $\mu_i$.
* The box (plate) represents the repetition for $i = 1, \dots, N$.

```{tikz fig-hierarchical-dag}
%| fig-cap: "Directed Acyclic Graph (DAG) of a Hierarchical Model"
%| echo: false
%| fig-align: "center"
%| out-extra: 'style="width: 60% !important;"'
%| engine.opts:
%|   extra.preamble: "\\usetikzlibrary{fit, positioning, shapes, backgrounds}"

\begin{tikzpicture}[
    node distance=1.5cm,
    obs/.style={circle, draw=black, fill=gray!30, thick, minimum size=1cm},
    latent/.style={circle, draw=black, thick, minimum size=1cm},
    plate/.style={draw, thick, shape=rectangle, rounded corners, inner sep=0.6cm}
]

% Nodes
\node[latent] (theta) {$\theta$};
\node[latent] (mu) [below=of theta] {$\mu_i$};
\node[obs] (x) [below=of mu] {$X_i$};

% Edges
\draw[-latex, thick] (theta) -- (mu);
\draw[-latex, thick] (mu) -- (x);

% Plate
\begin{scope}[on background layer]
    \node[plate, fit=(mu) (x), label={[anchor=south east]south east:$i=1,\dots,N$}] (p1) {};
\end{scope}
