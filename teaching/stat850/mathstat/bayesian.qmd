...
* **Conditional Posterior of $\mu|\tau, x$**:
    ...
    $$ E[\mu|\tau, x] = \frac{\tau_0\mu_0 + n\bar{x}}{\tau_0 + n} $$
   where 
      $$\tau' = \tau_0 + n$$ 
...
* **Marginal Posterior of $\tau|x$**:
    ...
    Using the approximation $E[\sigma^2|x] \approx 1/E[\tau|x] = \beta'/\alpha'$, the posterior expectation of the variance is a weighted average of the prior variance, the data variance, and the discrepancy between the prior and data means:
    
    $$
    E[\sigma^2|x] \approx \frac{\alpha_0 w_0 + n\hat{\sigma}^2_{\text{MLE}} + \frac{1}{1/n+1/\tau_0}(\bar{x}-\mu_0)^2}{\alpha_0 + n}
    $$
...
* **Conditional Posterior of $\tau|\mu, x$**:
   ...
   $$
   E[\sigma^2|\mu, x] \approx \frac{\alpha_0 w_0 + \sum_{i=1}^n (x_i-\mu)^2 + \tau_0(\mu-\mu_0)^2}{\alpha_0 + n + 1}
   $$
:::
...
$$
\boxed{ r(\pi, d) = E^X \left[ E^{\theta|X} \big( \mathcal{L}(\theta, d(X)) \big) \right] = E^\pi \left[ E^{X|\theta} \big( \mathcal{L}(\theta, d(X)) \big) \right] }
$$
...
In the second expression, the outer expectation $E^\pi$ is taken with respect to the **prior density** $\pi(\theta)$.
:::
...
$$
L(\theta|x) = \prod_{i=1}^n \frac{1}{\theta} e^{-x_i/\theta} = \theta^{-n} e^{-\frac{1}{\theta}\sum_{i=1}^n x_i}
$$

The log-likelihood function is:

$$
\ell(\theta) = \ln L(\theta|x) = -n \ln \theta - \frac{1}{\theta} \sum_{i=1}^n x_i
$$

Differentiating with respect to $\theta$ and setting to zero to find the critical point:
...
$$
\frac{n}{\theta} = \frac{\sum x_i}{\theta^2} \implies \hat{\theta}_{MLE} = \frac{\sum_{i=1}^n x_i}{n} = \bar{x}
$$
...
Let $T = \sum x_i$. The posterior density is proportional to the product of the likelihood and the prior:

$$
\begin{aligned}
\pi(\theta | x) &\propto L(\theta|x) \cdot \pi_k(\theta) \\
&\propto \left( \theta^{-n} e^{-T/\theta} \right) \cdot \left( \theta^{-\alpha_k - 1} e^{-\beta_k / \theta} \right) \\
&\propto \theta^{-(n + \alpha_k) - 1} e^{-(T + \beta_k)/\theta}
\end{aligned}
$$

This kernel corresponds to an Inverse-Gamma distribution with parameters:

* Shape: $\alpha^* = n + \alpha_k$
* Scale: $\beta^* = T + \beta_k$
...
$$
E[\theta^{-1} | x] = \frac{\alpha^*}{\beta^*} = \frac{n + \alpha_k}{T + \beta_k}
$$

$$
E[\theta^{-2} | x] = \frac{\alpha^* (\alpha^* + 1)}{(\beta^*)^2} = \frac{(n + \alpha_k)(n + \alpha_k + 1)}{(T + \beta_k)^2}
$$

**Derivation of the Bayes Estimator**

The Bayes estimator $d_{\pi_k}$ is the value that minimizes the posterior expected loss. We define the posterior expected loss function $Q(d)$:

$$
Q(d) = E[ \mathcal{L}(\theta, d) | x ] = E \left[ \left( \frac{d}{\theta} - 1 \right)^2 \Bigg| x \right]
$$

Expanding the quadratic term:

$$
Q(d) = E \left[ d^2 \theta^{-2} - 2d \theta^{-1} + 1 \Bigg| x \right]
$$

Using the linearity of expectation:

$$
Q(d) = d^2 E[\theta^{-2} | x] - 2d E[\theta^{-1} | x] + 1
$$

To find the minimum, we take the derivative with respect to $d$ and set it to zero:

$$
\frac{\partial Q(d)}{\partial d} = 2d E[\theta^{-2} | x] - 2 E[\theta^{-1} | x] = 0
$$

Solving for $d$:

$$
2d E[\theta^{-2} | x] = 2 E[\theta^{-1} | x] \implies d_{\pi_k} = \frac{E[\theta^{-1} | x]}{E[\theta^{-2} | x]}
$$

Substituting the Inverse-Gamma moments derived above:
...
$$
r(\pi_k, d_{\pi_k}) = 1 - \frac{(E[\theta^{-1}|x])^2}{E[\theta^{-2}|x]} = 1 - \frac{n + \alpha_k}{n + \alpha_k + 1} = \frac{1}{n + \alpha_k + 1}
$$

Since this expression is independent of the data $x$, the integrated Bayes risk is the same as the posterior risk. Taking the limit as the prior parameters approach zero ($\alpha_k \to 0$):
...
We compute the frequentist risk of our candidate estimator $d_0(x) = \frac{T}{n+1}$. Let $Y = T/\theta \sim \text{Gamma}(n, 1)$. Note that $E[Y]=n$ and $\text{Var}(Y)=n$.

$$
\begin{aligned}
R(\theta, d_0) &= E \left[ \left( \frac{d_0}{\theta} - 1 \right)^2 \right] = E \left[ \left( \frac{Y}{n+1} - 1 \right)^2 \right] \\
&= \text{Var}\left(\frac{Y}{n+1}\right) + \left(E\left[\frac{Y}{n+1}\right]-1\right)^2 \\
