...
$$
E_\theta [ \lVert X \rVert^2 ] = \lVert \theta \rVert^2 + p
$$
...
$$
E_\theta \left[ (X - \theta)^T g(X) \right] = E_\theta \left[ p \cdot c(\lVert X \rVert^2) + 2 \lVert X \rVert^2 \cdot c'(\lVert X \rVert^2) \right]
$$
...
* **Verifying the Mean:** If we set $c(\lVert X \rVert^2) = 1$, then $g(X) = X$. The lemma gives $E_0[\lVert X \rVert^2] = p + 2\lVert X \rVert^2(0) = p$, which is the expected value of a $\chi^2_p$ variable.
...
\end{aligned}
$$
The expectation is $(p+2)E_\theta[\lVert X \rVert^2]$. Since $\lVert X \rVert^2$ is a non-central $\chi^2_p$ with non-centrality parameter $\lVert \theta \rVert^2$, we know $E_\theta[\lVert X \rVert^2] = p + \lVert \theta \rVert^2$.
...
$$
\begin{aligned}
E_\theta \left[ (X-\theta)^T (\lVert X \rVert^2 X) \right] &= E_\theta \left[ \lVert X \rVert^2 (X^T X - \theta^T X) \right] \\
&= E_\theta \left[ \lVert X \rVert^4 - \theta^T X \lVert X \rVert^2 \right]
\end{aligned}
$$
...
$$2 E_\theta \left[ (X-\theta)^T g(X) \right] = 2 E_\theta [ \nabla \cdot g(X) ] = 2(p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]$$

The third term in the risk expansion is the squared magnitude of the shrinkage:
$$\lVert g(X) \rVert^2 = \left\lVert  \frac{p-2}{ \rVertX\lVert ^2} X \right \rVert^2 = \frac{(p-2)^2}{\lVert X \rVert^4} \lVert X \rVert^2 = \frac{(p-2)^2}{\lVert X \rVert^2}$$
...
$$
\begin{aligned}
R(\theta, d^{JS}) &= p - 2(p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right] + (p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right] \\
&= p - (p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]
\end{aligned}
$$
...
The term $(p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]$ represents the reduction in risk provided by the James-Stein estimator.

* When the observed signal $\lVert X \rVert^2$ is large (implying $\lVert \theta \rVert^2$ is likely large), the term $1/\lVert X \rVert^2$ is small. Consequently, the risk of the James-Stein estimator approaches that of the MLE ($p$), and the improvement is negligible.

* When the observed signal $\lVert X \rVert^2$ is small (implying $\lVert \theta \rVert^2$ is likely near zero), the term $1/\lVert X \rVert^2$ is large. In this scenario, the James-Stein estimator shrinks the estimate aggressively toward zero, resulting in a risk substantially lower than $p$.

   **Risk Ratio near the Origin**:
   As the true parameter vector shrinks to zero ($\lVert \theta \rVert \to 0$), the ratio of the risks converges to a constant fraction. We substitute the expectation $E[1/\lVert X \rVert^2] = 1/(p-2)$ directly into the numerator:
   
   $$
   \lim_{\lVert \theta \rVert \to 0} \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = \frac{p - (p-2)^2 \left( \frac{1}{p-2} \right)}{p} = \frac{p - (p-2)}{p} = \frac{2}{p}
   $$
...
ggplot(df, aes(x = theta_sq)) +
  ...
  labs(
      ...
      x = TeX(r'($\lVert \theta \rVert^2$ (Signal Strength))')
  ) +
  theme_minimal() +
  theme(
...
$$
R(\theta, d^{MLE}) = E_\theta \left[ \frac{\lVert X - \theta \rVert^2}{\lVert \theta \rVert^2} \right] = \frac{1}{\lVert \theta \rVert^2} E_\theta \left[ \lVert X - \theta \rVert^2 \right]
$$
Since $X \sim N_p(\theta, I)$, we have $E[\lVert X - \theta \rVert^2] = p$.
$$
R(\theta, d^{MLE}) = \frac{p}{\lVert \theta \rVert^2}
$$
...
$$
E_\theta [\lVert d^{JS} - \theta \rVert^2] = p - (p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]
$$
The risk under the normalized loss is simply this term scaled by $1/\lVert \theta \rVert^2$:
$$
R(\theta, d^{JS}) = \frac{1}{\lVert \theta \rVert^2} \left( p - (p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right] \right)
$$
...
$$
R(\theta, d^{MLE}) - R(\theta, d^{JS}) = \frac{1}{\lVert \theta \rVert^2} (p-2)^2 E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]
$$
...
* **Behavior near $\theta \approx 0$:**
    ...
    $$
    \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = \frac{p - (p-2)^2 E_\theta [1/\lVert X \rVert^2]}{p} = 1 - \frac{(p-2)^2}{p} E_\theta \left[ \frac{1}{\lVert X \rVert^2} \right]
    $$

    At $\theta = 0$, $\lVert X \rVert^2 \sim \chi^2_p$, and the expectation is $E[1/\lVert X \rVert^2] = 1/(p-2)$. Substituting this into the ratio:

    $$
    \lim_{\lVert \theta \rVert \to 0} \frac{R(\theta, d^{JS})}{R(\theta, d^{MLE})} = 1 - \frac{(p-2)^2}{p(p-2)} = 1 - \frac{p-2}{p} = \frac{2}{p}
    $$
