...
baseball_data <- data.frame(
  ...
  Season_HR = Yi,
  Season_AtBats = Ni,
  p_season = round(p_season, 3),
  true_mu = round(true_mu, 3)
)
...
### Error Analysis on Probability Scale

#### Recovering Estimated Probabilities ($\hat{p}_i$)
...
```{r}
#| label: tbl-prob-estimates-sorted
#| echo: true
...
```{r}
#| label: fig-error-prob-sorted
#| fig-cap: "Squared Error Comparison on Probability Scale (Sorted)"
#| echo: true
#| fig-height: 5
...
A key feature of Bayesian analysis is the ability to make inference about future observations, rather than just the model parameters. The **posterior predictive distribution** describes the probability of observing a new data point $x^*$ given the observed data $x$.


::: {#def-posterior-predictive}
### Posterior Predictive Distribution
Let $f(x^*|\theta)$ be the sampling distribution of a future observation $x^*$ given parameter $\theta$, and let $\pi(\theta|x)$ be the posterior distribution of $\theta$ given observed data $x$. The posterior predictive density is obtained by marginalizing over the parameter $\theta$:
...
* **Sampling Uncertainty (Aleatoric):** The inherent variability of the data generation process, represented by the variance in $f(x^*|\theta)$.
* **Parameter Uncertainty (Epistemic):** The uncertainty regarding the true value of $\theta$, represented by the variance in the posterior $\pi(\theta|x)$.
...
Consider a case where the data $x_1, \dots, x_n$ are independent and normally distributed with unknown mean $\mu$ and known variance $\sigma^2$:

$$
X_i | \mu \sim N(\mu, \sigma^2)
$$

Assume a conjugate prior for the mean: $\mu \sim N(\mu_0, \sigma_0^2)$. The posterior distribution is $\mu|x \sim N(\mu_n, \sigma_n^2)$, where $\mu_n$ and $\sigma_n^2$ are the updated posterior hyperparameters.

The predictive distribution for a new observation $x^*$ is derived as:

$$
\begin{aligned}
f(x^*|x) &= \int_{-\infty}^{\infty} f(x^*|\mu) \pi(\mu|x) \, d\mu \\
&= \int_{-\infty}^{\infty} \frac{1}{\sqrt{2\pi\sigma^2}} e^{-\frac{(x^*-\mu)^2}{2\sigma^2}} \times \frac{1}{\sqrt{2\pi\sigma_n^2}} e^{-\frac{(\mu-\mu_n)^2}{2\sigma_n^2}} \, d\mu
\end{aligned}
$$
...
$$
x^* | x \sim N(\mu_n, \sigma^2 + \sigma_n^2)
$$
...
### Bayesian Predictive Distributions of $Y_i$
...
::: {#def-predictive-estimate}
### Bayesian Predictive Estimate (Posterior Mean of $p$)
In the previous section, we estimated $\hat{p}_{Bayes}$ by transforming the posterior mean of $\mu$ (the "Plug-in" estimator):
$$ \hat{p}_{plug-in} = g^{-1}(E[\mu | \text{data}]) $$
...
1.  For each MCMC sample $t$, convert $\mu_i^{(t)}$ to probability: $p_i^{(t)} = g^{-1}(\mu_i^{(t)})$.
2.  Predict the season home runs: $Y_i^{(t)} = N_i p_i^{(t)}$.
3.  Average these predictions to get the expected season count: $\hat{Y}_i = \frac{1}{T} \sum Y_i^{(t)}$.
4.  Convert back to a rate: $\hat{p}_{pred} = \hat{Y}_i / N_i$.

This quantity $\hat{p}_{pred}$ approximates $E[p | \text{data}]$, the true posterior mean of the probability.
:::

```{r}
#| label: tbl-four-methods
#| echo: true
#| warning: false
#| message: false
...
### Comparison of Four Estimates (Probability Scale)
...
### Error Analysis of the Four Methods

We calculate the Sum of Squared Errors (SSE) for all four approaches on the probability scale.

### Assessment using Relative Standardized Error

We now evaluate the methods using a relative error metric that penalizes deviations based on the rarity of the event (home runs). The error is standardized by the distance of the true probability to the nearest boundary (0 or 1).

$$ \text{Metric}_i = \frac{|p_i^{\text{true}} - \hat{p}_i|}{\min(p_i^{\text{true}}, 1 - p_i^{\text{true}})} $$

```{r}
#| label: fig-error-relative
#| fig-cap: "Relative Error Assessment: Four Methods"
#| echo: true
#| fig-height: 6

# 1. Define the Error Metric Function
calc_metric <- function(p_hat, p_true) {
  # Avoid division by zero if p_true is exactly 0 or 1 (though not the case here)
  denom <- pmin(p_true, 1 - p_true)
  abs(p_hat - p_true) / denom
}

# 2. Calculate Errors for each method (using sorted data frame from previous steps)
# We assume df_four_sorted exists from the previous chunk
...
# 5. Legends
# Legend 1: Method Identification (Top Right)
legend("topright", 
       legend = c("MLE", "James-Stein", "Bayes (Plug-in)", "Bayes (Pred)"),
       col = c("black", "blue", "red", "darkgreen"), 
       pch = c(1, 19, 17, 15), 
       lty = c(2, 1, 1, 1),
       cex = 0.8, bg = "white")

# Legend 2: Total Error Summary (Top Middle)
legend("top", 
       title = "Sum of Relative Errors",
       legend = c(paste0("MLE: ", round(sum_mle, 3)), 
                  paste0("JS: ", round(sum_js, 3)), 
                  paste0("Plugin: ", round(sum_plugin, 3)),
                  paste0("Pred: ", round(sum_pred, 3))),
       fill = NA, border = NA, bty = "n", # No box to blend in
       cex = 0.9, horiz = TRUE, inset = c(0, 0.08)) # inset moves it down slightly
```

```{r}
#| label: fig-error-four
#| fig-cap: "Squared Error Comparison: Four Methods"
#| echo: true
#| fig-height: 5
