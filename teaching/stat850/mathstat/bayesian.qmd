...
::: {#lem-stein}
### Stein's Lemma (Integration by Parts)
...
::: {.proof name="Proof of Stein's Lemma" .collapse}
...
### An Example for Verifying Stein's Lemma with a Polynomial
...
Let $g(X) = \frac{p-2}{||X||^2}X$. We can write the James-Stein estimator as $d^{JS}(X) = X - g(X)$.
...
The first term is the risk of the MLE, which is $p$. For the second term, we apply **Stein's Lemma** (@lem-stein). We first compute the divergence of $g(X)$:

$$
\frac{\partial}{\partial X_i} \left( \frac{(p-2)X_i}{||X||^2} \right) = (p-2) \left( \frac{1}{||X||^2} - \frac{2X_i^2}{||X||^4} \right)
$$

Summing over all $i$:

$$
\nabla \cdot g(X) = (p-2) \left( \frac{p}{||X||^2} - \frac{2 \sum X_i^2}{||X||^4} \right) = \frac{(p-2)^2}{||X||^2}
$$
...
The third term in the risk expansion is:

$$
||g(X)||^2 = \sum \left( \frac{(p-2)X_i}{||X||^2} \right)^2 = (p-2)^2 \frac{||X||^2}{||X||^4} = (p-2)^2 \frac{1}{||X||^2}
$$

Substituting these back into the risk equation:
...
Since $p \ge 3$, $(p-2)^2 > 0$, and $1/||X||^2$ is a strictly positive random variable. Therefore:
$$
R(\theta, d^{JS}) < p = R(\theta, d^0) \quad \text{for all } \theta \in \mathbb{R}^p
$$

Thus, the MLE is strictly dominated by the James-Stein estimator.
:::
