...
| $E_{\boldsymbol{\theta}}[\cdot]$ | Expectation with respect to the density $f(\mathbf{x}|\boldsymbol{\theta})$ |
| $\text{Var}_{\boldsymbol{\theta}}(\cdot)$ | Variance (scalar) or Covariance Matrix (vector) |
...
| $\mathbf{I}(\boldsymbol{\theta})$ | Fisher Information matrix, $E[\mathbf{U}\mathbf{U}^\top] = E[\mathbf{J}]$ |
