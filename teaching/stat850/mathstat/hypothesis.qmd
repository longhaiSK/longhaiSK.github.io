...
Let $\tilde{\boldsymbol{\theta}} = (\boldsymbol{\theta}_0^*, \tilde{\boldsymbol{\theta}}_1)$ be the restricted MLE under $H_0$. Let $\mathbf{U}(\boldsymbol{\theta})$ be the total Score vector partitioned as $\mathbf{U} = [\mathbf{U}_0^T, \mathbf{U}_1^T]^T$, and let $\mathcal{I}(\boldsymbol{\theta})$ be the expected Fisher Information matrix with $\mathcal{I}^{00}$ representing the top-left block of its inverse $\mathcal{I}^{-1}$.

The Score statistic is $S = \mathbf{U}_0(\tilde{\boldsymbol{\theta}})^T \mathcal{I}^{00}(\tilde{\boldsymbol{\theta}}) \mathbf{U}_0(\tilde{\boldsymbol{\theta}})$. Under $H_0$, as $n \to \infty$:
...
2.  **Estimation Error of the Nuisance Parameter**
    Let $\mathbf{U}_0$ and $\mathbf{U}_1$ denote the unobservable true score vectors evaluated at the true parameters $(\boldsymbol{\theta}_0^*, \boldsymbol{\theta}_1)$. By definition, the restricted MLE $\tilde{\boldsymbol{\theta}}_1$ forces the nuisance score to zero: $\mathbf{U}_1(\boldsymbol{\theta}_0^*, \tilde{\boldsymbol{\theta}}_1) = \mathbf{0}$. 
    
    Taking a first-order Taylor expansion of $\mathbf{U}_1(\boldsymbol{\theta}_0^*, \tilde{\boldsymbol{\theta}}_1)$ around the true parameter $\boldsymbol{\theta}_1$:
    $$\mathbf{0} \approx \mathbf{U}_1 - \mathcal{I}_{11} (\tilde{\boldsymbol{\theta}}_1 - \boldsymbol{\theta}_1)$$
    Rearranging gives the asymptotic estimation error:
    $$(\tilde{\boldsymbol{\theta}}_1 - \boldsymbol{\theta}_1) \approx \mathcal{I}_{11}^{-1} \mathbf{U}_1$$

3.  **The Effective Score (Orthogonal Projection)**
    Next, we expand the evaluated score of interest $\mathbf{U}_0(\boldsymbol{\theta}_0^*, \tilde{\boldsymbol{\theta}}_1)$ around the true parameter $\boldsymbol{\theta}_1$:
    $$\mathbf{U}_0(\tilde{\boldsymbol{\theta}}) \approx \mathbf{U}_0 - \mathcal{I}_{01} (\tilde{\boldsymbol{\theta}}_1 - \boldsymbol{\theta}_1)$$
    
    Substituting the estimation error from Step 2:
    $$\mathbf{U}_0(\tilde{\boldsymbol{\theta}}) \approx \mathbf{U}_0 - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \mathbf{U}_1$$
...
4.  **Asymptotic Variance of the Effective Score**
    ...
    $$\text{Cov}(\mathbf{U}_0(\tilde{\boldsymbol{\theta}}), \mathbf{U}_1) \approx \text{Cov}(\mathbf{U}_0, \mathbf{U}_1) - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \text{Var}(\mathbf{U}_1) = \mathcal{I}_{01} - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \mathcal{I}_{11} = \mathbf{0}$$
    ...
    $$\text{Var}(\mathbf{U}_0(\tilde{\boldsymbol{\theta}})) = \mathcal{I}_{00} - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \mathcal{I}_{10}$$
    By the block matrix inversion theorem, this Schur complement is exactly the inverse of the top-left block of $\mathcal{I}^{-1}$:
    $$\text{Var}(\mathbf{U}_0(\tilde{\boldsymbol{\theta}})) = (\mathcal{I}^{00})^{-1}$$

5.  **Distributional Conclusion**
    Because the evaluated score $\mathbf{U}_0(\tilde{\boldsymbol{\theta}})$ is asymptotically normal with mean zero and variance $(\mathcal{I}^{00})^{-1}$, standardizing it by its variance creates a quadratic form that follows a Chi-square distribution:
    $$S = \mathbf{U}_0(\tilde{\boldsymbol{\theta}})^T \mathcal{I}^{00} \mathbf{U}_0(\tilde{\boldsymbol{\theta}}) \xrightarrow{d} \chi^2_q$$
...
1.  **Find the Restricted MLEs:** Under $H_0$, we fix $\mu = \mu_0$. Maximizing the restricted log-likelihood with respect to the nuisance parameter $\sigma^2$ yields:
    ...
    The restricted parameter vector is $\tilde{\boldsymbol{\theta}} = (\mu_0, \tilde{\sigma}^2)^T$.

2.  **Evaluate the Score Vector:** The gradient of the log-likelihood with respect to $\mu$ is $U_\mu(\mu, \sigma^2) = \frac{n(\bar{X} - \mu)}{\sigma^2}$. Evaluated at the restricted MLE:
    $$U_\mu(\tilde{\boldsymbol{\theta}}) = \frac{n(\bar{X} - \mu_0)}{\tilde{\sigma}^2}$$
    By definition, the score for the nuisance parameter evaluated at the restricted MLE is zero: $U_{\sigma^2}(\tilde{\boldsymbol{\theta}}) = 0$.
...
3.  **Determine the Fisher Information:** The expected Fisher Information matrix for a Normal distribution is diagonal because the mean and variance are orthogonal parameters:
    ...
    Because the matrix is diagonal, the top-left block of the inverse $\mathcal{I}^{-1}$ is simply the reciprocal of the top-left block of $\mathcal{I}$. Evaluated at $\tilde{\boldsymbol{\theta}}$:
    $$\mathcal{I}^{\mu\mu}(\tilde{\boldsymbol{\theta}}) = \frac{\tilde{\sigma}^2}{n}$$

4.  **Compute the Score Statistic $S$:** Constructing the quadratic form for the parameter of interest:
    $$S = U_\mu(\tilde{\boldsymbol{\theta}})^T \mathcal{I}^{\mu\mu}(\tilde{\boldsymbol{\theta}}) U_\mu(\tilde{\boldsymbol{\theta}})$$
    $$S = \left( \frac{n(\bar{X} - \mu_0)}{\tilde{\sigma}^2} \right) \left( \frac{\tilde{\sigma}^2}{n} \right) \left( \frac{n(\bar{X} - \mu_0)}{\tilde{\sigma}^2} \right) = \frac{n(\bar{X} - \mu_0)^2}{\tilde{\sigma}^2}$$
...
5.  **Asymptotic Distribution:** By the Score Theorem, since we are testing $q=1$ parameter restriction, the statistic follows a $\chi^2_1$ distribution:
    $$S = \frac{n(\bar{X} - \mu_0)^2}{\tilde{\sigma}^2} \xrightarrow{d} \chi^2_1$$
