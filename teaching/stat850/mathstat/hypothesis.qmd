...
1.  **Conflict of Optimal Regions:**
    Consider a specific alternative $\theta_1 > \theta^*$. By the Neyman-Pearson Lemma, the MP test $\phi_1$ rejects $H_0$ for large values of the sufficient statistic $T(\mathbf{X}) > k_1$.
    Conversely, consider an alternative $\theta_2 < \theta^*$. The MP test $\phi_2$ rejects $H_0$ for small values of the statistic $T(\mathbf{X}) < k_2$.
...
Under the null hypothesis $H_0: \boldsymbol{\theta}_0 = \boldsymbol{\theta}_0^*$, let $\hat{\boldsymbol{\theta}}$ be the unrestricted MLE and $\tilde{\boldsymbol{\theta}}$ be the restricted MLE. For large $n$, we expand the log-likelihood $\ell(\boldsymbol{\theta})$ around the unrestricted peak $\hat{\boldsymbol{\theta}}$. Using the expected Fisher Information $\mathcal{I} = \mathcal{I}(\boldsymbol{\theta}^*)$, we have:
$$\ell(\boldsymbol{\theta}) \approx \ell(\hat{\boldsymbol{\theta}}) - \frac{n}{2} (\hat{\boldsymbol{\theta}} - \boldsymbol{\theta})^T \mathcal{I} (\hat{\boldsymbol{\theta}} - \boldsymbol{\theta})$$
The likelihood ratio statistic $D = 2[\ell(\hat{\boldsymbol{\theta}}) - \ell(\tilde{\boldsymbol{\theta}})]$ can thus be expressed as the squared Mahalanobis distance between the unrestricted and restricted estimators:
$$D \approx n \|\hat{\boldsymbol{\theta}} - \tilde{\boldsymbol{\theta}}\|_{\mathcal{I}}^2 = n (\hat{\boldsymbol{\theta}} - \tilde{\boldsymbol{\theta}})^T \mathcal{I} (\hat{\boldsymbol{\theta}} - \tilde{\boldsymbol{\theta}})$$

**2. Constrained Optimization and the Linear Identity**
Finding the restricted MLE $\tilde{\boldsymbol{\theta}}$ is a constrained optimization problem: $\min_{\boldsymbol{\theta}} \frac{1}{2} (\boldsymbol{\theta} - \hat{\boldsymbol{\theta}})^T \mathcal{I} (\boldsymbol{\theta} - \hat{\boldsymbol{\theta}})$ subject to $\boldsymbol{\theta}_0 = \boldsymbol{\theta}_0^*$. Solving the first-order conditions for the nuisance parameter $\boldsymbol{\theta}_1$ yields the following identity for the displacement vector:

$$\tilde{\boldsymbol{\theta}} - \hat{\boldsymbol{\theta}} = \begin{pmatrix} \boldsymbol{\theta}_0^* - \hat{\boldsymbol{\theta}}_0 \\ \tilde{\boldsymbol{\theta}}_1 - \hat{\boldsymbol{\theta}}_1 \end{pmatrix} = \begin{pmatrix} \mathbf{I} \\ -\mathcal{I}_{11}^{-1} \mathcal{I}_{10} \end{pmatrix} (\boldsymbol{\theta}_0^* - \hat{\boldsymbol{\theta}}_0)$$

This shows that $\tilde{\boldsymbol{\theta}}$ is the **orthogonal projection** of $\hat{\boldsymbol{\theta}}$ onto the constraint subspace under the $\mathcal{I}$-metric. The term $-\mathcal{I}_{11}^{-1} \mathcal{I}_{10}$ adjusts the nuisance parameters to account for their correlation with the parameters of interest.

**3. The Pythagorean Decomposition**
Because $\tilde{\boldsymbol{\theta}}$ is a projection, the generalized Pythagorean theorem applies to the total error:
$$\underbrace{n\|\hat{\boldsymbol{\theta}} - \boldsymbol{\theta}^*\|_{\mathcal{I}}^2}_{\text{Total distance } (p \text{ d.f.})} \approx \underbrace{n\|\hat{\boldsymbol{\theta}} - \tilde{\boldsymbol{\theta}}\|_{\mathcal{I}}^2}_{\text{Wilks' Statistic } D} + \underbrace{n\|\tilde{\boldsymbol{\theta}} - \boldsymbol{\theta}^*\|_{\mathcal{I}}^2}_{\text{Restricted error } (p-q \text{ d.f.})}$$

**4. Matrix Form and Distribution**
Substituting the linear identity into the quadratic form for $D$:
$$D \approx n (\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*)^T \underbrace{\left[ \mathcal{I}_{00} - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \mathcal{I}_{10} \right]}_{(\mathcal{I}^{00})^{-1}} (\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*)$$
Since $\sqrt{n}(\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*) \xrightarrow{d} N(\mathbf{0}, \mathcal{I}^{00})$, the quadratic form $D$ is a standardized normal vector squared, leading to the distributional conclusion:
$$D \xrightarrow{d} \chi^2_q$$

:::

**R Illustration: Geometry of the Projection**

The following R code visualizes how the restricted MLE $\tilde{\boldsymbol{\theta}}$ is found by projecting the unrestricted MLE onto the null hypothesis line according to the curvature of the likelihood contours.

```{r}
#| label: fig-lrt-geometry
#| fig-cap: "The geometry of restricted optimization. Blue dot is the unrestricted MLE. Red dot is the restricted MLE."
library(ggplot2)

# 1. Setup parameters and Fisher Information
theta_hat <- c(2, 2)      
theta_star_0 <- 1         
I <- matrix(c(1, 0.6, 0.6, 1), nrow = 2) 

# 2. Calculate theta_tilde using the identity
delta_theta_0 <- theta_star_0 - theta_hat[1]
theta_tilde_1 <- theta_hat[2] - (1/I[2,2]) * I[2,1] * delta_theta_0
theta_tilde <- c(theta_star_0, theta_tilde_1)

# 3. Generate grid for quadratic log-likelihood contours
...
ggplot(grid, aes(x = theta0, y = theta1)) +
  ...
  geom_vline(xintercept = theta_star_0, linetype = "dashed", color = "red") +
  ...
  annotate("point", x = theta_tilde[1], y = theta_tilde[2], color = "red", size = 3) +
  annotate("text", x = theta_tilde[1] - 0.2, y = theta_tilde[2], 
           label = "tilde(theta)", parse = TRUE, color = "red") +
