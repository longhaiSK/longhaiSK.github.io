...
## Wilks' Theorem for General Likelihood Ratio Tests
...
Let $X_1, \dots, X_n$ be an i.i.d. sample from a regular family of distributions with parameters $\theta \in \Theta \subseteq \mathbb{R}^{p_1}$. Consider the test:
$$ H_0: \theta \in \Theta_0 \quad \text{vs} \quad H_1: \theta \in \Theta \setminus \Theta_0 $$
where $\Theta_0$ is a smooth subset of $\Theta$ with dimension $p_0 < p_1$. Let $\Lambda_n$ be the likelihood ratio statistic. Under $H_0$, as $n \to \infty$:
$$ -2 \ln \Lambda_n \xrightarrow{d} \chi^2_{k} $$
where the degrees of freedom $k = p_1 - p_0$ is the difference in the number of free parameters between the full model and the null model.
...
1.  **Quadratic Approximation:**
    ...
    From the asymptotic normality of the MLE, $\sqrt{n}(\hat{\theta} - \theta_0) \xrightarrow{d} N(0, I(\theta_0)^{-1})$. Therefore, the quadratic form converges to a Chi-square variable with $p_1$ degrees of freedom:
    $$ 2[\ell(\hat{\theta}) - \ell(\theta_0)] \xrightarrow{d} \chi^2_{p_1} $$
...
2.  **Decomposition of the Statistic:**
    ...
    $$ 2[\ell(\hat{\theta}) - \ell(\hat{\theta}_0)] = \underbrace{2[\ell(\hat{\theta}) - \ell(\theta_0)]}_{\approx \chi^2_{p_1}} - \underbrace{2[\ell(\hat{\theta}_0) - \ell(\theta_0)]}_{\approx \chi^2_{p_0}} $$
    The term $2[\ell(\hat{\theta}_0) - \ell(\theta_0)]$ represents the deviation of the *restricted* MLE from the truth. Since $\Theta_0$ has dimension $p_0$, this term converges to $\chi^2_{p_0}$.
...
3.  **Result via Cochran's Theorem Logic:**
    ...
    $$ D \xrightarrow{d} \chi^2_{p_1} - \chi^2_{p_0} \sim \chi^2_{p_1 - p_0} = \chi^2_k $$
