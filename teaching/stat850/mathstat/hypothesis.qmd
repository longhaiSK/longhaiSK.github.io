...
Because the family has a Monotone Likelihood Ratio (MLR) in $T(x)$, for any specific alternative $\theta_1 > \theta_0$, the likelihood ratio $\Lambda(x)$ is an increasing function of $T(x)$. Therefore, the rejection region $T(x) > t_0$ corresponds to $\Lambda(x) > k$. By the Neyman-Pearson Lemma, $\phi_0(x)$ is the Most Powerful (MP) test for testing $H_0': \theta = \theta_0$ vs $H_1': \theta = \theta_1$.

**Monotonicity of the Power Function**

**Proof 1:**

We claim that $W_{\phi_0}(\theta)$ is a non-decreasing function of $\theta$.

Let $\theta_2 < \theta_1$. Consider testing $\theta = \theta_2$ vs $\theta = \theta_1$. Let $\beta = W_{\phi_0}(\theta_2)$. Define a constant dummy test $\phi^*(x) = \beta$ for all $x$. The power of this test is constant: $W_{\phi^*}(\theta) = \beta$. Since $\phi_0(x)$ corresponds to the likelihood ratio test form for $\theta_2$ vs $\theta_1$, it is the MP test of size $\beta$. By the optimality of the NP Lemma:

$$W_{\phi_0}(\theta_1) \ge W_{\phi^*}(\theta_1) = \beta = W_{\phi_0}(\theta_2)$$

Thus, $W_{\phi_0}(\theta)$ is non-decreasing.

**Proof 2:**

We claim that $W_{\phi_0}(\theta)$ is increasing in $\theta$ by showing that the family of densities has the *Stochastic Increasing Property*.

Let $\theta_1 < \theta_2$. Let $f_1(t)$ and $f_2(t)$ be the densities of $T$ under $\theta_1$ and $\theta_2$.

1.  **Ratio Behavior:**
    By MLR, the likelihood ratio $r(t) = \frac{f_2(t)}{f_1(t)}$ is a strictly increasing function of $t$.

2.  **Single Crossing Property:**
    Since both $f_1$ and $f_2$ integrate to 1, the ratio $r(t)$ cannot be always $>1$ or always $<1$. It must cross the value 1 exactly once.
    Since $r(t)$ is increasing, it crosses from below. Thus, there exists a crossing point $t^*$ such that:
    $$
    f_2(t) < f_1(t) \quad \text{for } t < t^*
    $$
    $$
    f_2(t) > f_1(t) \quad \text{for } t > t^*
    $$
    Geometrically, $f_2$ moves probability mass from the left of $t^*$ to the right of $t^*$.

3.  **Tail Probability Comparison:**
    This "mass transfer" to the right implies **First Order Stochastic Dominance**: for *any* cutoff $t_0$, the probability of exceeding $t_0$ is higher under $\theta_2$.
    
    $$ P_{\theta_2}(T > t_0) \ge P_{\theta_1}(T > t_0) $$

    Since the power function is exactly this tail probability ($W(\theta) = P_\theta(T > t_0)$), $W(\theta)$ is non-decreasing.


**Size Control**

Since $W_{\phi_0}(\theta)$ is non-decreasing and we set $W_{\phi_0}(\theta_0) = \alpha$:
...
Let $\phi'(x)$ be any other test with size $\le \alpha$ for $H_0: \theta \le \theta_0$. This implies $W_{\phi'}(\theta_0) \le \alpha$. For any specific $\theta_1 > \theta_0$, we treat the problem as testing $\theta_0$ vs $\theta_1$. Since $\phi_0(x)$ is the MP test for $\theta_0$ vs $\theta_1$ (from Step 1), and $\phi'$ is a valid competitor (size $\le \alpha$ at $\theta_0$), we have:

$$W_{\phi_0}(\theta_1) \ge W_{\phi'}(\theta_1)$$

Since this holds for all $\theta_1 > \theta_0$, $\phi_0(x)$ is the Uniformly Most Powerful (UMP) test.
:::
...
# Force the use of the virtual environment
...
```{r}
#| echo: false
#| fig-cap: "Visualizing Stochastic Ordering (Mass Transfer)"
...
crossing_point <- (mu1 + mu2) / 2
y_cross <- dnorm(crossing_point, mean = mu1, sd = sigma)

# Plotting
plot(x, f1, type = "l", col = "blue", lwd = 2, 
     ylab = "Density", xlab = "T(x)", 
     main = "Visualizing Stochastic Ordering (Mass Transfer)")
lines(x, f2, col = "red", lwd = 2)

# Shading the "Deficit" (Mass Removed) - Area where f1 > f2 (Left)
idx_left <- x < crossing_point
x_poly_left <- c(x[idx_left], crossing_point, rev(x[idx_left]))
y_poly_left <- c(f1[idx_left], y_cross, rev(f2[idx_left]))

polygon(x_poly_left, y_poly_left, 
        col = rgb(0, 0, 1, 0.2), border = NA)

# Shading the "Surplus" (Mass Added) - Area where f2 > f1 (Right)
idx_right <- x > crossing_point
x_poly_right <- c(crossing_point, x[idx_right], crossing_point)
# Note: For the polygon to close correctly back to the start, we trace f2 then f1 back
x_poly_right <- c(x[idx_right], crossing_point, rev(x[idx_right])) 
y_poly_right <- c(f2[idx_right], y_cross, rev(f1[idx_right]))

polygon(x_poly_right, y_poly_right, 
        col = rgb(1, 0, 0, 0.2), border = NA)

# Add Lines
abline(v = crossing_point, col = "black", lty = 2)
abline(v = t0, col = "darkgreen", lwd = 2)

# Legend
legend("topleft", legend = c("f1 (Lower theta)", "f2 (Higher theta)"),
       col = c("blue", "red"), lwd = 2, bty = "n")
text(crossing_point, max(f1)*0.9, "Crossing Point t*", pos = 4)
text(t0, max(f1)*0.8, "Threshold t0", pos = 4, col = "darkgreen")
```
