...
### Wilks' Theorem for Likelihood Ratio Test

Let $\mathbf{Y} = (Y_1, \dots, Y_n)$ be an i.i.d. sample from a regular family of distributions parameterized by $\boldsymbol{\theta}$. Partition the parameter vector as $\boldsymbol{\theta} = (\boldsymbol{\theta}_0^T, \boldsymbol{\theta}_1^T)^T$, where $\boldsymbol{\theta}_0$ is the $q$-dimensional parameter of interest and $\boldsymbol{\theta}_1$ is the $(p-q)$-dimensional nuisance parameter. 

Consider the test:
$$H_0: \boldsymbol{\theta}_0 = \boldsymbol{\theta}_0^* \quad \text{vs} \quad H_1: \boldsymbol{\theta}_0 \neq \boldsymbol{\theta}_0^*$$

Let $\hat{\boldsymbol{\theta}} = (\hat{\boldsymbol{\theta}}_0^T, \hat{\boldsymbol{\theta}}_1^T)^T$ be the unrestricted MLE, and let $\tilde{\boldsymbol{\theta}} = ({\boldsymbol{\theta}_0^*}^T, \tilde{\boldsymbol{\theta}}_1^T)^T$ be the restricted MLE under $H_0$. 

The likelihood ratio statistic is $D = 2[\ell(\hat{\boldsymbol{\theta}}) - \ell(\tilde{\boldsymbol{\theta}})]$. Under $H_0$, as $n \to \infty$:
$$D \xrightarrow{d} \chi^2_{q}$$
where $q$ is the dimension of the restricted parameter $\boldsymbol{\theta}_0$.

**Algebraic Proof via Direct Taylor Expansion**

**1. Taylor Expansion of the Log-Likelihood**
We expand the restricted log-likelihood $\ell(\tilde{\boldsymbol{\theta}})$ around the unrestricted MLE $\hat{\boldsymbol{\theta}}$:
...
Because $\hat{\boldsymbol{\theta}}$ maximizes the unrestricted likelihood, the score vector vanishes: $\nabla \ell(\hat{\boldsymbol{\theta}}) = \mathbf{0}$. By the Law of Large Numbers, the sample Hessian converges to the expected Fisher Information matrix, meaning $-\frac{1}{n} \nabla^2 \ell(\hat{\boldsymbol{\theta}}) \xrightarrow{p} \mathcal{I}(\boldsymbol{\theta})$. Substituting this into the likelihood ratio statistic:
...
**2. Partitioning the Information Matrix**
We partition the expected Fisher Information matrix conformably with $\boldsymbol{\theta} = (\boldsymbol{\theta}_0^T, \boldsymbol{\theta}_1^T)^T$:
...
**3. Approximating the Restricted Nuisance Parameter**
To evaluate the quadratic form, we need the relationship between the restricted nuisance parameter estimate $\tilde{\boldsymbol{\theta}}_1$ and the unrestricted estimate $\hat{\boldsymbol{\theta}}_1$. We expand the score function for the nuisance parameter, $U_1(\boldsymbol{\theta}) = \frac{\partial \ell}{\partial \boldsymbol{\theta}_1}$, evaluated at $\tilde{\boldsymbol{\theta}}$, around $\hat{\boldsymbol{\theta}}$:
...
By definition, $U_1(\tilde{\boldsymbol{\theta}}) = \mathbf{0}$ (since $\tilde{\boldsymbol{\theta}}_1$ maximizes the restricted likelihood) and $U_1(\hat{\boldsymbol{\theta}}) = \mathbf{0}$ (from the unrestricted MLE). Replacing the sample second derivatives with their asymptotic expectations ($-n\mathcal{I}_{10}$ and $-n\mathcal{I}_{11}$):
...
**4. Evaluating the Quadratic Form**
Substitute this relationship back into the partitioned quadratic form from Step 1:
...
Carrying out the block matrix multiplication simplifies the expression strictly in terms of the parameter of interest $\boldsymbol{\theta}_0$:
...
By the standard formula for block matrix inversion, the bracketed term $[ \mathcal{I}_{00} - \mathcal{I}_{01} \mathcal{I}_{11}^{-1} \mathcal{I}_{10} ]$ is exactly the inverse of the top-left block of $\mathcal{I}^{-1}(\boldsymbol{\theta})$. Let us define this asymptotic variance block as $\mathcal{I}^{00}$. Thus:
$$D \approx n (\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*)^T (\mathcal{I}^{00})^{-1} (\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*)$$
From the asymptotic normality of the unrestricted MLE, we know that $\sqrt{n}(\hat{\boldsymbol{\theta}}_0 - \boldsymbol{\theta}_0^*) \xrightarrow{d} N(\mathbf{0}, \mathcal{I}^{00})$. 

Because $D$ is geometrically equivalent to a quadratic form of a normal random vector standardized by its own inverse covariance matrix, it converges to a Chi-square distribution with degrees of freedom equal to the dimension of $\boldsymbol{\theta}_0$ (which is $q$):
