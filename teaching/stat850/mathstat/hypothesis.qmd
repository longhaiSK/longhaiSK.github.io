...
```{r}
#| label: fig-lrt-normal-x
#| fig-cap: "Illustration of Test Function, Size, and Power"
#| fig-width: 7
#| fig-height: 5
...
```{r}
#| label: fig-lrt-proof-transparent-pink
#| fig-cap: "Visualizing the NP Lemma: The thick, transparent pink line is $\\phi_{LRT}$. The thin solid black line is $\\phi_{Other}$. Overlap is visible as a black line inside pink."
#| fig-width: 7
#| fig-height: 5
...
ggplot() +
  ...
  # alpha = 0.4 makes it look like a highlighter
  geom_segment(data = df_lrt,
               aes(x = x_start, xend = x_end, 
                   y = y_val, yend = y_val, 
                   color = Test, linetype = Test, size = Test),
               alpha = 0.4) + # Transparency added here
  ...
  scale_color_manual(name = "Test Function",
                     values = c("phi[LRT]" = "deeppink", "phi[Other]" = "black"),
                     labels = c(expression(phi[LRT]), expression(phi[Other]))) +
  scale_linetype_manual(name = "Test Function",
                        values = c("phi[LRT]" = "solid", "phi[Other]" = "solid"),
                        labels = c(expression(phi[LRT]), expression(phi[Other]))) +
  scale_size_manual(name = "Test Function",
                    values = c("phi[LRT]" = 2,   # Extra Thick
                               "phi[Other]" = 0.8), # Thin
                    labels = c(expression(phi[LRT]), expression(phi[Other]))) +
