...
4.  **Action Space ($\mathcal{A}$):**
    ...
    * *Example (Hypothesis Testing):*
        $H_0$ vs $H_1$.
        $\mathcal{A} = \{0, 1\}$ where $0$ means "Accept $H_0$" and $1$ means "Reject $H_0$".
    * *Example (Point Estimation):*
        $\mathcal{A} = \Theta$. An action is an estimate of $\theta$.
...
* *Examples:*
    * Point Estimation: $d(x) = \bar{x}$ (Sample mean).
    * Hypothesis Testing: $d(x) = 1$ if $\bar{x} > c$, else $0$.
...
If $L(\theta, a) = (\theta - a)^2$ and $d(x) = \bar{x}$:

$$R(\theta, d) = E_\theta[(\bar{x} - \theta)^2] = \text{MSE} = \text{Var}(\bar{x}) + \text{Bias}^2$$

#### Example: Hypothesis Testing Risk
For testing $H_0$ vs $H_1$ with 0-1 loss:

* If $\theta \in H_0$:
    $R(\theta, d) = 1 \cdot P_\theta(d(X)=1) + 0 = P(\text{Type I Error})$.
* If $\theta \in H_1$:
    $R(\theta, d) = 1 \cdot P_\theta(d(X)=0) + 0 = P(\text{Type II Error})$.
...
## Example: The "Great Aunt's Necklace"

**Scenario:**
Two boxes. One contains a real necklace ($\theta=1$), the other an imitation ($\theta=2$).
Loss is 1 if we choose the wrong box, 0 otherwise.

* $\theta \in \{1, 2\}$
* Action $a \in \{1, 2\}$ (Choose Box 1 or Box 2)

**Data ($X$):**
Great Aunt's judgment.
$X \in \{1, 2\}$ (Aunt says Box 1 or Box 2).
Probabilities:
