...
### Summary of Key Asymptotic Results
...
$$
\text{Var}(\mathbf{U}) = \begin{bmatrix} \frac{n}{\sigma^2} & 0 \\ 0 & \frac{n}{2\sigma^4} \end{bmatrix} = \mathcal{I}(\boldsymbol{\theta})
$$
...
3. Alternative Derivation via Score Statistic

   The Score Statistic $S$ approximates the Deviance using the gradient at the null hypothesis:
   $$
   S = U(\mu_0)^T \mathcal{I}^{-1} U(\mu_0)
   $$
   Substituting the components we derived earlier ($U(\mu_0) = \frac{n}{\sigma^2}(\hat{\mu} - \mu_0)$ and $\mathcal{I} = \frac{n}{\sigma^2}$):
   $$
   \begin{aligned}
   S &= \left[ \frac{n}{\sigma^2}(\hat{\mu} - \mu_0) \right] \left[ \frac{n}{\sigma^2} \right]^{-1} \left[ \frac{n}{\sigma^2}(\hat{\mu} - \mu_0) \right] \\
   &= \frac{n^2}{\sigma^4}(\hat{\mu} - \mu_0)^2 \cdot \frac{\sigma^2}{n} \\
   &= \frac{n}{\sigma^2}(\hat{\mu} - \mu_0)^2
   \end{aligned}
   $$
   **Result:** This is identical to the Taylor expansion of the Deviance derived above. For the Normal distribution with known variance, the log-likelihood is a perfect quadratic, so the "approximation" $D \approx S$ becomes an exact equality.
