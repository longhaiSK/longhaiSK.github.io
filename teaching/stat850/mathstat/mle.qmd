...
```{tikz}
%| label: fig-asymptotic-relationships-v3
%| fig-cap: "Asymptotic Relationships: The Fisher Information $\\mathcal{I}$ bridges the spaces, and can itself be approximated by the outer product of Scores or Errors."
%| echo: false
%| fig-align: "center"
%| engine.opts:
...
\begin{tikzpicture}[
    ...
    \node[block] (score) at (6, 4) {
        \textbf{Gradient Space} \\
        Score ($U$)
    };

    \node[block] (dev) at (3, 0) {
        \textbf{Likelihood Space} \\
        Deviance ($D$)
    };

    \node[infoblock] (info) at (3, 7) {
    ...
    % Score -> Info
    \draw[dashed_arrow] (score) to[bend right=20] node[midway, right=0.1cm, align=left] {
        $\mathcal{I} \approx U U^T$ \\
        (\textcolor{red}{Outer Product})
    } (info);

    % Error -> Info (Less common but valid algebraically if scaled)
    % \mathcal{I} \approx \mathcal{I} \Delta \theta \Delta \theta^T \mathcal{I} is essentially UU^T
    % We will represent this as coming from the Error side but passing through the Score logic
    \draw[dashed_arrow] (error) to[bend left=20] node[midway, left=0.1cm, align=right] {
        $\mathcal{I} \approx (\mathcal{I}\Delta\theta)(\mathcal{I}\Delta\theta)^T$ \\
        (\textcolor{red}{Scaled Outer Product})
    } (info);
