...
```{r}
#| label: fig-base-r-viz-expected-loglik-star
#| fig-cap: "Expected Log-Likelihood Q*(theta) and 10 realized ell*(theta) curves. The largest sample is highlighted in blue, showing the 1/2 Deviance gap. The MLEs are marked on the Score = 0 line."
#| fig-pos: "H"
#| 
library(tikzDevice)
library(tools)
...
# 2. Open TikZ device in standalone mode
tikz(tex_file, width = 7, height = 5, standAlone = TRUE)
...
lines(x_vals, expected_log_lik, lwd = 3, col = "black")
...
segments(x0 = x_bar_max, y0 = null_y, x1 = x_bar_max, y1 = peak_y, col = highlight_color, lwd = 3, lty = 1)
text(x_bar_max, (null_y + peak_y)/2, labels = "$\\frac{1}{2}D$", pos = 4, col = highlight_color, font = 2, cex = 1)
...
legend("topleft", 
       legend = c("Expected $Q^*(\\theta)$", "Realized $\\ell^*(\\theta)$", "True Null $\\theta_0$", "Score Function $U(\\theta)$"),
       col = c("black", grey_color, null_color, score_color),
       lty = c(1, 2, 1, 1), lwd = c(3, 1, 2, 3), bg = "white", cex = 0.85)


# --- END OF PLOT CODE ---

# 3. Close the device silently to prevent "quartz_off_screen" printing
invisible(dev.off())

# 4. Compile the .tex file to a permanent .pdf
tools::texi2pdf(tex_file, clean = TRUE)

# 5. Route the correct file type based on the output format
if (knitr::is_html_output()) {
  # If compiling to HTML, convert the PDF to a 300 DPI PNG
  if (!requireNamespace("pdftools", quietly = TRUE)) install.packages("pdftools")
  png_file <- "fig_expected_loglik.png"
  
  # capture.output prevents pdftools from printing conversion text to your document
  capture.output(
    pdftools::pdf_convert(pdf_file, format = "png", dpi = 300, filenames = png_file)
  )
  knitr::include_graphics(png_file)
  
} else {
  # If compiling to Typst or LaTeX, use the pure vector PDF
