...
2. Visualizing Log-Sampling Distribution vs. Log-Likelihoods
...
3. Asympototic Distributions
    ...
    4. **Wilks' Theorem (Deviance Approximation):**
        For $H_0: \theta = \theta_0$, the Deviance is $D = 2[\ell(\hat{\theta}) - \ell(\theta_0)]$.
        
        * **Step A: Taylor Expansion:** Because the log-likelihood is exactly quadratic, the expansion around the MLE $\hat{\theta}$ is exact:
        $$
        \ell(\theta_0) = \ell(\hat{\theta}) + 0 + \frac{1}{2}(\theta_0 - \hat{\theta})^2 \left(-\frac{n}{\sigma^2}\right)
        $$
        
        * **Step B: Exactness:**
        $D = \frac{n}{\sigma^2}(\hat{\theta} - \theta_0)^2$. The Deviance is algebraically identical to the Wald statistic.
        
        * **Step C: Equivalence:**
        $$D = (\hat{\theta} - \theta_0)^2 \mathcal{I}_n(\theta_0) = U_n(\theta_0)^T \mathcal{I}_n(\theta_0)^{-1} U_n(\theta_0)$$
        
        * **Step D: Conclusion:**
        Since $(\hat{\theta} - \theta_0) \sim N(0, \sigma^2/n)$, the statistic $D$ is the square of a standard Normal variable:
        $$D = Z^2 \sim \chi^2_1$$
