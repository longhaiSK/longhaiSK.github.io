...
1. Derivation of Key Quantities
    ...
    * **Log-Likelihood:**
        Using the identity $\sum (x_i - \theta)^2 = \sum (x_i - \overline{x})^2 + n(\overline{x} - \theta)^2$, the log-likelihood is a perfect quadratic centered at $\overline{x}$:
        $$
    ...
    * **Maximum Likelihood Estimator ($\hat{\theta}_n$):**
        Setting $U_n(\hat{\theta}) = 0 \implies \hat{\theta}_n = \overline{X}_n$.
...
2. Asymptotic Distributions
    ...
    1. **Consistency:**
        The MLE $\hat{\theta}_n = \overline{X}_n \xrightarrow{p} \theta_0$ by the Weak Law of Large Numbers.
    ...
    4. **Wilks' Theorem (Deviance):**
        The Deviance is defined as $D = 2[\ell(\hat{\theta}) - \ell(\theta_0)]$.Using the quadratic form of the log-likelihood derived above, we can directly compute the values at the MLE ($\hat{\theta} = \overline{x}$) and the null ($\theta_0$):
...
Under the conditions of the Lindeberg-Feller CLT, we can derive the asymptotic distribution for the sample mean $\overline{X}_n = \frac{1}{n}\sum_{i=1}^n X_i$.
...
3. **Applying Convergence Theorems for $\hat{\sigma}^2$**
   ...
   Consider the term $\sqrt{n}(\overline{X} - \mu)^2$. We can rewrite this as:
...
If the average information converges to a positive definite limit $\overline{\mathcal{I}}_n \to \mathcal{I}_{\infty}$, we have the following approximation for the scaled average score $\sqrt{n} \overline{\mathbf{U}}_n(\boldsymbol{\theta}_0; \mathbf{Y})$:
$$
...
### Alkeike Information Criterion for Estimating Out-of-sample Deviance


```{r}
#| label: fig-base-r-viz-expected-loglik-star
#| fig-cap: "Expected Log-Likelihood $\\ell^*(\\theta)$ and 10 realized $\\ell(\\theta)$ curves. The largest sample is highlighted in black, showing the 1/2 Deviance gap. The expected out-of-sample drop is marked in orange."
#| fig-pos: "H"
...
# 6. Feed the PNG to Quarto for all formats
...
#### 1. The Realized Deviance Expansions {.unnumbered}
...
#### 2. The Raw Optimism Gap {.unnumbered}
...
#### 3. The Expected AIC Penalty {.unnumbered}
...
#### 4. The Akaike Information Criterion (AIC){.unnumbered}
...
[See this page](./gradient-nn.html).
