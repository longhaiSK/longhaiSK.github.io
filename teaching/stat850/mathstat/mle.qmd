...
In the limit as $n \to \infty$ (or exactly for the Normal distribution), the log-likelihood behaves like a quadratic function. This leads to the following relationships between the Deviance ($D$), Score ($U$), Error ($\Delta \theta$), and Information ($\mathcal{I}$).

| Relationship | Formula | Intuition (Geometry of Parabola) |
| :--- | :--- | :--- |
| **Deviance vs. Error** | $D \approx \Delta \theta^T \mathcal{I} \Delta \theta$ | The vertical drop ($D$) is proportional to the squared horizontal distance ($\Delta \theta^2$), scaled by curvature ($\mathcal{I}$). This is the **Wald Statistic**. |
| **Score vs. Error** | $U \approx \mathcal{I} \Delta \theta$ | The slope ($U$) increases linearly with horizontal distance ($\Delta \theta$). The rate of increase is the curvature ($\mathcal{I}$). |
| **Deviance vs. Score** | $D \approx U^T \mathcal{I}^{-1} U$ | The vertical drop ($D$) is proportional to the squared slope ($U^2$), *divided* by curvature ($\mathcal{I}$). This is the **Score Statistic**. |
| **Information Definition** | $\mathcal{I} \approx -\nabla U$ | The curvature ($\mathcal{I}$) is the rate of change of the slope ($U$) (negative Hessian). |
| **Information Approximation** | $\mathcal{I} \approx U U^T$ | The curvature ($\mathcal{I}$) can be approximated by the "squared slope" (outer product of scores). This relies on Bartlett's 2nd Identity: $E[\mathcal{I}] = E[U U^T]$. |
