...
```{r}
#| label: fig-log-lik-viz
#| fig-cap: "Log-PDF of the sampling distribution (black) and 10 realized log-likelihoods (colored dashed lines). Points on the x-axis represent the realized MLEs."
#| message: false
...
# 1. Log-PDF of x_bar ~ N(theta_0, se^2)
...
# 3. Create Plot
df_log_pdf <- data.frame(x = x_vals, y = log_pdf_xbar)
colors <- scales::hue_pal()(10)

p <- ggplot() +
  geom_line(data = df_log_pdf, aes(x, y), size = 1.2, color = "black") +
  geom_vline(xintercept = theta_0, linetype = "dotted", color = "red", size = 1) +
  labs(x = expression(paste(bar(x), " or ", theta)), 
       y = "Log-Value",
       title = "Log-Sampling Distribution vs. 10 Realized Log-Likelihoods") +
  theme_minimal()

for (i in 1:10) {
  # Log-likelihood l(theta) centered at the observed x_bar
  log_lik_vals <- dnorm(x_bar_samples[i], mean = x_vals, sd = se, log = TRUE)
  df_lik <- data.frame(x = x_vals, y = log_lik_vals)
  
  p <- p + 
    geom_line(data = df_lik, aes(x, y), linetype = "dashed", alpha = 0.5, color = colors[i]) +
    # Mark the x_bar on the x-axis (bottom of plot)
    geom_point(aes(x = x_bar_samples[i], y = y_min), color = colors[i], size = 3) +
    # Vertical line segment connecting x-axis point to the peak of the likelihood
    geom_segment(aes(x = x_bar_samples[i], xend = x_bar_samples[i], 
                     y = y_min, yend = dnorm(0, sd = se, log = TRUE)), 
                 color = colors[i], linetype = "dotted", alpha = 0.3)
}
