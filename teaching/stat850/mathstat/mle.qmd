...
\begin{tikzpicture}[
    ...
    % Score -> Info
    % Connecting from the "Gradient Space" to "Information Space" horizontally
    \draw[dashed_arrow] (score) -- 
        node[midway, above=0.1cm] {Bartlett Identity: $\mathcal{I} = \text{Var}(\mathbf{U})$} 
        node[midway, below=0.1cm] {Empirical Fisher: $\hat{\mathcal{I}} = \sum_i \mathbf{U}_i \mathbf{U}_i^T$} 
    (info);
