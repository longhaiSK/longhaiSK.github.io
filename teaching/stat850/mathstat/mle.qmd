...
Let $f(\mathbf{x}|\boldsymbol{\theta})$ be the joint probability density function (or mass function) of the data $\mathbf{X} = (X_1, \dots, X_n)$. 
...
The MLE $\hat{\boldsymbol{\theta}}_{\text{MLE}}$ is the value in the parameter space $\Theta$ that maximizes the likelihood (and equivalently, the log-likelihood) function:
$$
...
The score function is defined as the gradient of the log-likelihood with respect to the parameter vector $\boldsymbol{\theta}$. Finding the MLE often involves solving the score equation $\mathbf{U}(\boldsymbol{\theta}; \mathbf{x}) = \mathbf{0}$.

1. **Total Score Function ($\mathbf{U}$):**
   $$
...
The Fisher Information measures the curvature of the log-likelihood surface and, correspondingly, the amount of information the data carries about the unknown parameter.
...
2. **Expected Fisher Information ($\mathcal{I}$):**
   ...
   * **Total:** $\mathcal{I}_n(\boldsymbol{\theta}) = E_{\boldsymbol{\theta}} \left[ \mathbf{J}_n(\boldsymbol{\theta}; \mathbf{X}) \right]$
...
### Example: MLE of Normal Sample

:::: {#exm-normal-mle-asymptotics}

#### Example: Normal Distribution (Exact Asymptotics)

Let $X_1, \dots, X_n \overset{iid}{\sim} N(\theta, \sigma^2)$ with $\sigma^2$ known. We explicitly derive the key quantities and map them to the four general asymptotic results under $H_0: \theta = \theta_0$.

1. Derivation of Key Quantities

    * **Log-Likelihood:**
        Using the identity $\sum (x_i - \theta)^2 = \sum (x_i - \bar{x})^2 + n(\bar{x} - \theta)^2$, the log-likelihood is a perfect quadratic centered at $\bar{x}$:
        $$
    ...
    * **Maximum Likelihood Estimator ($\hat{\theta}_n$):**
        Setting $U_n(\hat{\theta}) = 0 \implies \hat{\theta}_n = \bar{X}_n$.
...
2. Asymptotic Distributions
    ...
    1. **Consistency:**
        The MLE $\hat{\theta}_n = \bar{X}_n \xrightarrow{p} \theta_0$ by the Weak Law of Large Numbers.

    2. **Asymptotic Normality of the Score:**
        Evaluated at the null, the score follows a Normal distribution exactly:
        $$
        U_n(\theta_0) \sim N(0, \mathcal{I}_n(\theta_0))
        $$

    3. **Asymptotic Normality of the Estimator:**
        The distribution of the standardized MLE centered at the null value is:
        $$
        \sqrt{n}(\hat{\theta}_n - \theta_0) \sim N(0, \sigma^2) = N(0, \mathcal{I}_1(\theta_0)^{-1})
        $$

    4. **Wilks' Theorem (Deviance):**
        The Deviance is defined as $D = 2[\ell(\hat{\theta}) - \ell(\theta_0)]$.Using the quadratic form of the log-likelihood derived above, we can directly compute the values at the MLE ($\hat{\theta} = \bar{x}$) and the null ($\theta_0$):
...
The following plot illustrates the log-density of the sampling distribution of $\bar{x}$ (where the true $\theta_0 = 2$) alongside 10 realized log-likelihood functions. The colored points on the x-axis mark the MLE ($\bar{x}$) for each realization.

```{r}
#| label: fig-base-r-viz-score-legend
#| fig-cap: "Log-PDF of the sampling distribution and 10 realized log-likelihoods. The largest sample is highlighted in blue, showing the 1/2 Deviance gap. The MLEs are marked on the Score = 0 line."
#| fig-pos: "H"
# Parameters
theta_0 <- 2
sigma <- 10
n <- 100
se <- sigma / sqrt(n)  # Standard Error = 1

# Define range for x-axis
x_vals <- seq(theta_0 - 4*se, theta_0 + 4*se, length.out = 500)

# 1. Calculate Log-PDF of x_bar ~ N(theta_0, se^2)
log_pdf_xbar <- dnorm(x_vals, mean = theta_0, sd = se, log = TRUE)

# Determine plot limits for the left y-axis
y_min <- min(log_pdf_xbar)
# Expanded top limit to make room for the legend
y_max <- dnorm(0, sd = se, log = TRUE) + 2.5 

# 2. Draw 10 random x_bar samples
set.seed(123)
x_bar_samples <- rnorm(10, mean = theta_0, sd = se)

# Find the largest positive x_bar
...
# 3. Create Base R Plot
# Adjust margins to make room for the right-hand y-axis
par(mar = c(5, 4, 4, 4) + 0.3)

# Setup the empty plot with axes and labels for Log-Likelihoods
plot(x_vals, log_pdf_xbar, type = "n", 
     ylim = c(y_min, y_max),
     xlab = expression(theta), 
     ylab = "Log-Value",
     main = "Log-Likelihoods, Deviance (1/2 D), and Score Function",
     frame.plot = TRUE)

# Add the Log-PDF of the sampling distribution
lines(x_vals, log_pdf_xbar, lwd = 3, col = "black")

# Add the true theta line
abline(v = theta_0, col = null_color, lty = 1, lwd = 2)

# Loop to add all Likelihood curves
for (i in 1:10) {
  log_lik_vals <- dnorm(x_bar_samples[i], mean = x_vals, sd = se, log = TRUE)
  
  # Determine color and width based on whether it's the maximum
  if (i == idx_max) {
    current_col <- highlight_color
    l_width <- 2
  } else {
    current_col <- grey_color
    l_width <- 1
  }
...
# 4. Draw Deviance (1/2 D) segments for the largest x_bar
peak_y <- dnorm(x_bar_max, mean = x_bar_max, sd = se, log = TRUE)
null_y <- dnorm(x_bar_max, mean = theta_0, sd = se, log = TRUE)

# Horizontal line at l(theta_0) from theta_0 to x_bar
segments(x0 = theta_0, y0 = null_y, x1 = x_bar_max, y1 = null_y, 
         col = highlight_color, lwd = 2, lty = 1)

# Vertical line at x_bar from l(theta_0) up to peak l(x_bar)
segments(x0 = x_bar_max, y0 = null_y, x1 = x_bar_max, y1 = peak_y, 
         col = highlight_color, lwd = 3, lty = 1)

# Label 1/2 D next to the vertical segment (pos = 4 places it to the right)
text(x_bar_max, (null_y + peak_y)/2, labels = expression((1/2)*D), 
     pos = 4, col = highlight_color, font = 2, cex = 1)

# Mark intersection on the likelihood curve
points(theta_0, null_y, pch = 16, col = highlight_color, cex = 1.5)

# 5. Add Score Function using a 2nd y-axis
par(new = TRUE)

# Score U(theta) = (n/sigma^2)*(x_bar - theta). Here n/sigma^2 = 1.
score_vals <- x_bar_max - x_vals

# Setup the secondary plot window (empty)
# Expanded upper ylim from 5 to 9 to match the top space created in the first plot
plot(x_vals, score_vals, type = "n", 
     axes = FALSE, xlab = "", ylab = "", ylim = c(-5, 9))

# Add horizontal line for Score = 0
abline(h = 0, col = "gray", lty = 3, lwd = 2)

# Plot the score line
lines(x_vals, score_vals, col = score_color, lwd = 3, lty = 1)

# Add the MLE points (x_bar) precisely on the Score = 0 line
for (i in 1:10) {
  if (i == idx_max) {
    points(x_bar_samples[i], 0, col = highlight_color, pch = 16, cex = 1.5)
  } else {
    points(x_bar_samples[i], 0, col = grey_color, pch = 16, cex = 1.2)
  }
}

# Add right axis and label for the Score function
axis(4, col = score_color, col.axis = score_color, lwd = 2)
mtext(expression(paste("Score Function ", U(theta))), side = 4, line = 2.5, col = score_color, font = 2)

# 6. Add Legend
legend("topright", 
       legend = c("Log-PDF of Sampling Dist.", "Realized Log-Likelihoods", 
                  expression(paste("True Null ", theta[0])), "Score Function"),
       col = c("black", grey_color, null_color, score_color),
       lty = c(1, 2, 1, 1),
       lwd = c(3, 1, 2, 3),
       bg = "white", cex = 0.85)
```
...
## Newton-Raphson and Fisher Scoring Algorithms for Finding MLE
...
### Scalable First-Order and Adaptive Optimization Methods
...
#### Stochastic Gradient and Momentum Methods
...
#### Natural SGD with Empirical Fisher Information
...
1. At each step, it simulates "fake" target data $\mathbf{y}_{sim}$ from the current postulated model, $p(\cdot | \mathbf{X}, \boldsymbol{\theta}^{(t)})$. 
2. It calculates a simulated score vector $\mathbf{U}_{sim}$ based on this generated data.
3. It estimates the true Fisher Information using the cross-products of the simulated scores: $\hat{\mathcal{I}}_{\text{true}} \approx \mathbf{U}_{sim} \mathbf{U}_{sim}^\top$.
...
2. **Apply the Chain Rule** Note that $\frac{\partial (\mathbf{x}_i^\top \boldsymbol{\beta})}{\partial \beta_j} = x_{ij}$. Using the chain rule on the exponential term:
   $$
...
1. **Differentiate the Score** We take the derivative of the $j$-th component of the score, $U_j = \sum_{i=1}^n (y_i - \lambda_i(\boldsymbol{\beta})) x_{ij}$, with respect to $\beta_k$:
   $$
...
2. **Differentiate the Mean Function** Recall $\lambda_i(\boldsymbol{\beta}) = \exp(\mathbf{x}_i^\top \boldsymbol{\beta})$. Its derivative with respect to $\beta_k$ is:
   $$
...
### Derivation of the Expected Fisher Information $\mathcal{I}(\boldsymbol{\beta})$ via $\text{Var}(\mathbf{U})$
...
1. **Set Up the Variance Expression** Starting from the previously derived Score vector components:
   $$
   U_j(\boldsymbol{\beta}) = \sum_{i=1}^n (y_i - \lambda_i(\boldsymbol{\beta})) x_{ij}
   $$
   We find the $(j,k)$-th element of the Expected Information by taking the covariance between the $j$-th and $k$-th score components. Since the observations $y_i$ are independent:
   $$
   \mathcal{I}(\boldsymbol{\beta})_{jk} = \text{Cov}(U_j(\boldsymbol{\beta}), U_k(\boldsymbol{\beta})) = \sum_{i=1}^n \text{Cov}((y_i - \lambda_i(\boldsymbol{\beta})) x_{ij}, (y_i - \lambda_i(\boldsymbol{\beta})) x_{ik})
   $$

2. **Apply Variance Properties** Because the predicted mean $\lambda_i(\boldsymbol{\beta})$ and the covariates $x_{ij}, x_{ik}$ are treated as fixed constants, the covariance simplifies to the variance of $y_i$ scaled by the covariates:
   $$
   \mathcal{I}(\boldsymbol{\beta})_{jk} = \sum_{i=1}^n x_{ij} x_{ik} \text{Var}(y_i)
   $$

3. **Substitute the Poisson Variance** For a Poisson distribution, the variance is equal to the mean: $\text{Var}(y_i) = \lambda_i(\boldsymbol{\beta})$. Substituting this known variance into our equation gives:
   $$
...
4. **Construct the Matrix Form** The full matrix can therefore be written using the exact same weight matrix $\mathbf{W}(\boldsymbol{\beta})$:
   $$
