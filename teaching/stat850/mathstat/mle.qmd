...
### Generalized Wilks' Theorem (Composite Null)

::: {#thm-wilks-general}
#### Wilks' Theorem for Nested Models
Consider two nested parameter spaces $\Theta_0 \subset \Theta_1$, where:
* $H_1$ (Full Model): $\boldsymbol{\theta} \in \Theta_1$ with dimension $p$.
* $H_0$ (Reduced Model): $\boldsymbol{\theta} \in \Theta_0$ with dimension $q < p$.

Let $\hat{\boldsymbol{\theta}}_1$ be the MLE under the full model, and $\hat{\boldsymbol{\theta}}_0$ be the MLE constrained to the null hypothesis space.

The Deviance statistic converges to a Chi-squared distribution with degrees of freedom equal to the difference in dimensions:
$$
D_n = 2 [\ell_n(\hat{\boldsymbol{\theta}}_1) - \ell_n(\hat{\boldsymbol{\theta}}_0)] \xrightarrow{d} \chi^2_{p-q}
$$
:::

::: {.proof}
1.  **Decomposition of the Deviance**
    We can rewrite the test statistic by adding and subtracting $2\ell_n(\boldsymbol{\theta}^*)$ (where $\boldsymbol{\theta}^*$ is the true parameter):
    $$
    2 [\ell_n(\hat{\boldsymbol{\theta}}_1) - \ell_n(\hat{\boldsymbol{\theta}}_0)] = \underbrace{2 [\ell_n(\hat{\boldsymbol{\theta}}_1) - \ell_n(\boldsymbol{\theta}^*)]}_{\text{Fit of Full Model}} - \underbrace{2 [\ell_n(\hat{\boldsymbol{\theta}}_0) - \ell_n(\boldsymbol{\theta}^*)]}_{\text{Fit of Reduced Model}}
    $$

2.  **Quadratic Approximation (Simple Wilks)**
    From the simple Wilks' theorem proof, we know that near the true parameter, the likelihood difference behaves like a squared distance (quadratic form).
    Let $\mathbf{Z} \sim N(\mathbf{0}, \mathbf{I}_p)$ be the standardized score vector.

    * **Term 1 (Full Model):** The MLE $\hat{\boldsymbol{\theta}}_1$ can move in any of the $p$ dimensions to maximize likelihood.
        $$
        2 [\ell_n(\hat{\boldsymbol{\theta}}_1) - \ell_n(\boldsymbol{\theta}^*)] \approx \mathbf{Z}^T \mathbf{Z} \sim \chi^2_p
        $$
    * **Term 2 (Reduced Model):** The restricted MLE $\hat{\boldsymbol{\theta}}_0$ can only move in the $q$ dimensions of $\Theta_0$. It essentially projects the score vector $\mathbf{Z}$ onto a $q$-dimensional subspace.
        $$
        2 [\ell_n(\hat{\boldsymbol{\theta}}_0) - \ell_n(\boldsymbol{\theta}^*)] \approx \mathbf{Z}_0^T \mathbf{Z}_0 \sim \chi^2_q
        $$

3.  **Geometric Interpretation (Pythagorean Theorem)**
    Asymptotically, Maximum Likelihood Estimation is equivalent to **Least Squares Projection**.
    * $\hat{\boldsymbol{\theta}}_1$ projects the data onto the full $p$-dimensional space.
    * $\hat{\boldsymbol{\theta}}_0$ projects the data onto the restricted $q$-dimensional subspace.

    The "total length" (Full Model Fit) is the sum of the "restricted length" (Reduced Model Fit) plus the "orthogonal length" (the improvement):
    $$
    \|\mathbf{Y} - \boldsymbol{\theta}^*\|^2 \approx \|\mathbf{Y} - \hat{\boldsymbol{\theta}}_0\|^2 + \|\hat{\boldsymbol{\theta}}_1 - \hat{\boldsymbol{\theta}}_0\|^2
    $$
    Therefore, the difference (our test statistic) is the squared length of the component **orthogonal** to $\Theta_0$ but inside $\Theta_1$.

4.  **Degrees of Freedom**
    Since $\Theta_0$ is a $q$-dimensional subspace of the $p$-dimensional space $\Theta_1$, the orthogonal complement has dimension $p - q$.
    Thus, the difference of the quadratic forms follows a Chi-squared distribution with degrees of freedom equal to that difference:
    $$
    D_n \approx \chi^2_p - \chi^2_q \sim \chi^2_{p-q}
    $$
:::
