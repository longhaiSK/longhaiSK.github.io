...
2. **High Dimensionality** \n
   The parameter vector $\boldsymbol{\theta}$ can contain millions or billions of weights. This makes the storage and inversion of the Observed Information Matrix $\mathbf{J}(\boldsymbol{\theta})$ (the $p \times p$ Hessian) computationally intractable.
...
### Gradient-Based Methods and the Role of Fisher Information

Because computing the Hessian $\mathbf{H} = -\mathbf{J}$ is impossible in large networks, deep learning relies on **First-Order Methods** (like SGD, Adam, or RMSProp). These methods primarily use the score vector $\mathbf{U}(\boldsymbol{\theta})$ to update parameters:

$$
\boldsymbol{\theta}^{(t+1)} = \boldsymbol{\theta}^{(t)} + \alpha \mathbf{U}(\boldsymbol{\theta}^{(t)})
$$

However, the second-order information remains theoretically crucial. Modern optimizers leverage the **Second Moment Identity** (Bartlett's Identity) to approximate the curvature without ever computing the second derivatives:

::: {#thm-fisher-approximation}
### Fisher Information Approximation

Recall from @thm-score-identities that at the true parameter (or a good estimate), the Fisher Information is both the variance of the score and the expected Hessian:

$$
\mathbf{I}(\boldsymbol{\theta}) = \text{Cov}(\mathbf{U}) = E[\mathbf{J}]
$$

In deep learning, we approximate $\mathbf{I}(\boldsymbol{\theta})$ using the "outer product of gradients" (empirical Fisher):

$$
\hat{\mathbf{I}} = \frac{1}{n} \sum_{i=1}^n \nabla \ell(\boldsymbol{\theta}; Y_i) \nabla \ell(\boldsymbol{\theta}; Y_i)^\top
$$
:::



### Avoiding the Hessian via the Score Covariance

Adaptive optimizers like **RMSProp** and **Adam** use this identity to perform a coordinate-wise scaling of the learning rate. Instead of computing the full Hessian $\mathbf{H}$, they maintain a running average of the squared scores ($U_j^2$):

1. **Scaling by Variance:** Since $\text{Var}(U_j) = I_{jj}$, the squared gradients provide an estimate of the local curvature along each dimension.
2. **Preconditioning:** By dividing the update by $\sqrt{\text{Var}(U_j)}$, these algorithms effectively "flatten" the likelihood surface, performing a simplified version of the Newton-Raphson update:
   
   $$
   \theta_j^{(t+1)} \approx \theta_j^{(t)} + \frac{\alpha}{\sqrt{\hat{I}_{jj}}} U_j(\boldsymbol{\theta}^{(t)})
   $$

This approach allows deep learning models to benefit from the second-order geometry of the log-likelihood while only requiring first-order gradients, bypassing the $O(p^2)$ cost of the Hessian.

::: {#rem-natural-gradient}
### Natural Gradient Descent
The most direct application of this theory is **Natural Gradient Descent**, which updates parameters using $\Delta \boldsymbol{\theta} = \alpha \mathbf{I}(\boldsymbol{\theta})^{-1} \mathbf{U}(\boldsymbol{\theta})$. This update is invariant to the parameterization of the model, as it moves the parameters a fixed distance in the space of *probability distributions* (measured by KL divergence) rather than the space of *weights*.
:::
