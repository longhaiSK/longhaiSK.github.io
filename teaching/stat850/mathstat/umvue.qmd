...
We test two statistics against the **MSS Condition Theorem**:
$$
\text{Parallel Log-Likelihoods} \implies \text{Statistic}(x) = \text{Statistic}(y)
$$

**Step 1: Establish the Parallel Condition**
For Bernoulli data, the log-likelihood is:
$$
\ell(p; x) = (\sum x_i) \ln p + (n - \sum x_i) \ln(1-p)
$$
The difference $\ell(p; x) - \ell(p; y)$ is constant (parallel) if and only if the coefficients of $\ln p$ are identical:
$$
\sum x_i = \sum y_i
$$

**Test 1: The Statistic $T(X) = \sum_{i=1}^3 X_i$**
We check if the implication holds:
$$
\sum x_i = \sum y_i \implies T(x) = T(y)
$$
Since $T(x) = \sum x_i$, this becomes:
$$
\sum x_i = \sum y_i \implies \sum x_i = \sum y_i
$$
**Result:** The implication is trivially true.
**Conclusion:** $T(X)$ satisfies the necessary condition for being a Minimal Sufficient Statistic.

**Test 2: The Statistic $S(X) = (X_1, \sum_{i=2}^3 X_i)$**
We check if the implication holds:
$$
\sum x_i = \sum y_i \implies S(x) = S(y)
$$
We use a counter-example to show this fails.
Consider two data sets:

* $x = (1, 0, 1)$
* $y = (0, 1, 1)$

**Check Assumption (Parallel Log-Likelihoods):**
$$
\sum x_i = 1+0+1 = 2
$$
$$
...
**Check Conclusion (Statistic Equality):**
$$
S(x) = (1, 0+1) = (1, 1)
$$
$$
...
**Conclusion:**
We found a pair $x, y$ where the log-likelihoods are parallel, but the statistic values differ.
Therefore, $S(X)$ does not compress the data as much as possible and is **not** a Minimal Sufficient Statistic.
