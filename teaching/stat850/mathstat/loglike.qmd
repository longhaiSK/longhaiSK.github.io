...
## Methods of Moment Matching for Estimating Parameters in Exponential Families

::: {#def-moment-matching-canonical}
### Maximum Likelihood via Moment Matching
Let $\mathbf{X}_1, \dots, \mathbf{X}_n$ be an independent sample from an exponential family in canonical form. The log-likelihood function is proportional to:

$$
\ell(\boldsymbol{\eta}) = \sum_{i=1}^n \boldsymbol{\eta}^\top \mathbf{T}(\mathbf{X}_i) - n A(\boldsymbol{\eta})
$$

Taking the gradient with respect to the parameter vector $\boldsymbol{\eta}$ and setting it to zero yields the score equation:

$$
\nabla \ell(\boldsymbol{\eta}) = \sum_{i=1}^n \mathbf{T}(\mathbf{X}_i) - n \nabla A(\boldsymbol{\eta}) = \mathbf{0}
$$

Substituting the first moment identity $E[\mathbf{T}(\mathbf{X})] = \nabla A(\boldsymbol{\eta})$ from the Cumulant Generating Theorem, we obtain the fundamental moment matching condition:

$$
\frac{1}{n} \sum_{i=1}^n \mathbf{T}(\mathbf{X}_i) = E[\mathbf{T}(\mathbf{X})]
$$

Thus, the method of moment matching in a canonical exponential family simplifies to equating the empirical sample average of each sufficient statistic to its theoretical expected value:

$$
\bar{T}_j = \frac{\partial A(\boldsymbol{\eta})}{\partial \eta_j} \quad \text{for } j = 1, \dots, k
$$
:::
