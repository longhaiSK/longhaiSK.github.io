...
$$ 
Note that $\mathbf{T}(\mathbf{x})$ is constant with respect to $\boldsymbol{\eta}$, so its derivative vanishes. By the Second Moment Identity, $\mathbf{I}(\boldsymbol{\eta}) = E[\mathbf{J}(\boldsymbol{\eta})] = \text{Cov}(\mathbf{U}(\boldsymbol{\eta}))$. Since $\mathbf{U}(\boldsymbol{\eta}) = \mathbf{T}(\mathbf{X}) - \text{constant}$, $\text{Cov}(\mathbf{U}(\boldsymbol{\eta})) = \text{Cov}(\mathbf{T}(\mathbf{X}))$. Therefore: 
$$
