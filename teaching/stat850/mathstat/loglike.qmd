...
---
title: "Likelihood and Efficiency"
format: html
---

### R Illustration

We visualize the Log-Likelihoods (solid lines) and Score functions (dashed lines) for two different sample sizes ($n=10$ and $n=20$) using a true parameter $\theta^* = 5$.

* **Global Scaling:** The y-axis ranges are fixed across both plots to show how $n=20$ results in steeper curves (more information).
* **MLE ($\hat{\theta}$):** The triangles on the x-axis mark the Maximum Likelihood Estimate (where $U(\theta)=0$).
* **Score at True Parameter ($U(\theta^*)$):** The solid circles on the vertical dashed line mark the value of the Score function at the true parameter. Notice how these points fluctuate around the zero line (Bartlett's First Identity).
...
theta_true <- 5  
n_sizes <- c(10, 50)
theta_grid <- seq(3, 8, length.out = 200) 

# Define functions
log_lik_fn <- function(theta, data) {
  n <- length(data)
  -n * log(theta) - sum(data) / theta
}

score_fn <- function(theta, data) {
  n <- length(data)
  -n / theta + sum(data) / theta^2
}

# 1. Generate All Data First
data_list <- list()
for(n in n_sizes) {
  # Generate 3 datasets per sample size
  data_list[[paste0("n", n)]] <- replicate(3, rexp(n, rate = 1/theta_true), simplify = FALSE)
}
...
for (n in n_sizes) {
  ...
  if (n == n_sizes[1]) { 
     legend("topright", 
            legend = c("Log-Lik (Solid)", "Score (Dashed)", "Score at True Theta", "MLE (x-axis)"),
            lty = c(1, 2, NA, NA), 
            pch = c(NA, NA, 19, 17),
...
Let $T(X)$ be any estimator with finite variance, and let $U(\theta) = \frac{\partial}{\partial \theta} \log f(X|\theta)$ be the Score function. Under standard regularity conditions, the covariance between the estimator and the score is equal to the derivative of the estimator's expectation:
$$
...
\end{aligned}
$$ 
Recall that the expected score is zero ($E_\theta[U] = 0$). Thus, the second term vanishes: 
$$
\begin{aligned}
...
\end{aligned}
$$ 
Using the logarithmic derivative identity $\frac{\partial \log f}{\partial \theta} f = \frac{1}{f} \frac{\partial f}{\partial \theta} f = \frac{\partial f}{\partial \theta}$:
$$
\begin{aligned}
...
$$
where $\mathcal{I}(\theta) = E_\theta \left[ \left( \frac{\partial}{\partial \theta} \log f(X|\theta) \right)^2 \right]$ is the scalar Fisher Information.
...
$$
Substituting this result and $\text{Var}_\theta(U) = \mathcal{I}(\theta)$ back into the covariance inequality:
$$
