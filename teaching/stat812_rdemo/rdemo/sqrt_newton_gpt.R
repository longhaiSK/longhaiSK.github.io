...
ui <- fluidPage(
    ...
    h3("Square Root Calculator"),
    # Titles side-by-side at the top
    fluidRow(
        column(6, h3("Input of S and Initial Values")),
        column(6, h3("Results of sqrt(S)"))
    ),
    ...
    fluidRow(
        column(6,
               ...
               selectInput("tol", "Select tolerance (tol) between steps:",
    ...
    fluidRow(
        column(12, plotOutput("convergence_plot"))
    ),
    ...
    fluidRow(
        column(2,offset =4, div(style = "margin-bottom: 10px;", numericInput("example_S", "S:", value = 25, min = 1))),
        column(2, div(style = "margin-bottom: 10px;", numericInput("example_x0", "x₀:", value = 2, min = 1)))
    ),
    
    fluidRow(
        column(6,  # Left: Iterations Section
               h4("Values of Iterations:"),
               uiOutput("iterations")  # Auto-generated steps
        ),
        
        column(6,  # Right: Animation with Controls
               h4("Animation of Iterations:"),
               fluidRow(
                   column(2, actionButton("prev_step", "Prev Step")),  # Move backward
                   column(2, actionButton("next_step", "Next Step"))  # Move forward
               ),
    ...
    div(style = "margin-top: 30px; text-align: center; font-size: 14px;",
        HTML("<hr> Author: <strong>Longhai Li</strong><br>
              ...
              <a href='https://longhaisk.github.io' target='_blank'>longhaisk.github.io</a>")
    )
)

# Shiny Server
server <- function(input, output) {
    input_data <- reactiveValues(data = data.frame(S = c(2, 5, 7, 8, 9, 11), x0 = c(1, 2.5, 3.5, 4, 4.5, 5.5)))
    
    output$input_table <- renderRHandsontable({ rhandsontable(input_data$data) })
    ...
    output$convergence_plot <- renderPlot({
        ...
        observeEvent(input$example_S, { current_step(1) }, ignoreInit = TRUE)
        observeEvent(input$example_x0, { current_step(1) }, ignoreInit = TRUE)
        ...
        output$iterations <- renderUI({
            ...
            merged_text <- paste0(
                "<p style='line-height: 2.5;", if (step == 1) " color: red; font-weight: bold;'" else "'>",
            ...
            merged_text <- paste0(
                ...
                paste(
                    sapply(1:(length(trace) - 1), function(i) {
                        paste0(
                            "<p style='line-height: 2.5;", if (i == step - 1) " color: red; font-weight: bold;'" else "'>",
                            "<strong>Iteration ", i, ":</strong> \\( x_", i, 
                            " = \\frac{1}{2} \\left( ", round(trace[i], 5), 
                            " + \\frac{", input$example_S, "}{", round(trace[i], 5), "} \\right) \\approx ", 
                            round(trace[i+1], 5), " \\)</p>"
                        )
                    }), collapse = ""
                )
        ...
        output$newton_animation <- renderPlot({
            ...
            ggplot() +
                ...
                geom_line(data = df_tangent, aes(x = x, y = y), 
                          color = "deeppink", size = 1.2, linetype = "solid") +
                geom_text(aes(x = x_i, y = -1), label = label_expr, 
                          size = 6, vjust = 1.5) +
                labs(title = TeX(paste0("Newton's Method - Iteration ", iter_label)),
                     x = "x", 
                     y = TeX("$f(x) = x^2 - S$")) +
