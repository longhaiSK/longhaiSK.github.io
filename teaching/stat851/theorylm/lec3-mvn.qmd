...
# 1. PARAMETERS (Updated Sigma)
mu <- c(1, 2) 
sigma <- matrix(c(2, 2, 2, 4), nrow=2) # Covariance = 2
...
# Rescale factor (3x larger than previous 1.5)
scale_factor <- 4.5

# A. Conditional Distribution at x=3
# y|x=3 ~ N(0.5*3, 1) = N(1.5, 1)
mu_cond <- 0.5 * 3
sd_cond <- sqrt(1)
dens_cond <- dnorm(dens_y, mean=mu_cond, sd=sd_cond)
df_cond <- data.frame(y=dens_y, x = 3 + dens_cond * scale_factor)

# B. Distribution of Conditional Mean (Explained) at x=6
# E[y|x] ~ N(1, 1)
dens_expl <- dnorm(dens_y, mean=1, sd=sqrt(1))
df_expl <- data.frame(y=dens_y, x = 6 + dens_expl * scale_factor)

# C. Marginal Distribution of y (Total) at x=9
# y ~ N(1, 2)
dens_marg <- dnorm(dens_y, mean=1, sd=sqrt(2))
df_marg <- data.frame(y=dens_y, x = 9 + dens_marg * scale_factor)
...
#| fig-cap: "Variance Decomposition in Multivariate Normal"
#| echo: true
#| warning: false
#| message: false
#| fig-width: 7
...
#| fig-cap: "Interactive 3D Plot: Observed Data vs Fitted Plane"
#| echo: true
#| warning: false
