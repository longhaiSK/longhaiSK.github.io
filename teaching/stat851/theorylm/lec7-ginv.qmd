...
In this example, we project a vector $\mathbf{y} \in \mathbb{R}^4$ onto a design matrix $X = [\mathbf{1}, x_1, x_2]$. We examine the specific case where the predictors sum to the intercept, such that $x_1 + x_2 = \mathbf{1}$. Because of this dependency, the matrix $X$ is rank-deficient.
...
4. Demonstrate that the projection of $\mathbf{y}$ onto $L(\mathbf{1}, x_1, x_2)$ is exactly the same as projecting onto the reduced space $L(\mathbf{1}, x_1)$.

### 1. Define the Vectors and Design Matrix {.unnumbered}
Let's define our vectors in $\mathbb{R}^4$. We use the specific vectors provided, which perfectly sum to the intercept vector $\mathbf{1}$.

$$x_1 = \begin{bmatrix} 1 \\ 0 \\ 2 \\ 0 \end{bmatrix}, \quad x_2 = \begin{bmatrix} 0 \\ 1 \\ -1 \\ 1 \end{bmatrix}, \quad \mathbf{y} = \begin{bmatrix} 4 \\ 2 \\ 1 \\ 3 \end{bmatrix}$$

Notice that $x_1 + x_2 = \begin{bmatrix} 1 & 1 & 1 & 1 \end{bmatrix}^T = \mathbf{1}$. 

The full design matrix $X = [\mathbf{1}, x_1, x_2]$ is:
$$X = \begin{bmatrix} 1 & 1 & 0 \\ 1 & 0 & 1 \\ 1 & 2 & -1 \\ 1 & 0 & 1 \end{bmatrix}$$

### 2. Compute $X^T X$ {.unnumbered}
To find the generalized inverse, we first compute $X^T X$:
$$X^T X = \begin{bmatrix} 1 & 1 & 1 & 1 \\ 1 & 0 & 2 & 0 \\ 0 & 1 & -1 & 1 \end{bmatrix} \begin{bmatrix} 1 & 1 & 0 \\ 1 & 0 & 1 \\ 1 & 2 & -1 \\ 1 & 0 & 1 \end{bmatrix} = \begin{bmatrix} 4 & 3 & 1 \\ 3 & 5 & -2 \\ 1 & -2 & 3 \end{bmatrix}$$

Notice that the first row is the sum of the second and third rows. The matrix has dimensions $3 \times 3$ but its rank is only 2. The determinant is $0$, meaning it is singular and we must use a generalized inverse.

### 3. Compute Two Generalized Inverses {.unnumbered}
A generalized inverse $G$ satisfies $X^T X G X^T X = X^T X$. Because the rank is 2, we can easily find G-inverses by taking the regular inverse of a full-rank $2 \times 2$ submatrix and padding the rest with zeros.

**Choice 1 ($G_1$):** Invert the top-left $2 \times 2$ matrix corresponding to $[\mathbf{1}, x_1]$.
The submatrix is $\begin{bmatrix} 4 & 3 \\ 3 & 5 \end{bmatrix}$. Its determinant is $(4)(5) - (3)(3) = 11$.
$$G_1 = \frac{1}{11} \begin{bmatrix} 5 & -3 & 0 \\ -3 & 4 & 0 \\ 0 & 0 & 0 \end{bmatrix}$$

**Choice 2 ($G_2$):** Invert the bottom-right $2 \times 2$ matrix corresponding to $[x_1, x_2]$.
The submatrix is $\begin{bmatrix} 5 & -2 \\ -2 & 3 \end{bmatrix}$. Its determinant is $(5)(3) - (-2)(-2) = 11$.
$$G_2 = \frac{1}{11} \begin{bmatrix} 0 & 0 & 0 \\ 0 & 3 & 2 \\ 0 & 2 & 5 \end{bmatrix}$$

### 4. The Projection Matrix $P$ is Invariant {.unnumbered}
The projection matrix is calculated as $P = X G X^T$. 

Let's compute $P$ using $G_1$:
$$X G_1 = \frac{1}{11} \begin{bmatrix} 1 & 1 & 0 \\ 1 & 0 & 1 \\ 1 & 2 & -1 \\ 1 & 0 & 1 \end{bmatrix} \begin{bmatrix} 5 & -3 & 0 \\ -3 & 4 & 0 \\ 0 & 0 & 0 \end{bmatrix} = \frac{1}{11} \begin{bmatrix} 2 & 1 & 0 \\ 5 & -3 & 0 \\ -1 & 5 & 0 \\ 5 & -3 & 0 \end{bmatrix}$$

$$P_1 = X G_1 X^T = \frac{1}{11} \begin{bmatrix} 2 & 1 & 0 \\ 5 & -3 & 0 \\ -1 & 5 & 0 \\ 5 & -3 & 0 \end{bmatrix} \begin{bmatrix} 1 & 1 & 1 & 1 \\ 1 & 0 & 2 & 0 \\ 0 & 1 & -1 & 1 \end{bmatrix} = \frac{1}{11} \begin{bmatrix} 3 & 2 & 4 & 2 \\ 2 & 5 & -1 & 5 \\ 4 & -1 & 9 & -1 \\ 2 & 5 & -1 & 5 \end{bmatrix}$$

Using $G_2$ to compute $P_2 = X G_2 X^T$ will yield the exact same $4 \times 4$ projection matrix. Thus, $P_1 = P_2 = P$. 

The projection of $\mathbf{y}$ onto the full model $L(\mathbf{1}, x_1, x_2)$ is:
$$\hat{\mathbf{y}} = P\mathbf{y} = \frac{1}{11} \begin{bmatrix} 3 & 2 & 4 & 2 \\ 2 & 5 & -1 & 5 \\ 4 & -1 & 9 & -1 \\ 2 & 5 & -1 & 5 \end{bmatrix} \begin{bmatrix} 4 \\ 2 \\ 1 \\ 3 \end{bmatrix} = \frac{1}{11} \begin{bmatrix} 26 \\ 32 \\ 20 \\ 32 \end{bmatrix} \approx \begin{bmatrix} 2.36 \\ 2.91 \\ 1.82 \\ 2.91 \end{bmatrix}$$

### 5. Comparison to Projecting onto $\mathbf{1}$ and $x_1$ Individually {.unnumbered}
If we drop $x_2$ and project only onto the reduced space $X_{red} = [\mathbf{1}, x_1]$, the matrix $X_{red}^T X_{red}$ is full rank (invertible).
$$X_{red}^T X_{red} = \begin{bmatrix} 4 & 3 \\ 3 & 5 \end{bmatrix}$$

The projection matrix for this reduced model is $P_{red} = X_{red}(X_{red}^T X_{red})^{-1}X_{red}^T$. 
Notice that $(X_{red}^T X_{red})^{-1}$ is exactly the $2 \times 2$ block we used to build $G_1$. Therefore, padding it with zeros and multiplying by the full $X$ is mathematically identical to just multiplying the reduced matrices:
$$P_{red} = P_1 = P$$

We have shown that $P_{red}\mathbf{y} = P\mathbf{y} = \hat{\mathbf{y}}$. Because $x_2$ exists in the space already spanned by $\mathbf{1}$ and $x_1$, incorporating $x_2$ into the model adds no new geometric dimensions, and the projection is identical.
...
# G1: Inverse of the top-left 2x2 block
G1 <- matrix(0, nrow = 3, ncol = 3)
G1[1:2, 1:2] <- solve(XTX[1:2, 1:2])

# G2: Inverse of the bottom-right 2x2 block
G2 <- matrix(0, nrow = 3, ncol = 3)
G2[2:3, 2:3] <- solve(XTX[2:3, 2:3])
...
# Compute Projections onto L(1, x1, x2)
y_hat_G1 <- P1 %*% y
y_hat_G2 <- P2 %*% y

# Compute Projection onto reduced model L(1, x1) only
X_red <- cbind(int, x1)
P_red <- X_red %*% solve(t(X_red) %*% X_red) %*% t(X_red)
y_hat_red <- P_red %*% y
...
results <- data.frame(
  ...
  Proj_red = round(as.vector(y_hat_red), 4)
)
