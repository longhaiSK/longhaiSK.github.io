...
## Estimation and F-test in Non-full-rank Models
...
1.  That is, the effects model has the same model matrix as the cell-means model, but with one extra column, a column of ones, in the first position.
2.  Notice that $\sum_i x_i = j_N$. Therefore, the columns of the model matrix for the effects model are linearly dependent.
...
### Interpretation of Parameters and Constraints

Subject to the constraint $\sum_i \alpha_i = 0$, the parameters of the effects model have the following interpretations:

1.  $\mu$: grand mean response across all treatments.
2.  $\alpha_i$: deviation from the grand mean placing $\mu_i$ (the $i^{th}$ treatment mean) up or down from the grand mean; i.e., the effect of the $i^{th}$ treatment.
...
### Approaches to Non-Full Rank Models
...
2.  **Add constraints to the model parameters**
...
3.  **Analyze the model as a non-full rank model**
...
::: {#thm-consistency-normal-equations}

### Consistency of Normal Equations
For $X$ an $n \times p$ matrix of rank $k < p \le n$, the normal equations $X^T X \hat{\beta} = X^T y$ form a consistent system of equations because $\text{Col}(X^T X) = \text{Col}(X^T)$.
...
In the model $y \sim N(X\beta, \sigma^2 I)$:
...
### Hypothesis Testing and Nested Models
...
%| fig-cap: "Geometric Interpretation of the Nested Models Test"
%| echo: false
%| fig-align: "center"
...
\begin{tikzpicture}[font=\sffamily, scale=1.2]
  ...
  \definecolor{v0color}{RGB}{255, 255, 255}  % White for V0 hole
  ...
  \node[linecolor, align=center] at (0, 0) {\large $H_0:$\\[0.2em] \Large $\mu \in V_0$};
  ...
  \node[linecolor] (v0_label) at (0, -2.0) {\large $V_0 = \text{Col}(X_1)$};
