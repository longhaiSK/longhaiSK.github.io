...
:::{#exm-mean-ss-decomposition name="Expectation of Sum of Squares Decomposition"}

Consider a random sample $x = (x_1, \dots, x_n)'$ with mean vector $\mu_x = \mu j_n$ and covariance $\Sigma = \sigma^2 I_n$. We analyze the two components of the total sum of squares by projecting $x$ onto the mean space ($H$) and the residual space ($I-H$).

**1. The Projection Vectors**

First, we write the explicit forms of the projected vectors:

* **Mean Vector ($Hx$):** Projecting $x$ onto the column space of $j_n$ replaces every element with the sample mean $\bar{x}$.
    $$
    Hx = \begin{pmatrix} \bar{x} \\ \vdots \\ \bar{x} \end{pmatrix} = \bar{x} j_n
    $$

* **Residual Vector ($(I-H)x$):** Subtracting the mean projection from $x$ yields the deviations.
    $$
    (I-H)x = \begin{pmatrix} x_1 - \bar{x} \\ \vdots \\ x_n - \bar{x} \end{pmatrix}
    $$
...
$$
x'Hx = ||Hx||^2 = \sum_{i=1}^n \bar{x}^2 = n\bar{x}^2
$$
Applying the corollary with $P=H$:

* **Rank:** $\text{tr}(H) = 1$.
* **Mean:** $H\mu_x = H(\mu j_n) = \mu j_n$. The squared norm is $n\mu^2$.

$$
E[||Hx||^2] = \sigma^2(1) + n\mu^2
$$

**Part B: Sum of Squared Errors (SSE)**
The quadratic form is the squared norm of the residual vector:
$$
x'(I-H)x = ||(I-H)x||^2 = \sum_{i=1}^n (x_i - \bar{x})^2
$$
Applying the corollary with $P=I-H$:

* **Rank:** $\text{tr}(I-H) = n - 1$.
* **Mean:** $(I-H)\mu_x = \mu_x - H\mu_x = \mu j_n - \mu j_n = 0$. The squared norm is $0$.

$$
E[||(I-H)x||^2] = \sigma^2(n-1) + 0
$$

**Conclusion**
These results confirm the standard properties of the sample mean and variance:
$$
E(\bar{x}^2) = \frac{\sigma^2}{n} + \mu^2 \quad \text{and} \quad E(S^2) = E\left[ \frac{SSE}{n-1} \right] = \sigma^2
$$
:::
