...
* **Non-central Component ($X_1$):** $X_1 \sim \chi^2(n_1, 2\lambda)$.
  Here, we use the "natural" definition where $\lambda$ is **half the sum of squared means** (or signal energy). This simplifies density formulas into direct Poisson mixtures.
  *(Note: R defines the non-centrality parameter as the full sum of squares, i.e., $2\lambda$.)*

* **Central Component ($X_2$):** $X_2 \sim \chi^2(n_2)$.
  Represents the **Noise Sum of Squares** (SSE), arising from pure error.

We visualize these components as partitioning the Total Sum of Squares (SST).

```{tikz}
%| label: fig-variance-partition
%| fig-cap: "Partitioning of Total Variation ($SST = X_1 + X_2$)."
%| echo: false
%| fig-align: "center"
...
\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=2pt}, thick, red!70!black] 
    (0, \yBrace) -- (\lenNoise, \yBrace) node[midway, below=8pt] {$\sim \chi^2(n_2)$};
\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=2pt}, thick, blue!70!black] 
    (\lenNoise, \yBrace) -- (\lenNoise + \lenSignal, \yBrace) node[midway, below=8pt] {$\sim \chi^2(n_1, 2\lambda)$};
\end{tikzpicture}
```

### The Non-central F-distribution $f(n_1, N_2, \lambda)$

::: {#def-noncentral-f}
### Non-central F
Let $X_1 \sim \chi^2(n_1, 2\lambda)$ and $X_2 \sim \chi^2(n_2)$ be independent. The random variable $V$ follows a **non-central F-distribution**, defined as:
$$V = \frac{X_1/n_1}{X_2/n_2} \sim F(n_1, n_2, \lambda)$$
:::

Physically, this compares the *density* of the signal to the *density* of the noise.

* **PDF:** A Poisson mixture of central F densities with weights $P(\text{Poisson}=\lambda)$.
  $$f_V(v) = \sum_{j=0}^{\infty} \frac{e^{-\lambda}\lambda^j}{j!} f_{F}\left(v; n_1+2j, n_2\right)$$

* **Exact Mean:** $E[V] = \frac{n_2(n_1 + 2\lambda)}{n_1(n_2 - 2)}$
* **Approximate Mean:** $E[V] \approx 1 + \frac{2\lambda}{n_1}$
...
### Type I Non-central Beta $\text{beta}_i(n_1/2, N_2/2, \lambda)$

::: {#def-noncentral-beta1}
### Type I Non-central Beta
The random variable $B_I$ follows a **Type I non-central Beta distribution**, defined as the signal's proportion of the total sum:
$$B_I = \frac{X_1}{X_1 + X_2} \sim \text{Beta}\left(\frac{n_1}{2}, \frac{n_2}{2}, \lambda\right)$$
:::

This represents the **Coefficient of Determination ($R^2$)**.

$$f_{B_I}(x) = \sum_{j=0}^{\infty} \frac{e^{-\lambda}\lambda^j}{j!} f_{\text{Beta}}\left(x; \frac{n_1}{2}+j, \frac{n_2}{2}\right)$$

* **Exact Mean:** $E[B_I] = e^{-\lambda} \frac{n_1}{n_1+n_2} \,_1F_1\left(\frac{n_1}{2}+1; \frac{n_1+n_2}{2}+1; \lambda\right)$
* **Approximate Mean:** $E[B_I] \approx \frac{n_1 + 2\lambda}{n_1 + n_2 + 2\lambda}$
...
### Type II Non-central Beta $\text{beta}_{ii}(n_2/2, N_1/2, \lambda)$

::: {#def-noncentral-beta2}
### Type II Non-central Beta
$$B_{II} = \frac{X_2}{X_1 + X_2} = 1 - B_I \sim \text{Beta}\left(\frac{n_2}{2}, \frac{n_1}{2}, \lambda\right)$$
:::

Represents the proportion of **unexplained variance ($\text{SSE}/\text{SST}$)**.

* **Approximate Mean:** $E[B_{II}] \approx \frac{n_2}{n_1 + n_2 + 2\lambda}$
...
### Scaled Type II Beta $\text{scaled-beta}_{ii}(n_2/2, N_1/2, \lambda)$

::: {#def-scaled-beta}
### Scaled Type II Beta
$$S = \frac{X_2/n_2}{(X_1+X_2)/(n_1+n_2)} \sim \text{Scaled-Beta}_{II}\left(\frac{n_2}{2}, \frac{n_1}{2}, \lambda\right)$$
:::

Represents the ratio of variances **$\text{MSE}/MST$**.

* **Approximate Mean:** $E[S] \approx \frac{n_1+n_2}{n_1+n_2+2\lambda}$
...
### The Non-central T-distribution $t(n_2, \delta)$

::: {#def-noncentral-t}
### Non-central T
Let $Z \sim N(\delta, 1)$ and $X_2 \sim \chi^2(n_2)$ be independent. The random variable $T$ follows a **non-central t-distribution**:
$$T = \frac{Z}{\sqrt{X_2/n_2}} \sim t(n_2, \delta)$$
:::
...
* **Exact Mean:** $E[T] = \delta \sqrt{\frac{n_2}{2}} \frac{\Gamma((n_2-1)/2)}{\Gamma(n_2/2)}$
* **Approximate Mean:** $E[T] \approx \delta$
...
We use the projection matrix $P_{j_n} = \frac{1}{n}j_n j_n'$ and its complement $(I_n - P_{j_n})$ to partition the transformed vector. By applying **@thm-proj-matrix**, we derive the sampling distributions for the Sum of Squared Errors under each model.
...
* **Total SSE ($SSE_0$ for $M_0$):**
    ...
    Since $E[y^*] \neq 0$, it follows a non-central distribution:
...
* **Residual SSE ($SSE_1$ for $M_1$):**
    ...
    This captures the random noise (central component):
...
* **Difference SS ($SS_{\text{diff}}$):**
    ...
    This captures the signal (non-central component):
...
* **The Type I Beta statistic ($B_I$):**
    Represents the proportion of total variation explained by the deviation from $\mu_0$ (analogous to $R^2$).
    $$B_I = \frac{SS_{\text{diff}}}{SSE_0} = \frac{n(\bar{Y} - \mu_0)^2}{\sum (Y_i - \mu_0)^2}$$

* **The Type II Beta statistic ($B_{II}$):**
    Represents the proportion of total variation that remains *unexplained* (Noise/Total).
    $$B_{II} = \frac{SSE_1}{SSE_0} = \frac{\sum (Y_i - \bar{Y})^2}{\sum (Y_i - \mu_0)^2} = 1 - B_I$$

* **The Scaled Type II Beta statistic ($S_{\text{scaled}}$):**
    Represents the ratio of Mean Square Error to Mean Square Total (MSE/MST).
...
| Statistic | Mean under $H_0$ ($\mu = \mu_0$) | Approximate Mean under $H_1$ ($\mu \neq \mu_0$) |
| :--- | :--- | :--- |
| **$T$** | $0$ | $\delta$ |
| **$F$** | $\frac{n-1}{n-3} \approx 1$ | $1 + 2\lambda$ |
| **$B_I$** | $\frac{1}{n}$ | $\frac{1 + 2\lambda}{n + 2\lambda}$ |
| **$B_{II}$** | $\frac{n-1}{n}$ | $\frac{n-1}{n + 2\lambda}$ |
| **$S_{\text{scaled}}$** | $1$ | $\frac{n}{n + 2\lambda}$ |
