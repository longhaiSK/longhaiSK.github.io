...
We assume the sample size $n$ is large enough for the mean of $F$ to exist ($n > 3$).

**Signal Term:** Let $SNR = \frac{n(\mu - \mu_0)^2}{\sigma^2} = 2\lambda$.

| Statistic | Exact Mean under $H_0$ ($\lambda=0$) | Approximate Mean under $H_1$ ($\lambda \neq 0$) |
| :--- | :--- | :--- |
| **$T$** | $0$ | $\frac{\sqrt{n}(\mu - \mu_0)}{\sigma}$ |
| **$F$** | $\frac{n-1}{n-3} \approx 1$ | $1 + SNR$ |
| **$B_I$** | $\frac{1}{n}$ | $\frac{1 + SNR}{n + SNR}$ |
| **$B_{II}$** | $\frac{n-1}{n}$ | $\frac{n-1}{n + SNR}$ |
| **$S_{\text{scaled}}$** | $1$ | $\frac{n}{n + SNR}$ |

**Key Interpretation:**
All statistics are functionally driven by the **Signal-to-Noise Ratio (SNR)** term. As the true mean $\mu$ deviates further from $\mu_0$, this term grows, driving $F$ up, $B_I$ toward 1, and $S_{\text{scaled}}$ toward 0.
