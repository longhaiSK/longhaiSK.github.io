...
### Idempotency and General Transformations
...
::: {.proof}
$(\Rightarrow)$ Assume $A\Sigma A\Sigma = A\Sigma$. Then:
$$
\begin{aligned}
(\Sigma^{1/2}A\Sigma^{1/2})(\Sigma^{1/2}A\Sigma^{1/2}) &= \Sigma^{1/2}A(\Sigma^{1/2}\Sigma^{1/2})A\Sigma^{1/2} \\
&= \Sigma^{1/2}(A\Sigma A)\Sigma^{1/2} \\
&= \Sigma^{1/2}A\Sigma^{1/2}
\end{aligned}
$$
The reverse follows by multiplying by $\Sigma^{-1/2}$ on both sides.
:::

::: {#thm-quad-dist name="Distribution of y'Ay"}
Let $y \sim N_p(\mu, \Sigma)$, let $A$ be a symmetric matrix of constants of rank $r$, and let $\lambda = \frac{1}{2}\mu'A\mu$. Then $y'Ay$ is $\chi^2(r, \lambda)$ if and only if $A\Sigma$ is idempotent.
:::
...
## Distributions of Projected Multivariate Normal

::: {#thm-proj-dist name="Distribution of Projections"}
Let $V$ be a $k$-dimensional subspace of $\mathcal{R}^n$ and let $y$ be a random vector in $\mathcal{R}^n$ with mean $E(y)=\mu$. Then:

1. $E\{p(y|V)\} = p(\mu|V)$.
2. If $\text{var}(y)=\sigma^2 I_n$, then $\text{var}\{p(y|V)\} = \sigma^2 P_V$ and $E\{\|p(y|V)\|^2\} = \sigma^2 k + \|p(\mu|V)\|^2$.
3. If $y \sim N_n(\mu, \sigma^2 I_n)$, then $\frac{1}{\sigma^2}\|p(y|V)\|^2 = \frac{1}{\sigma^2}y'P_Vy \sim \chi^2(k, \frac{1}{2\sigma^2}\|p(\mu|V)\|^2)$.
:::

::: {.proof}
1. Since the projection operation is linear, $E\{p(y|V)\} = p(E(y)|V) = p(\mu|V)$.
2. $p(y|V) = P_V y$, so $\text{var}\{p(y|V)\} = \text{var}(P_V y) = P_V \sigma^2 I_n P_V^T = \sigma^2 P_V$. The expectation of the squared norm follows from the mean of a quadratic form: $E(\|p(y|V)\|^2) = \text{tr}(\sigma^2 P_V) + \mu^T P_V \mu = \sigma^2 k + \|p(\mu|V)\|^2$.
3. This follows from the general distribution of quadratic forms where $A = \frac{1}{\sigma^2} P_V$.
:::
...
::: {.proof}
1. For $i \ne j$, $\text{cov}(\hat{y}_i, \hat{y}_j) = \sigma^2 P_i P_j = 0$ because the subspaces are orthogonal. Under normality, zero covariance implies independence.
2. Since $\hat{y}_i$ are independent, any functions of them, such as their norms, are also independent.
3. This is a direct application of the Projection Distribution theorem to each subspace.
:::
...
::: {.proof}
If $B\Sigma A = 0$, the normal vectors $By$ and $Ay$ have zero covariance and are thus independent. Consequently, $By$ is independent of any measurable function of $Ay$, such as the quadratic form $y'Ay = \|Ay\|^2$ (assuming $A$ is idempotent). The quadratic-quadratic case follows a similar logic.
:::

### Cochran's Theorem

::: {#thm-cochran-result name="Cochran's Result"}
Let $y \sim N_n(\mu, \sigma^2 I)$ and $y'y = \sum_{i=1}^k y'A_iy$. The quadratic forms $y^T A_i y / \sigma^2$ are mutually independent $\chi^2(r_i, \lambda_i)$ if and only if any one of the following holds:
...
## Non-central F, t, and Beta distributions

::: {#def-f-t-dist name="F and t Distributions"}

1. **F-Distribution:** If $U_1 \sim \chi^2(n_1, \lambda)$ and $U_2 \sim \chi^2(n_2)$ are independent, then $V = \frac{U_1/n_1}{U_2/n_2}$ follows a non-central F distribution.
2. **t-Distribution:** If $W \sim N(\delta, 1)$ and $Y \sim \chi^2(m)$ are independent, then $T = \frac{W}{\sqrt{Y/m}}$ follows a non-central t distribution $t(m, \delta)$.
:::

## Example: i.i.d. Normal Samples (Student's Theorem)

::: {#exm-iid-normal name="Student's Theorem"}
Let $Y_1, \dots, Y_n \sim i.i.d. N(\mu, \sigma^2)$.

* **Sample Mean:** $\bar{Y} \sim N(\mu, \sigma^2/n)$.
* **Sample Variance:** $\frac{(n-1)S^2}{\sigma^2} \sim \chi^2(n-1, 0)$.
* **Independence:** $\bar{Y}$ and $S^2$ are independent.
* **t-statistic:** $T = \frac{\bar{Y}-\mu_0}{S/\sqrt{n}} \sim t(n-1, \lambda)$ where $\lambda = \frac{\mu-\mu_0}{\sigma/\sqrt{n}}$.
:::

::: {.proof}
Let $y \sim N_n(\mu j_n, \sigma^2 I_n)$.

1. $\bar{Y}$ is a linear transformation $\frac{1}{n} j_n' y$.
2. $(n-1)S^2$ is the quadratic form $y'(I_n - P_{j_n})y$. Since $(I_n - P_{j_n})$ is idempotent with rank $n-1$ and projects the mean vector to zero, the result is a central chi-square.
3. Independence holds because the product of the linear form's matrix and the quadratic form's matrix is zero: $\frac{1}{n} j_n' (I_n - P_{j_n}) = 0$.
4. The t-statistic is the ratio of these independent components.
:::