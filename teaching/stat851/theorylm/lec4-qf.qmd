...
## Non-Central Distributions Derived from $\chi^2$

Suppose $X_1 \sim \chi^2(n_1, \lambda)$ is a non-central chi-squared random variable representing the **Signal**, and $X_2 \sim \chi^2(n_2)$ is a central chi-squared random variable representing the **Noise**. We assume $X_1$ and $X_2$ are independent.

The additive relationship between these independent components and the Total Sum of Squares is displayed in @fig-variance-partition.

```{r}
#| label: fig-variance-partition
#| fig-cap: "Partitioning of Total Variation into Independent Signal and Noise Components."
#| echo: false
#| fig-width: 8
#| fig-height: 3
#| warning: false
...
ggplot() +
  # --- 1. Main Line Segments ---
  # Signal Segment (Blue)
  geom_segment(aes(x = x_start, xend = x_break, y = 0, yend = 0), 
               color = "darkblue", linewidth = 2) +
  # Noise Segment (Red)
  geom_segment(aes(x = x_break, xend = x_end, y = 0, yend = 0), 
               color = "darkred", linewidth = 2) +
  
  # --- 2. Vertical Ticks ---
  geom_segment(aes(x = c(x_start, x_break, x_end), 
                   xend = c(x_start, x_break, x_end),
                   y = -0.2, yend = 0.2), 
               color = "black", linewidth = 1) +

  # --- 3. Labels Above (X1, X2) ---
  annotate("text", x = (x_start + x_break)/2, y = 0.5, 
           label = "X[1] ~ (Signal)", parse = TRUE, 
           color = "darkblue", fontface = "bold", size = 5) +
  annotate("text", x = (x_break + x_end)/2, y = 0.5, 
           label = "X[2] ~ (Noise)", parse = TRUE, 
           color = "darkred", fontface = "bold", size = 5) +

  # --- 4. Distribution Brackets (Level 1) ---
  # Blue Bracket (Manual construction for portability)
  geom_segment(aes(x = x_start, xend = x_break, y = -0.5, yend = -0.5), color = "darkblue") +
  geom_segment(aes(x = x_start, xend = x_start, y = -0.5, yend = -0.3), color = "darkblue") +
  geom_segment(aes(x = x_break, xend = x_break, y = -0.5, yend = -0.3), color = "darkblue") +
  annotate("text", x = (x_start + x_break)/2, y = -0.9, 
           label = "chi^2(n[1], lambda)", parse = TRUE, color = "darkblue", size = 4) +

  # Red Bracket
  geom_segment(aes(x = x_break, xend = x_end, y = -0.5, yend = -0.5), color = "darkred") +
  geom_segment(aes(x = x_break, xend = x_break, y = -0.5, yend = -0.3), color = "darkred") +
  geom_segment(aes(x = x_end, xend = x_end, y = -0.5, yend = -0.3), color = "darkred") +
  annotate("text", x = (x_break + x_end)/2, y = -0.9, 
           label = "chi^2(n[2])", parse = TRUE, color = "darkred", size = 4) +

  # --- 5. Total Sum Bracket (Level 2) ---
  geom_segment(aes(x = x_start, xend = x_end, y = -1.6, yend = -1.6), color = "black", linewidth = 0.8) +
  geom_segment(aes(x = x_start, xend = x_start, y = -1.6, yend = -1.4), color = "black", linewidth = 0.8) +
  geom_segment(aes(x = x_end, xend = x_end, y = -1.6, yend = -1.4), color = "black", linewidth = 0.8) +
  annotate("text", x = (x_start + x_end)/2, y = -2.1, 
           label = "Total ~ Sum == X[1] + X[2]", parse = TRUE, 
...
Using $X_1$ and $X_2$, we define the four primary non-central distributions below.

### 1. The Non-Central F-Distribution

The Non-central F-distribution arises when we compare the normalized magnitude of the signal to the normalized magnitude of the noise. It is the fundamental distribution for power analysis in ANOVA and regression, testing if the signal density is significantly larger than the noise density.

::: {#def-noncentral-f}
### Non-central F
The random variable $V$ follows a **non-central F-distribution** $F(n_1, n_2, \lambda)$ defined as:
$$V = \frac{X_1/n_1}{X_2/n_2}$$
:::

The probability density function is expressed as a weighted sum of central F densities, where the weights are Poisson probabilities. This structure reflects the stochastic nature of the non-centrality parameter $\lambda$.

$$f_V(v) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{F}\left(v; n_1+2j, n_2\right)$$

* **Exact Mean** (for $n_2 > 2$):
    $$E[V] = \frac{n_2(n_1 + \lambda)}{n_1(n_2 - 2)}$$
* **Approximate Mean:**
    $$E[V] \approx 1 + \frac{\lambda}{n_1}$$

```{r}
#| label: fig-nc-f
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Non-Central F (n1=5, n2=20)"

x_f <- seq(0, 5, length.out = 300)
df_list <- lapply(c(0, 5, 10), function(l) {
  data.frame(x = x_f, y = df(x_f, 5, 20, ncp = l), lambda = as.factor(l))
})
plot_data <- do.call(rbind, df_list)

ggplot(plot_data, aes(x, y, color = lambda)) + geom_line(size=1) +
  labs(title="Non-central F-Distribution", y="Density") + theme_minimal()
```

### 2. Type I Non-Central Beta Distribution

The Type I Non-central Beta distribution models the ratio of the signal segment to the total stick length. In regression analysis, this statistic is the Coefficient of Determination ($R^2$). It measures the proportion of total variation explained by the model.

::: {#def-noncentral-beta1}
### Type I Non-central Beta
The random variable $B_I$ follows a **Type I non-central Beta distribution** $Beta_I(n_1/2, n_2/2, \lambda)$ defined as:
$$B_I = \frac{X_1}{X_1 + X_2}$$
:::

The density is defined by a Poisson mixture of central Beta distributions. As $\lambda$ increases, the probability mass shifts towards 1, indicating a higher proportion of explained variance.

$$f_{B_I}(x) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{Beta}\left(x; \frac{n_1}{2}+j, \frac{n_2}{2}\right)$$

* **Exact Mean** (using $_1F_1$ confluent hypergeometric function):
    $$E[B_I] = e^{-\lambda/2} \frac{n_1}{n_1+n_2} \,_1F_1\left(\frac{n_1}{2}+1; \frac{n_1+n_2}{2}+1; \frac{\lambda}{2}\right)$$
* **Approximate Mean:**
    $$E[B_I] \approx \frac{n_1 + \lambda}{n_1 + n_2 + \lambda}$$

```{r}
#| label: fig-nc-beta1
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Type I Beta (R-squared)"
...
ggplot(plot_data, aes(x, y, color = lambda)) + geom_line(size=1) +
  labs(title="Type I Non-central Beta (R²)", y="Density") + theme_minimal()
```

### 3. Type II Non-Central Beta Distribution

The Type II Non-central Beta distribution models the complementary ratio: the proportion of the total stick length occupied by the noise ($X_2$). This corresponds to the ratio $SSE/SST$ in linear models.

::: {#def-noncentral-beta2}
### Type II Non-central Beta
The random variable $B_{II}$ follows a **Type II non-central Beta distribution** defined as:
$$B_{II} = \frac{X_2}{X_1 + X_2} = 1 - B_I$$
:::

Here, the non-centrality parameter $\lambda$ affects the *second* shape parameter of the mixture components. As the signal ($\lambda$) gets stronger, the proportion of unexplained noise decreases, shifting the density mass towards 0.

$$f_{B_{II}}(x) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{Beta}\left(x; \frac{n_2}{2}, \frac{n_1}{2}+j\right)$$

* **Exact Mean:**
    $$E[B_{II}] = e^{-\lambda/2} \frac{n_2}{n_1+n_2} \,_1F_1\left(\frac{n_1}{2}; \frac{n_1+n_2}{2}+1; \frac{\lambda}{2}\right)$$
* **Approximate Mean:**
    $$E[B_{II}] \approx \frac{n_2}{n_1 + n_2 + \lambda}$$

```{r}
#| label: fig-nc-beta2
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Type II Beta (Unexplained Variance)"

x_b <- seq(0, 1, length.out = 300)
df_list <- lapply(c(0, 5, 10), function(l) {
  data.frame(x = x_b, y = dbeta(1-x_b, 2.5, 10, ncp = l), lambda = as.factor(l))
})
plot_data <- do.call(rbind, df_list)

ggplot(plot_data, aes(x, y, color = lambda)) + geom_line(size=1) +
  labs(title="Type II Non-central Beta (SSE/SST)", y="Density") + theme_minimal()
```

### 4. Scaled Type II Beta Distribution

The ratio of Mean Square Error ($MSE$) to Mean Square Total ($MST$) is a rescaled version of the Type II Beta variable. It accounts for the degrees of freedom associated with the signal and noise segments, providing a direct metric for comparing error variance to total variance.

::: {#def-scaled-beta}
### Scaled Type II Beta
The statistic $S = MSE/MST$ follows a **Scaled Type II Beta distribution** defined as:
$$S = \frac{X_2/n_2}{(X_1+X_2)/(n_1+n_2)} = \left( \frac{n_1+n_2}{n_2} \right) B_{II}$$
:::

This variable is bounded by $[0, \frac{n_1+n_2}{n_2}]$ rather than $[0, 1]$. Its probability density function is derived by a linear transformation of the Type II Beta PDF.

$$f_S(s) = \frac{n_2}{n_1+n_2} f_{B_{II}}\left( s \cdot \frac{n_2}{n_1+n_2} \right)$$

* **Exact Mean:**
    $$E[S] = \left( \frac{n_1+n_2}{n_2} \right) E[B_{II}]$$
* **Approximate Mean:**
    $$E[S] \approx \frac{n_1+n_2}{n_1+n_2+\lambda}$$
...
x_s <- seq(0, scale_factor, length.out = 300)

df_list <- lapply(c(0, 5, 10), function(l) {
  # Map grid back to [0,1] for calculation
  y_val <- (1/scale_factor) * dbeta(1 - (x_s/scale_factor), n1/2, n2/2, ncp = l)
  y_val[x_s > scale_factor] <- 0
  data.frame(x = x_s, y = y_val, lambda = as.factor(l))
})
plot_data <- do.call(rbind, df_list)

ggplot(plot_data, aes(x, y, color = lambda)) + geom_line(size=1) +
  labs(title="Scaled Type II Beta (MSE/MST)", y="Density") + theme_minimal()
```

## Application: Inference on the Mean of a Normal Sample

Consider a random sample $y \sim N_n(\mu j_n, \sigma^2 I_n)$. We test the full model $M_1: \mu \neq \mu_0$ against the reduced model $M_0: \mu = \mu_0$.

Let $y^* = y - \mu_0 j_n$. We partition the sums of squares as:

1.  **Signal ($U_1$):** $SS_{\text{diff}} = n(\bar{Y} - \mu_0)^2$. Under $M_1$, $SS_{\text{diff}}/\sigma^2 \sim \chi^2(1, \lambda)$.
2.  **Noise ($U_2$):** $SSE_1 = \sum (Y_i - \bar{Y})^2$. Under $M_1$, $SSE_1/\sigma^2 \sim \chi^2(n-1)$.
    
    *(Note: $n_1 = 1$ and $n_2 = n-1$)*.

### Mapping Statistics to Distributions

The standard test statistics used in this inference problem map directly to the distributions defined above.

| Statistic | Formula | Distribution | Interpretation |
| :--- | :--- | :--- | :--- |
| **F-Statistic** | $F = \frac{SS_{\text{diff}}/1}{SSE_1/(n-1)}$ | **Non-central F** <br> $F(1, n-1, \lambda)$ | Ratio of Signal Variance to Noise Variance. |
| **Beta Statistic** | $B = \frac{SS_{\text{diff}}}{SS_{\text{diff}} + SSE_1}$ | **Type I Beta** <br> $Beta_I(\frac{1}{2}, \frac{n-1}{2}, \lambda)$ | Proportion of total variation explained by the model ($R^2$). |
| **Unexplained** | $1-B = \frac{SSE_1}{SS_{\text{diff}} + SSE_1}$ | **Type II Beta** <br> $Beta_{II}(\frac{1}{2}, \frac{n-1}{2}, \lambda)$ | Proportion of total variation remaining in the residuals. |
| **Inverse F** | $S = \frac{1}{F} = \frac{S^2}{n(\bar{Y}-\mu_0)^2}$ | **Scaled Type II** | Ratio of Noise Variance to Signal Variance. |
...
### Sum Squares and Their Distributions

We use the projection matrix $P_{j_n} = \frac{1}{n}j_n j_n'$ and its complement $(I_n - P_{j_n})$ to partition the transformed vector $y^* = y - \mu_0 j_n$. By applying **@thm-proj-matrix**, we derive the sampling distributions for the Sum of Squared Errors under each model and their difference.

1.  **Total SSE ($SSE_0$ for $M_0$):**
    This represents the total squared deviation from the hypothesized mean $\mu_0$.
    $$SSE_0 = \|I_n y^*\|^2 = \sum_{i=1}^n (Y_i - \mu_0)^2$$
    Since $I_n$ has rank $n$ and $E[y^*] = (\mu - \mu_0)j_n$, it follows a non-central distribution:
    $$\frac{SSE_0}{\sigma^2} \sim \chi^2(n, \lambda) \quad \text{where } \lambda = \frac{n(\mu - \mu_0)^2}{2\sigma^2}$$

2.  **Residual SSE ($SSE_1$ for $M_1$):**
    This represents the internal variation around the sample mean $\bar{Y}$.
    $$SSE_1 = \|(I_n - P_{j_n})y^*\|^2 = \sum_{i=1}^n (Y_i - \bar{Y})^2$$
    Since $(I_n - P_{j_n})$ projects onto a space of rank $n-1$ and $(I_n - P_{j_n})E[y^*] = 0$, the distribution is central:
    $$\frac{SSE_1}{\sigma^2} \sim \chi^2(n-1, 0)$$
    *Note: The sample variance is defined as $S^2 = \frac{SSE_1}{n-1}$.*

3.  **Difference SS ($SS_{\text{diff}}$):**
    This represents the variation explained by the sample mean's deviation from $\mu_0$.
    $$SS_{\text{diff}} = \|P_{j_n} y^*\|^2 = n(\bar{Y} - \mu_0)^2$$
    Since $P_{j_n}$ has rank 1 and $E[y^*] = (\mu - \mu_0)j_n$, it captures the non-centrality:
    $$\frac{SS_{\text{diff}}}{\sigma^2} \sim \chi^2(1, \lambda)$$

By Cochran's Theorem, because $P_{j_n}(I_n - P_{j_n}) = 0$, $SS_{\text{diff}}$ and $SSE_1$ are independent.

### Distributions of three equivalent Statistics

Below are the explicit forms of the statistics used to compare $M_0$ and $M_1$:
...
3. **The Beta-statistic:**
   $$B = \frac{n(\bar{Y} - \mu_0)^2}{n(\bar{Y} - \mu_0)^2 + \sum_{i=1}^n (Y_i - \bar{Y})^2}$$



### Distributions under $M_1$ and $M_0$

Under the Full Model ($M_1$), these statistics follow **non-central** distributions because the mean of $y^*$ is $(\mu - \mu_0)j_n \neq 0$. Under the Reduced Model ($M_0$), these simplify to the **central** distributions familiar from introductory statistics.



| Statistic | Under $M_1$ ($\mu$ is unknown) | Under $M_0$ ($\mu = \mu_0$) | Intro Stats Connection |
| :--- | :--- | :--- | :--- |
| **$T$** | $t(n-1, \delta)$ | $t(n-1)$ | One-sample t-test |
| **$F$** | $F(1, n-1, \lambda)$ | $F(1, n-1)$ | Squared t-test / ANOVA |
| **$B$** | $\text{Beta}(\frac{1}{2}, \frac{n-1}{2}, \lambda)$ | $\text{Beta}(\frac{1}{2}, \frac{n-1}{2})$ | $R^2$ (Coefficient of Determination) |



where, the shift in these distributions is governed by the distance between the true mean and the null hypothesis:

* $\delta = \frac{\mu - \mu_0}{\sigma/\sqrt{n}}$
* $\lambda = \frac{n(\mu - \mu_0)^2}{2\sigma^2} = \frac{1}{2}\delta^2$
