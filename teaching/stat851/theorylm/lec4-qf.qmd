...
**Method 3: Using Double Spectral Decomposition**

Since both $A$ and $\Sigma$ are symmetric, we can apply spectral decomposition to both:
$$
A = Q_A \Lambda_A Q_A' = \sum_{i=1}^n \lambda_{A,i} q_{A,i} q_{A,i}'
$$
$$
\Sigma = Q_\Sigma \Lambda_\Sigma Q_\Sigma' = \sum_{j=1}^n \lambda_{\Sigma,j} q_{\Sigma,j} q_{\Sigma,j}'
$$
First, expand the quadratic form using the decomposition of $A$:
$$
y'Ay = y' \left( \sum_{i=1}^n \lambda_{A,i} q_{A,i} q_{A,i}' \right) y = \sum_{i=1}^n \lambda_{A,i} (q_{A,i}' y)^2
$$
Let $z_i = q_{A,i}' y$ be the projection of $y$ onto the $i$-th eigenvector of $A$. By @lem-simple-qf (applied to a scalar), $E(z_i^2) = \text{Var}(z_i) + [E(z_i)]^2$. We calculate these terms:

1.  **Mean:** $E(z_i) = q_{A,i}' E(y) = q_{A,i}' \mu$.
2.  **Variance:** Using the spectral form of $\Sigma$:
    $$
    \begin{aligned}
    \text{Var}(z_i) &= q_{A,i}' \text{Var}(y) q_{A,i} = q_{A,i}' \Sigma q_{A,i} \\
    &= q_{A,i}' \left( \sum_{j=1}^n \lambda_{\Sigma,j} q_{\Sigma,j} q_{\Sigma,j}' \right) q_{A,i} \\
    &= \sum_{j=1}^n \lambda_{\Sigma,j} (q_{A,i}' q_{\Sigma,j})^2
    \end{aligned}
    $$

Substituting these back into the expectation sum:
$$
\begin{aligned}
E(y'Ay) &= \sum_{i=1}^n \lambda_{A,i} \left[ \underbrace{\sum_{j=1}^n \lambda_{\Sigma,j} (q_{A,i}' q_{\Sigma,j})^2}_{\text{Var}(z_i)} + \underbrace{(q_{A,i}' \mu)^2}_{[E(z_i)]^2} \right] \\
&= \sum_{i=1}^n \sum_{j=1}^n \lambda_{A,i} \lambda_{\Sigma,j} (q_{A,i}' q_{\Sigma,j})^2 + \mu' \left( \sum_{i=1}^n \lambda_{A,i} q_{A,i} q_{A,i}' \right) \mu
\end{aligned}
$$

**Geometric Interpretation:**
The term $(q_{A,i}' q_{\Sigma,j})$ represents the cosine of the angle between the eigenvectors of $A$ and $\Sigma$. The total expectation is the sum of:
1.  **Trace Term:** $\text{tr}(A\Sigma) = \sum_{i,j} \lambda_{A,i} \lambda_{\Sigma,j} \cos^2(\theta_{ij})$, which measures how the principal directions of the quadratic form align with the principal directions of the noise.
2.  **Mean Term:** $\mu'A\mu$, which is the value of the quadratic form at the mean.
:::
