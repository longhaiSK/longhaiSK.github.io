...
# 1. Generate Data from N_3(mu, I_3)
set.seed(123)
n <- 200
mu <- c(2, 3, 4)
sigma <- diag(3) # Identity matrix
data <- mvrnorm(n, mu, sigma)
df <- as.data.frame(data)
colnames(df) <- c("x", "y", "z")

# 2. Define the Projection Subspace (Plane)
# Let's use the standard axes for q1 and q2 for clarity in "labeling axes"
# This makes the "floor" the z=0 plane.
q1 <- c(1, 0, 0)
q2 <- c(0, 1, 0)

# 3. Project Points
# The projection of y onto the plane spanned by q1, q2 is:
# y_hat = (y . q1)q1 + (y . q2)q2
# Since q1, q2 are orthonormal and align with axes, this is just (x, y, 0)
proj_points <- t(apply(data, 1, function(p) {
  sum(p * q1) * q1 + sum(p * q2) * q2
}))
...
plot_ly() %>%
  ...
  # Draw lines connecting point to projection (Residuals)
  add_segments(
  ...
  # Label the axes of the subspace (q1 and q2)
  add_segments(
    x = c(0, 0), xend = c(5, 0),
    y = c(0, 0), yend = c(0, 5),
    z = c(0, 0), zend = c(0, 0),
