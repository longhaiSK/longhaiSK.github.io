...
We begin by considering two independent Chi-squared random variables that form the building blocks for several distributions fundamental to statistical power analysis and hypothesis testing.

Let the random variables $X_1$ and $X_2$ be defined as:

1.  **Non-central Component ($X_1$):** $X_1 \sim \chi^2(n_1, \lambda)$.
    This variable contains the non-centrality parameter $\lambda$. In applications such as linear models, this typically represents the **Signal Sum of Squares** (the variation explained by the model), where $\lambda$ quantifies the systematic deviation from the null hypothesis.

2.  **Central Component ($X_2$):** $X_2 \sim \chi^2(n_2)$.
    This variable follows a central Chi-squared distribution. It typically represents the **Noise (or Error) Sum of Squares**, arising from pure random variation.

We can visualize the summation of these independent components as partitioning a fixed total length.

```{tikz}
%| label: fig-variance-partition
%| fig-cap: "Partitioning of Total Variation. The axis marks the cumulative sums: $X_2$ (Noise) and $X_1$ (Signal). In linear models, these correspond to SSE and Model SS, summing to SST."
%| echo: false
%| fig-align: "center"
%| out-extra: 'style="width: 78% !important;"'
%| engine.opts:
...
\def\lenNoise{4}
\def\lenSignal{6}
\def\yBar{1.0} % Height of the bars to "leave 0" visible
\def\yBrace{0.8}

% Coordinates on Axis (y=0)
\coordinate (O) at (0,0);
\coordinate (P_SSE) at (\lenNoise, 0);
\coordinate (P_SST) at (\lenNoise + \lenSignal, 0);

% Coordinates for Bars (y=\yBar)
...
% --- 2. The Bars (Lifted Up) ---
% Segment X2 (Noise) - Red
\draw[ultra thick, red!70!black] (Bar_Start) -- (Bar_Mid);
\node[above, red!70!black, font=\bfseries] at ($(Bar_Start)!0.5!(Bar_Mid)$) {$X_2$ (Noise)};

% Segment X1 (Signal) - Blue
\draw[ultra thick, blue!70!black] (Bar_Mid) -- (Bar_End);
\node[above, blue!70!black, font=\bfseries] at ($(Bar_Mid)!0.5!(Bar_End)$) {$X_1$ (Signal)};

% --- 3. Braces (Under the Bars) ---
% Brace for X2
\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=2pt}, thick, red!70!black] 
    (0, \yBrace) -- (\lenNoise, \yBrace) 
    node[midway, below=8pt] {$\sim \chi^2(n_2)$};

% Brace for X1
\draw[decorate, decoration={brace, mirror, amplitude=6pt, raise=2pt}, thick, blue!70!black] 
    (\lenNoise, \yBrace) -- (\lenNoise + \lenSignal, \yBrace) 
    node[midway, below=8pt] {$\sim \chi^2(n_1, \lambda)$};

\end{tikzpicture}
```

Using these two components, we construct four distributions.

### The Non-central F-distribution

::: {#def-noncentral-f}
### Non-central F
Let $X_1 \sim \chi^2(n_1, \lambda)$ and $X_2 \sim \chi^2(n_2)$ be independent. The random variable $V$ follows a **non-central F-distribution** $F(n_1, n_2, \lambda)$, defined as the ratio of the mean squares:
$$V = \frac{X_1/n_1}{X_2/n_2}$$
:::

Physically, this statistic compares the "density" of the signal ($X_1/n_1$) to the "density" of the noise ($X_2/n_2$). In the context of ANOVA or regression, this is the standard test statistic. When $\lambda=0$ (the null hypothesis), $V$ follows the central F-distribution. As $\lambda$ increases, the numerator $X_1$ grows stochastically larger, shifting the distribution to the right and increasing the probability of rejection.

The probability density function is a Poisson mixture of central F densities:

$$f_V(v) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{F}\left(v; n_1+2j, n_2\right)$$

**Expectation** (for $n_2 > 2$):
The mean increases linearly with $\lambda$.
...
* **Approximate Mean:**
    ...
    *(Note: This approximation assumes $n_2$ is large. It converges to the exact central mean $1$ only asymptotically as $n_2 \to \infty$.)*

```{r}
#| label: fig-nc-f
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Non-Central F ($n_1=5, n_2=20$) for varying $\\lambda$."
...
### Type I Non-central Beta Distribution

::: {#def-noncentral-beta1}
### Type I Non-central Beta
The random variable $B_I$ follows a **Type I non-central Beta distribution** $Beta_I(n_1/2, n_2/2, \lambda)$, defined as the proportion of the non-central component to the sum:
$$B_I = \frac{X_1}{X_1 + X_2}$$
:::

This distribution models the proportion of total variation attributable to the signal. In linear regression, this is the distribution of the **Coefficient of Determination ($R^2$)** under the alternative hypothesis. As $\lambda$ increases, $X_1$ dominates the total sum, causing the probability mass to concentrate near 1.

The density is a Poisson mixture where the non-centrality increments the first shape parameter:

$$f_{B_I}(x) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{\text{Beta}}\left(x; \frac{n_1}{2}+j, \frac{n_2}{2}\right)$$
...
* **Approximate Mean:**
    ...
    *(Note: This formula becomes the exact mean of the central Beta distribution when $\lambda = 0$.)*

```{r}
#| label: fig-nc-beta1
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Type I Beta ($R^2$) for varying $\\lambda$."
...
The random variable $B_{II}$ follows a **Type II non-central Beta distribution**, defined as the proportion of the central component to the sum:
$$B_{II} = \frac{X_2}{X_1 + X_2} = 1 - B_I$$
:::

This distribution describes the proportion of total variation that remains unexplained (noise). In regression, this corresponds to the ratio **SSE/SST**. While the amount of noise ($X_2$) does not change with $\lambda$, its *relative* share of the total diminishes as the signal $X_1$ grows. Thus, the distribution shifts toward 0 as $\lambda$ increases.

The Poisson weights affect the second shape parameter, reflecting that the non-centrality is present in the denominator (the total) but not the numerator (the noise):

$$f_{B_{II}}(x) = \sum_{j=0}^{\infty} \frac{e^{-\lambda/2}(\lambda/2)^j}{j!} f_{\text{Beta}}\left(x; \frac{n_2}{2}, \frac{n_1}{2}+j\right)$$
...
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Type II Beta ($SSE/SST$) for varying $\\lambda$."
...
### Scaled Type II Beta Distribution

::: {#def-scaled-beta}
### Scaled Type II Beta
The statistic $S$ follows a **Scaled Type II Beta distribution**, defined as the ratio of the mean square of the central component to the mean square of the total:
$$S = \frac{X_2/n_2}{(X_1+X_2)/(n_1+n_2)} = \left( \frac{n_1+n_2}{n_2} \right) B_{II}$$
:::

This distribution models the ratio **MSE/MST**. Unlike the standard Type II Beta, which compares raw sums, this statistic adjusts for degrees of freedom to compare *variances*. Because the Mean Square Total averages over more degrees of freedom ($n_1+n_2$) than the Mean Square Error ($n_2$), the support of this distribution is $[0, \frac{n_1+n_2}{n_2}]$, extending beyond 1.

The density function is a linear transformation of the Type II Beta PDF:
...
* **Approximate Mean:**
    ...
    *(Note: This formula becomes the exact mean when $\lambda = 0$, yielding $E[S]=1$.)*
...
While the distributions above model *variances* (sums of squares), the **Non-central t-distribution** models *amplitudes*. It is the standard statistic for testing the mean of a single group or the difference between two means (t-test) when the null hypothesis is false.

This statistic compares a **Signal Amplitude** (Normal variable) to a **Noise Amplitude** (Root-mean-square error).

::: {#def-noncentral-t}
### Non-central t
Let $Z \sim N(\delta, 1)$ and $X_2 \sim \chi^2(n_2)$ be independent. The random variable $T$ follows a **non-central t-distribution** $t(n_2, \delta)$:
$$T = \frac{Z}{\sqrt{X_2/n_2}}$$
:::

**Relationship to F-distribution:**
The square of a non-central t-variable is a non-central F-variable with 1 numerator degree of freedom:
$$T^2 \sim F(1, n_2, \lambda) \quad \text{where } \lambda = \delta^2$$

**Expectation:**
The mean is proportional to the non-centrality parameter $\delta$.

* **Exact Mean** (for $n_2 > 1$):
    $$E[T] = \delta \sqrt{\frac{n_2}{2}} \frac{\Gamma((n_2-1)/2)}{\Gamma(n_2/2)}$$

* **Approximate Mean:**
    $$E[T] \approx \delta$$
    *(Note: As $n_2 \to \infty$, the denominator $\sqrt{X_2/n_2}$ converges to 1, and $T$ converges to $Z \sim N(\delta, 1)$.)*

```{r}
#| label: fig-nc-t
#| echo: false
#| fig-height: 4
#| fig-cap: "Densities of Non-Central t ($df=20$) for varying $\\delta$."
