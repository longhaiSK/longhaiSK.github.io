...
::: proof
The decomposition $y = P_0 y + (P_1 - P_0) y + (I - P_1) y$ holds by simple algebraic expansion (summing the matrices gives the identity $I$).

To prove the Pythagorean relationship, we show the three components lie in mutually orthogonal subspaces:

1.  **Component 1:** $\hat{y}_0 = P_0 y \in \text{Col}(P_0)$.

2.  **Component 2:** Let $v_2 = (P_1 - P_0) y$. By @thm-diff-projection, $P_1 - P_0$ projects onto $\text{Col}(P_0)^\perp \cap \text{Col}(P_1)$. Thus, $v_2$ is orthogonal to $\text{Col}(P_0)$ (and therefore orthogonal to $\hat{y}_0$).

3.  **Component 3:** Let $v_3 = (I - P_1) y$. By definition of the complement projection, $v_3 \in \text{Col}(P_1)^\perp$.

**Mutual Orthogonality:**

-   $\hat{y}_0 \perp v_2$: Because $\hat{y}_0 \in \text{Col}(P_0)$ and $v_2 \in \text{Col}(P_0)^\perp$.
-   $(\hat{y}_0 + v_2) \perp v_3$: The sum $\hat{y}_0 + v_2 = P_1 y = \hat{y}_1$ lies entirely within $\text{Col}(P_1)$. Since $v_3$ is in $\text{Col}(P_1)^\perp$, it is orthogonal to the entire space $\text{Col}(P_1)$, and thus orthogonal to both $\hat{y}_0$ and $v_2$.

Since the three vectors are mutually orthogonal, the Pythagorean theorem applies:
$$
\|y\|^2 = \|\hat{y}_0\|^2 + \|\hat{y}_1 - \hat{y}_0\|^2 + \|y - \hat{y}_1\|^2
$$
:::
...
**Conclusion:** $$ \underbrace{\|y\|^2 - N\bar{y}_{..}^2}_{\text{SST}} = \underbrace{(\sum n_i\bar{y}_{i.}^2 - N\bar{y}_{..}^2)}_{\text{SSB}} + \underbrace{(\sum \sum y_{ij}^2 - \sum n_i\bar{y}_{i.}^2)}_{\text{SSW}} $$ 
