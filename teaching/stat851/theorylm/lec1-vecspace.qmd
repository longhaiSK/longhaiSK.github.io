...
::: {.proof}
  ...
  Thus, the column space of $X$, $\text{Col}(X)$, is a subspace of the column space of $C$.
  $$
  \text{Dim}(\text{Col}(X)) \le \text{Dim}(\text{Col}(C)) \le r
  $$
...
::: {.proof}
Let $k = \text{Nullity}(X)$. Let $\{u_1, \dots, u_k\}$ be a basis for $\text{Ker}(X)$.
Since these $k$ vectors are linearly independent in $\mathbb{R}^p$, we can extend this basis to form a basis for the entire space $\mathbb{R}^p$: $\{u_1, \dots, u_k, w_1, \dots, w_r\}$, where $k + r = p$.

We claim that $\{Xw_1, \dots, Xw_r\}$ forms a basis for $\text{Col}(X)$ (the image of $X$).

1.  **Spanning:** For any $y \in \text{Col}(X)$, there exists $v \in \mathbb{R}^p$ such that $y = Xv$. We can write $v$ as a linear combination of the basis vectors:
    $$v = \sum a_i u_i + \sum b_j w_j$$
    Then:
    $$y = Xv = \sum a_i \underbrace{X u_i}_{0} + \sum b_j X w_j = \sum b_j (X w_j)$$
    Thus, the vectors $Xw_j$ span the column space.

2.  **Independence:** Suppose $\sum c_j Xw_j = 0$. Then $X(\sum c_j w_j) = 0$.
    This implies that the vector $z = \sum c_j w_j$ lies in $\text{Ker}(X)$.
    Therefore, $z$ must be expressible as a linear combination of the kernel basis $\{u_i\}$:
    $$\sum c_j w_j = \sum d_i u_i \implies \sum c_j w_j - \sum d_i u_i = 0$$
    Since $\{u_1, \dots, u_k, w_1, \dots, w_r\}$ is a basis for $\mathbb{R}^p$, all coefficients must be zero. Thus, $c_j = 0$ for all $j$.
    This proves $\{Xw_1, \dots, Xw_r\}$ are linearly independent.

Since the dimension of $\text{Col}(X)$ is $r$, we have:
$\text{Rank}(X) = r = p - k = p - \text{Nullity}(X)$.
:::
