...
#| fig-cap: "Geometric Proof of Pythagorean Theorem using Similar Triangles"
#| echo: false
...
% Define base dimensions (3-4-5 triangle)
\def\a{3}
\def\b{4}
\def\c{5}
\def\angleA{36.87} % asin(3/5)
\def\angleB{53.13} % asin(4/5)

% --- PART 1: The Decomposed Triangle (Top) ---

\begin{scope}[shift={(0, 3)}]
    % Coordinates for a standard right triangle resting on hypotenuse
    % We rotate it so hypotenuse is horizontal to match standard proof visualization
    % Or we can stick to legs-axis aligned. Let's do legs-axis for clarity of coordinates
    % but rotated slightly to look like the sketch.
    
    % Let's define the triangle ABC where C is the right angle.
    % To calculate the altitude split easily, we place the hypotenuse on the x-axis.
    % Coordinate calculation for a 3-4-5 triangle sitting on hypotenuse:
    % Altitude h = 2.4. 
    % Projection of leg 3 (a) is 1.8.
    % Projection of leg 4 (b) is 3.2.
    
    \coordinate (A) at (0,0);       % Left vertex
    \coordinate (B) at (5,0);       % Right vertex
    \coordinate (C) at (1.8, 2.4);  % Top vertex (Right Angle)
    \coordinate (H) at (1.8, 0);    % Foot of altitude

    % Draw S1 (Left sub-triangle) - The "a" triangle
    \fill[yellow!20] (A) -- (H) -- (C) -- cycle;
    \draw[thick, orange] (A) -- (H) -- (C) -- cycle;
    
    % Draw S2 (Right sub-triangle) - The "b" triangle
    \fill[green!20] (H) -- (B) -- (C) -- cycle;
    \draw[thick, green!60!black] (H) -- (B) -- (C) -- cycle;

    % Outline the main triangle S
    ...
    % Right angle markers
    \draw (1.8, 0) -- (1.8, 0.2) -- (1.6, 0.2); % At altitude
    % Note: The top angle at C is 90, but split. 

    % Equation to the right
    \node[anchor=west] at (6, 1.5) {\Large $S = S_1 + S_2$};
\end{scope}

% --- PART 2: The Exploded Similar Triangles (Bottom) ---

\begin{scope}[shift={(0, -1)}]
    % We draw three triangles oriented identically to show similarity
    % Orientation: Hypotenuse at bottom, angle theta at left.
    
    % -- Triangle S1 (Scaled by a/c = 0.6) --
    \begin{scope}[shift={(0,0)}]
        \def\k{0.6} % Scale factor
        ...
        % Labels
        \node[below] at (2.5*\k, 0) {$a$};
        \node at (1.5, -0.8) {\large $S_1$};
        \draw (0.3,0) arc (0:\angleB:0.3); % Theta angle
        \node at (0.5, 0.2) {\tiny $\theta$};
    \end{scope}

    % -- Triangle S2 (Scaled by b/c = 0.8) --
    \begin{scope}[shift={(3.5,0)}]
        \def\k{0.8} % Scale factor
        ...
        % Labels
        \node[below] at (2.5*\k, 0) {$b$};
        \node at (2, -0.8) {\large $S_2$};
        \draw (0.3,0) arc (0:\angleB:0.3);
        \node at (0.5, 0.2) {\tiny $\theta$};
    \end{scope}

    % -- Triangle S (Original scale = 1.0) --
    \begin{scope}[shift={(8,0)}]
        \def\k{1.0} % Scale factor
        ...
        % Labels
        \node[below] at (2.5*\k, 0) {$c$};
        \node at (2.5, -0.8) {\large $S$};
        \draw (0.3,0) arc (0:\angleB:0.3);
        \node at (0.5, 0.2) {\tiny $\theta$};
    \end{scope}
\end{scope}

% --- PART 3: The Proof Text (Bottom) ---
\begin{scope}[shift={(0, -3.5)}]
    \node[right, align=left] at (0, 0) {
        Since the triangles are similar, their areas are proportional to the squares of their corresponding sides (the hypotenuses):\\
        $S_1 = a^2 \cdot k$\\
        $S_2 = b^2 \cdot k$\\
        $S \phantom{_1} = c^2 \cdot k \quad \text{where } k = \frac{1}{2}\cos\theta\sin\theta$
    };
    
    \node[right, align=left, color=blue!80!black] at (0, -2.0) {
        Substituting into $S = S_1 + S_2$:\\
        $c^2 \cdot k = a^2 \cdot k + b^2 \cdot k$\\
        $\Rightarrow \mathbf{c^2 = a^2 + b^2}$
    };
\end{scope}
