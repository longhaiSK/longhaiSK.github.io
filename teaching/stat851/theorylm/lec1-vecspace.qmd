...
Let $e = y - \hat{y} = (I_p - P)y$. To verify that $e$ is orthogonal to $\text{Col}(P)$, it suffices to show that $e$ is orthogonal to every column of $P$. In matrix notation, this is equivalent to showing $e^\top P = 0$. We compute this directly:
...
Let $P$ be a $p \times p$ matrix. $P$ is the orthogonal projection matrix onto $\text{Col}(P)$ if and only if:
...
* **Proof of (1):** Let $v \in \text{Col}(P)$. Then $v = Px$ for some $x$.
    $$
    Pv = P(Px) = P^2 x = Px = v
    $$
...
* **Proof of Idempotence ($P^2 = P$):**
    ...
    For any vector $x \in \mathbb{R}^p$, let $y = Px$. By definition, $y \in \text{Col}(P)$.
...
* **Proof of Symmetry ($P^\top = P$):**

    We decompose any two vectors $x, y \in \mathbb{R}^p$ into components inside and orthogonal to $\text{Col}(P)$.
...
Let $P$ be a $p \times p$ orthogonal projection matrix operating in the space $\mathbb{R}^p$. The matrix $M$ defined as:
$$
M = I_p - P
$$
is the orthogonal projection matrix onto the orthogonal complement of the column space of $P$, denoted $\text{Col}(P)^\perp \subseteq \mathbb{R}^p$.
:::
...
* **Direction 1**: $\text{Col}(M) \subseteq \text{Col}(P)^\perp$
...
* **Direction 2**: $\text{Col}(P)^\perp \subseteq \text{Col}(M)$
...
Let $P_0$ and $P_1$ be $p \times p$ orthogonal projection matrices such that $\text{Col}(P_0) \subseteq \text{Col}(P_1)$. Then:
...
Let $y \in \mathbb{R}^p$ be an arbitrary vector. By definition, the vector $v = P_0 y$ lies in $\text{Col}(P_0)$. Given $\text{Col}(P_0) \subseteq \text{Col}(P_1)$, it follows that $v \in \text{Col}(P_1)$.
...
Since $P_1 P_0 y = P_0 y$ holds for all $y \in \mathbb{R}^p$, we conclude $P_1 P_0 = P_0$.
...
Let $P_0$ and $P_1$ be $p \times p$ orthogonal projection matrices. Let $\text{Col}(P_0)$ and $\text{Col}(P_1)$ denote the column spaces (images) of $P_0$ and $P_1$ respectively. We assume that the column space of $P_0$ is a subspace of the column space of $P_1$, i.e., $\text{Col}(P_0) \subseteq \text{Col}(P_1)$.
...
Let $y \in \mathbb{R}^p$ be an arbitrary vector. By the definition of a projection matrix, the vector $v = P_0 y$ must lie in the column space $\text{Col}(P_0)$. Given the condition $\text{Col}(P_0) \subseteq \text{Col}(P_1)$, it follows that $v$ is also an element of $\text{Col}(P_1)$.

A fundamental property of any projection matrix is that it acts as the identity operator for any vector already residing in its column space. Therefore, since $v \in \text{Col}(P_1)$, we have $P_1 v = v$. Substituting $v = P_0 y$, we obtain:
$$
P_1 (P_0 y) = P_0 y
$$

To establish equality between the matrices themselves, we rely on the principle that if $Ay = By$ for all $y \in \mathbb{R}^p$, then $A = B$. Since the equation holds for all arbitrary $y$, we conclude $P_1 P_0 = P_0$.

**2. Proof of** $P_0 P_1 = P_0$:

Taking the transpose of the result from part 1 ($P_1 P_0 = P_0$) and applying the symmetry property of orthogonal projection matrices ($P' = P$):
$$
(P_1 P_0)' = P_0' \implies P_0' P_1' = P_0' \implies P_0 P_1 = P_0
$$
:::
