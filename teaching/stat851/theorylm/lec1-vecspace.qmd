...
Let $P$ be the orthogonal projection matrix onto a subspace $\mathcal{V}$. The matrix $M$ defined as:
$$
M = I - P
$$
is the orthogonal projection matrix onto the orthogonal complement, $\mathcal{V}^\perp$.
:::

::: {.proof}
**1. Symmetry and Idempotency**
Since $P$ is a projection matrix, we know $P^\top = P$ and $P^2 = P$. We verify these properties for $M$:
$$
...
**2. Identifying the Subspace**
By @thm-proj-col, $M$ projects onto its own column space, $\text{Col}(M)$. A vector $v$ is in $\text{Col}(M)$ if and only if it is fixed by the projection ($Mv = v$).
$$
Mv = v
$$ {#eq-fixed-point}
...
The condition $Pv = 0$ in @eq-nullspace implies that $v$ belongs to the null space of $P$. Since $P$ is the orthogonal projection onto $\mathcal{V}$, its null space is exactly the orthogonal complement $\mathcal{V}^\perp$. Thus, $\text{Col}(M) = \mathcal{V}^\perp$.
:::
