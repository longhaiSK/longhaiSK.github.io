...
To prove $P_0 P_1 = P_0$, we consider the action of the matrix $P_0(P_1 - P_0)$ on an arbitrary vector $y \in \mathbb{R}^n$.

Let $\hat{y}_1 = P_1 y$ and $\hat{y}_0 = P_0 y$ be the orthogonal projections of $y$ onto $\text{Col}(P_1)$ and $\text{Col}(P_0)$, respectively. We define the corresponding residual vectors as:

$$
e_1 = y - \hat{y}_1 \quad \text{and} \quad e_0 = y - \hat{y}_0
$$

By the properties of orthogonal projections, $e_1 \perp \text{Col}(P_1)$ and $e_0 \perp \text{Col}(P_0)$. Because $\text{Col}(P_0) \subseteq \text{Col}(P_1)$, any vector orthogonal to $\text{Col}(P_1)$ is necessarily orthogonal to $\text{Col}(P_0)$. Thus, $e_1 \perp \text{Col}(P_0)$, which implies $P_0 e_1 = 0$.

Now, consider the vector $w = (P_1 - P_0) y$. This can be expressed as the difference between the two fitted value vectors:

$$
w = \hat{y}_1 - \hat{y}_0 = (y - e_1) - (y - e_0) = e_0 - e_1
$$

Since both $e_0$ and $e_1$ are orthogonal to $\text{Col}(P_0)$, their difference $w$ must also be orthogonal to $\text{Col}(P_0)$. Applying the projection matrix $P_0$ to this difference yields:

$$
P_0 (P_1 - P_0) y = P_0 w = P_0 e_0 - P_0 e_1 = 0 - 0 = 0
$$

Since this holds for any $y \in \mathbb{R}^n$, we have:

$$
P_0(P_1 - P_0) = 0 \implies P_0 P_1 - P_0^2 = 0
$$

Given that $P_0$ is idempotent ($P_0^2 = P_0$), it follows that $P_0 P_1 = P_0$. The first identity, $P_1 P_0 = P_0$, is obtained by taking the transpose of both sides: $(P_0 P_1)' = P_1' P_0' = P_1 P_0 = P_0' = P_0$.
:::
