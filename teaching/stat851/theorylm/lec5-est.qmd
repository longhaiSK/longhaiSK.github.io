...
$$ E[1 - R^2 | H_0] = 1 - \frac{k}{n-1} = \frac{n-k-1}{n-1} $$
...
```{tikz}
%| label: fig-variance-sticks-v4
%| fig-cap: "Comparison of Variance Estimators. The vertical positions of the braces are slightly offset to avoid overlap, but the Expectation labels are aligned horizontally for easier comparison."
%| echo: false
%| fig-align: "center"
%| engine.opts:
%|   extra.preamble: "\\usetikzlibrary{decorations.pathreplacing, arrows.meta}"
...
\begin{tikzpicture}[xscale=1, yscale=1.3]
  ...
  % --- Parameters ---
  % n=20, k=5. Visual Scale: 1 unit = 20 units of SS
  \def\xSSErr{3.0}
  \def\xSSTot{10.0}
  \def\xStwoE{4.28}
  \def\xStwoY{10.53}
  ...
  % Y-shift: -2.5
  \coordinate (O2) at (0, -2.5);
  \coordinate (End2) at (12, -2.5);

  \draw[axis] (-0.5, -2.5) -- (End2) node[right] {\textbf{MLE} ($\hat{\sigma}^2$)};
  \draw[tick] (O2) -- ++(0, -0.15);
  \draw[tick] (\xSSErr, -2.5) -- ++(0, -0.15) node[below] {$\hat{\sigma}^2_e$};
  \draw[tick] (\xSSTot, -2.5) -- ++(0, -0.15) node[below] {$\hat{\sigma}^2_y$};

  \draw[dashedline] (\xSSErr, 0) -- (\xSSErr, -2.5);
  \draw[dashedline] (\xSSTot, 0) -- (\xSSTot, -2.5);

  % --- MLE Braces ---
  % Strategy for alignment:
  % Brace 1 (Error): raise=15pt. Node offset: below=20pt. Total depth ~ 35pt.
  % Brace 2 (Total): raise=25pt. Node offset: below=10pt. Total depth ~ 35pt.
  
  \draw[decorate, decoration={brace, amplitude=5pt, mirror, raise=15pt}, thick, color=blue!80]
    (O2) -- (\xSSErr, -2.5) 
    node[midway, below=20pt] {$E = \sigma^2 \frac{n-k-1}{n}$};
    
  \draw[decorate, decoration={brace, amplitude=5pt, mirror, raise=25pt}, thick, color=blue!80]
    (O2) -- (\xSSTot, -2.5) 
    node[midway, below=10pt] {$E = \sigma^2 \frac{n-1+\lambda}{n}$};

  % --- Stick 3: Unbiased Estimator (SS / df) ---
  % Y-shift: -6.5
  \coordinate (O3) at (0, -6.5);
  \coordinate (End3) at (12, -6.5);

  \draw[axis] (-0.5, -6.5) -- (End3) node[right] {\textbf{Unbiased} ($s^2$)};
  \draw[tick] (O3) -- ++(0, -0.15);
  \draw[tick] (\xStwoE, -6.5) -- ++(0, -0.15) node[below] {$s^2_e$};
  \draw[tick] (\xStwoY, -6.5) -- ++(0, -0.15) node[below] {$s^2_y$};

  \draw[dashedline] (\xSSErr, -2.5) -- (\xSSErr, -6.5);
  \draw[dashedline] (\xSSTot, -2.5) -- (\xSSTot, -6.5);

  \draw[shiftarrow] (\xSSErr, -6.5) -- (\xStwoE, -6.5) node[midway, above, font=\scriptsize] {$\times \frac{n}{n-k-1}$};
  \draw[shiftarrow] (\xSSTot, -6.5) -- (\xStwoY, -6.5) node[midway, above, font=\scriptsize] {$\times \frac{n}{n-1}$};
  
  % --- Unbiased Braces ---
  % Same alignment strategy
  
  \draw[decorate, decoration={brace, amplitude=5pt, mirror, raise=15pt}, thick, color=green!60!black]
    (O3) -- (\xStwoE, -6.5) 
    node[midway, below=20pt] {$E = \sigma^2$};

  \draw[decorate, decoration={brace, amplitude=5pt, mirror, raise=25pt}, thick, color=green!60!black]
    (O3) -- (\xStwoY, -6.5) 
    node[midway, below=10pt] {$E = \sigma^2 (1 + \frac{\lambda}{n-1})$};
...
## Inference for Population Proportion's Multiple Correlation ($\rho^2$)

### Expectation of Adjusted $R^2$
...
Using the approximation $E[f/g] \approx E[f]/E[g]$ for large $n$, we can express the expectation of $R^2_a$ in terms of the underlying true signal. Let $\mu = X\beta$ be the vector of true means. The centered signal vector is $X_c\beta = (I - P_{j_n})\mu$.

$$E[R^2_a] \approx \frac{\frac{\|X_c\beta\|^2}{n}}{\sigma^2 + \frac{\|X_c\beta\|^2}{n}}$$

### Derivation of Signal Variance ($\sigma^2_\mu$)
...
$$
X_c\beta = P_{X_c} \mu = (H - P_{j_n})\mu = \mu - \bar{\mu}j_n = 
\begin{pmatrix} 
\mu_1 - \bar{\mu} \\ 
\mu_2 - \bar{\mu} \\ 
\vdots \\ 
...
This vector represents the deviation of each observation's true mean from the grand mean. Consequently, we define the **Signal Variance** ($\sigma^2_\mu$) as the average squared deviation (using $n$ as the divisor):

$$\sigma^2_\mu = \frac{\|X_c\beta\|^2}{n} = \frac{\sum_{i=1}^n (\mu_i - \bar{\mu})^2}{n}$$

Substituting this back into our expectation formula, we see that $R^2_a$ is an asymptotically unbiased estimator for the **proportion of variance explained by the signal** in the population, which we denote by $\rho^2$:

$$ E[R^2_a] \approx \frac{\sigma^2_\mu}{\sigma^2 + \sigma^2_\mu} = \rho^2$$

where the total variance is $\sigma^2_Y = \sigma^2 + \sigma^2_\mu$.

::: {.callout-note}
## Fixed vs. Random Covariates
...
1.  **Estimating Signal Variance ($\sigma^2_\mu$):**
    Observing that $E[\text{MST}] \approx \sigma^2 + \sigma^2_\mu$ and $E[\text{MSE}] = \sigma^2$, the difference $\text{MST} - \text{MSE}$ provides a direct method-of-moments estimator for the variance of the signal itself.

2.  **Testing for Signal Existence (MSR):**
...
1.  **Expressing $F$ via Standard $R^2$:**
    ...
    $$
    F = \frac{\text{SSR}/k}{\text{SSE}/(n-k-1)} = \frac{R^2}{1 - R^2} \cdot \frac{n - k - 1}{k}
    $$

2.  **Expected Value of $F$ as a function of $\sigma^2_\mu$ and $\sigma^2$:**
    Using the new definition of population signal variance $\sigma^2_\mu = \frac{\|X_c\beta\|^2}{n}$ and noise variance $\sigma^2$, we can approximate the expected value of the $F$-statistic.
    
    Recall that $E[\text{SSR}] \approx \|X_c\beta\|^2 + k\sigma^2 = n\sigma^2_\mu + k\sigma^2$. Thus:
    $$
    E[F] \approx \frac{E[\text{MSR}]}{E[\text{MSE}]} \approx \frac{(n\sigma^2_\mu + k\sigma^2)/k}{\sigma^2} = 1 + \frac{n}{k} \left( \frac{\sigma^2_\mu}{\sigma^2} \right)
    $$
    
    The exact mean, derived from the non-central $F$ distribution $F(k, n-k-1, \lambda)$, is:
    $$
    E[F] = \frac{n-k-1}{n-k-3} \left( 1 + \frac{n}{k} \frac{\sigma^2_\mu}{\sigma^2} \right), \quad \text{for } n-k-1 > 3
    $$
...
Recall that the $F$-statistic follows a non-central distribution $F(k, n-k-1, \lambda)$. The non-centrality parameter $\lambda$ is directly related to the population $\rho^2$. Using the definition $\sigma^2_\mu = \frac{\|X_c\beta\|^2}{n}$:

$$ \lambda = \frac{\|X_c \beta\|^2}{\sigma^2} = \frac{n \sigma^2_\mu}{\sigma^2} = n \left( \frac{\sigma^2_\mu}{\sigma^2} \right) $$
...
$$ \boxed{\lambda(\rho^2) = n \left( \frac{\rho^2}{1-\rho^2} \right)} $$
