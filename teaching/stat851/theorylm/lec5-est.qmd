...
Consider the decomposition defined by the projection matrices $P_{X_c}$ and $M = I - P_X$.

1.  **Independence:** The quadratic forms $\text{SSR}$ and $\text{SSE}$ are statistically independent because the subspaces $L(X_c)$ and $\text{Col}(X)^\perp$ are orthogonal.

2.  **Distribution of SSE:** The scaled sum of squared errors follows a central Chi-squared distribution:
    $$ \frac{\text{SSE}}{\sigma^2} = \frac{\|(I - P_X)y\|^2}{\sigma^2} \sim \chi^2(n-p) $$
    **Mean:**
    $$ E[\text{SSE}] = \sigma^2(n-p) $$

3.  **Distribution of SSR:** The scaled regression sum of squares follows a **non-central** Chi-squared distribution:
    $$ \frac{\text{SSR}}{\sigma^2} = \frac{\|P_{X_c}y\|^2}{\sigma^2} \sim \chi^2(p-1, \lambda) $$
    **Mean:**
    $$ E[\text{SSR}] = \sigma^2(p-1) + \|P_{X_c}\mu\|^2 $$
...
1.  **For SSE (Error Space):**
    $\text{SSE}$ is defined by the projection matrix $P_V = I - P_X$.
    
    * **Dimension:** The rank of $(I - P_X)$ is $n - \text{rank}(X) = n - p$.
    * **Non-centrality:** Since $\mu \in \text{Col}(X)$, the projection onto the orthogonal complement is zero: $\|(I - P_X)\mu\|^2 = 0$. Thus, $\lambda = 0$.
    * **Expectation:** Using Part 2 of @thm-proj-dist ($E(\|P_V y\|^2) = \sigma^2 k + \|P_V \mu\|^2$):
        $$ E[\text{SSE}] = \sigma^2(n-p) + 0 = \sigma^2(n-p) $$

2.  **For SSR (Regression Space):**
    $\text{SSR}$ is defined by the projection matrix $P_V = P_{X_c}$.
    
    * **Dimension:** The rank of $P_{X_c}$ is $p - 1$.
...
$$R^2_a = 1 - \frac{\text{SSE} / (n - k - 1)}{\text{SST} / (n - 1)} = 1 - \frac{\text{MSE}}{\text{MST}}$$
Relationship to $R^2$:
...
The distribution of the coefficient of determination $R^2 = \text{SSR} / (\text{SSR} + \text{SSE})$ follows directly from the ratio of the independent Chi-squared variables defined in @thm-distribution-ss-v2.

::: {#thm-r2-noncentral name="Non-Central Distribution of R-Squared"}
If $\beta_1 \neq 0$, the $R^2$ statistic follows a non-central Beta distribution:
$$ R^2 \sim \text{Beta}\left( \frac{p-1}{2}, \frac{n-p}{2}; \lambda \right) $$
where the non-centrality parameter is $\lambda = \frac{1}{2\sigma^2} \beta_1' X_c' X_c \beta_1$.
:::

::: proof
Let $U = \text{SSR}/\sigma^2$ and $V = \text{SSE}/\sigma^2$. From @thm-distribution-ss-v2, we have established:

1.  $U \sim \chi^2(p-1, \lambda)$ (Non-central Chi-squared).
2.  $V \sim \chi^2(n-p)$ (Central Chi-squared).
3.  $U$ and $V$ are independent.

We express $R^2$ in terms of these scaled variables:
$$ R^2 = \frac{\text{SSR}}{\text{SSR} + \text{SSE}} = \frac{U\sigma^2}{U\sigma^2 + V\sigma^2} = \frac{U}{U + V} $$

**Definition:** A random variable $X$ follows a non-central Beta distribution $\text{Beta}(\alpha, \beta; \lambda)$ if it can be written as $X = \frac{A}{A+B}$, where $A \sim \chi^2(2\alpha, \lambda)$ and $B \sim \chi^2(2\beta)$ are independent.

Matching our terms to this definition:
* $2\alpha = p-1 \implies \alpha = \frac{p-1}{2}$
* $2\beta = n-p \implies \beta = \frac{n-p}{2}$
...
::: {#cor-r2-null-behavior name="Null Behavior of R-Squared and Adjusted R-Squared"}
Under the null hypothesis of no regression effect ($H_0: \beta_1 = 0$), the non-centrality parameter vanishes ($\lambda = 0$), leading to the following properties:

1.  **Distribution of $R^2$:** It follows a central Beta distribution:
    $$ R^2 \sim \text{Beta}\left( \frac{p-1}{2}, \frac{n-p}{2} \right) $$

2.  **Expected Value of $R^2$ (Bias):**
    Even when no relationship exists, the expected $R^2$ is positive due to chance correlation:
    $$ E[R^2 | H_0] = \frac{p-1}{n-1} $$

3.  **Expected Value of $R^2_{adj}$ (Unbiasedness):**
    The Adjusted $R^2$ corrects for this bias. Using the linear relation $R^2_{adj} = 1 - (1 - R^2) \frac{n-1}{n-p}$:
    $$
    E[R^2_{adj} | H_0] = 1 - \left( 1 - \frac{p-1}{n-1} \right) \frac{n-1}{n-p} = 0
    $$
:::
...
A real estate agency wants to refine their pricing model. They regress the selling price of houses ($y$) on four predictors ($X$): Size, Age, Bedrooms, and Garage Capacity.

We assume the data has been collected and saved to `house_prices_4pred.csv`.

```{r}
#| label: generate-housing-data-4p
#| echo: false
...
price <- round(30000 + 140 * size - 800 * age + 
               15000 * beds + 10000 * garage + rnorm(n, 0, 10000), 0)
...
write.csv(housing_data, "house_prices_4pred.csv", row.names = FALSE)
```
...
# Load data
df <- read.csv("house_prices_4pred.csv")
...
# 1. Center the data
y_bar <- mean(y)
X_raw <- as.matrix(df[, c("Size", "Age", "Beds", "Garage")])
