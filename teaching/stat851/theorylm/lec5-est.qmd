...
### Distribution under $H_1$ and $H_0$

Using the independence and distributional results from @thm-distribution-ss-v2, the ratio follows a non-central F-distribution.

* **Under $H_1$:**
    $$ F \sim F(k, n-k-1, \lambda) $$
    where the non-centrality parameter is $\lambda = \frac{\|X_c \beta_1\|^2}{2\sigma^2}$.

* **Under $H_0$:**
    Since $\beta_1 = 0$, the non-centrality vanishes ($\lambda = 0$). The statistic follows a central F-distribution:
    $$ F \sim F(k, n-k-1) $$

### Justification via Expectations
...
Therefore, we reject $H_0$ for sufficiently large values of $F$. Specifically, we reject at level $\alpha$ if:
...
$$ E[R^2_a] \approx \frac{\text{Var}_{\text{signal}}}{\text{Var}_{\text{noise}} + \text{Var}_{\text{signal}}} = \frac{\text{Var}(E[Y_i|x_{(i)}])}{\text{Var}(Y_i)} $$
...
### Comparing Model Selection Criteria under $H_0$ and $H_1$

We simulate a dataset with $n=30$ observations and consider a sequence of nested models adding groups of predictors.

**Predictor Groups:**

1.  **Step 1 ($k=1$):** Add $x_1$. (This is the **Signal** under $H_1$, but Noise under $H_0$).
2.  **Step 2 ($k=6$):** Add $x_2, \dots, x_6$ (Pure Noise).
3.  **Step 3 ($k=11$):** Add $x_7, \dots, x_{11}$ (Pure Noise).

We visualize how the estimators for variance ($MSE$ vs $MLE$) and model fit ($R^2$ vs $R^2_{adj}$) behave as we add these variables.
...
```{r}
#| include: false
#| label: setup-animation-func

# --- Common Setup for Both Animations ---
library(ggplot2)
library(patchwork)
require("latex2exp")

# knobs
n_frames <- 30  # Number of simulation frames
fps      <- 2
n        <- 30
sigma    <- 1

# Define the sequence of models (number of predictors k)
# k=0 (Int), k=1 (x1), k=6 (x1..x6), k=11 (x1..x11)
model_steps <- c(0, 1, 6, 11)
...
generate_animation <- function(scenario = "H0", filename) {
  ...
  for (j in 1:n_frames) {
    ...
    # 1. Generate Data
    # 11 predictors total
    X <- as.data.frame(replicate(11, rnorm(n)))
    names(X) <- paste0("x", 1:11)
    
    # Define True Beta based on Scenario
    # H0: all betas 0
    # H1: beta1 = 2 (Signal), others 0
    beta_vec <- rep(0, 11)
    if (scenario == "H1") beta_vec[1] <- 2 
    
    # Generate y
    y <- as.numeric(as.matrix(X) %*% beta_vec + rnorm(n, sd = sigma))
    
    # 2. Fit Sequence of Models and Collect Stats
    stats_list <- lapply(model_steps, function(k) {
      ...
      if (k == 0) {
        ...
        form <- as.formula(paste("y ~", paste(names(X)[1:k], collapse = " + ")))
        fit <- lm(form, data = X)
      }
      
      # Extract Stats
      rss <- sum(residuals(fit)^2)
      p   <- k + 1  # param count (k predictors + intercept)
      
      # Variance Estimators
      # MSE (Unbiased): SSE / (n-p)
      mse <- rss / (n - p)
      # MLE (Biased): SSE / n
      mle <- rss / n
    ...
    # Plot A: Variance Estimators (MSE vs MLE)
    g1 <- ggplot(df, aes(x = k)) +
      # True Sigma^2 Reference
      geom_hline(yintercept = sigma^2, linetype = "dashed", color = "gray50") +
      annotate("text", x = 0, y = sigma^2 + 0.1, label = "True Variance", color="gray50", hjust=0, size=3) +
      
      # Lines
      geom_line(aes(y = MSE, color = "MSE (Unbiased)"), size = 1) +
      geom_point(aes(y = MSE, color = "MSE (Unbiased)"), size = 2) +
      geom_line(aes(y = MLE, color = "MLE (SSE/n)"), size = 1) +
      geom_point(aes(y = MLE, color = "MLE (SSE/n)"), size = 2) +
      
      scale_color_manual(values = c("MSE (Unbiased)" = "blue", "MLE (SSE/n)" = "red")) +
    ...
    # Plot B: R-squared vs Adjusted R-squared
    g2 <- ggplot(df, aes(x = k)) +
      geom_line(aes(y = R2, color = "R-squared"), size = 1) +
      geom_point(aes(y = R2, color = "R-squared"), size = 2) +
      geom_line(aes(y = R2_adj, color = "Adjusted R^2"), size = 1, linetype="dashed") +
      geom_point(aes(y = R2_adj, color = "Adjusted R^2"), size = 2) +
      
      scale_color_manual(values = c("R-squared" = "darkgreen", "Adjusted R^2" = "purple")) +
      scale_x_continuous(breaks = model_steps) +
      scale_y_continuous(limits = c(-0.2, 1), labels = scales::percent) +
      labs(title = "Model Fit Measures",
...
# Generate H0 Video
video_path_h0 <- "figs/rss-h0-v2.mp4"
generate_animation("H0", video_path_h0)
```
...
if (knitr::is_html_output()) {
  htmltools::tags$video(
    ...
    htmltools::tags$source(src = "figs/rss-h0-v2.mp4", type = "video/mp4")
  )
} else {
  # Static placeholder for PDF
  # (In practice, you might generate a static plot here)
...
Under $H_1$, $x_1$ is a true predictor ($\beta_1 = 2$). The subsequent groups ($x_2 \dots x_{11}$) remain noise.
...
# Generate H1 Video
video_path_h1 <- "figs/rss-h1-v2.mp4"
generate_animation("H1", video_path_h1)
```
...
if (knitr::is_html_output()) {
  htmltools::tags$video(
    controls = "controls", width = "100%", 
    htmltools::tags$source(src = "figs/rss-h1-v2.mp4", type = "video/mp4")
  )
} else {
  plot(1, 1, type="n", axes=FALSE, xlab="", ylab="", main="Animation available in HTML version")
