...
Consider the data partition $X = [X_1 \ X_2]$, where $X_1$ is $n \times p_1$ and $X_2$ is $n \times p_2$.

**1. Reduced Model (Index 0)**
Uses only $X_1$.
$$
\begin{aligned}
P_0 &= X_1(X_1^T X_1)^{-1}X_1^T & (\text{Projection Matrix}) \\
M_0 &= I - P_0 & (\text{Annihilator Matrix}) \\
\hat{y}_0 &= P_0 y & (\text{Reduced Estimator})
\end{aligned}
$$

**2. Full Model (Index 1)**
Uses the full matrix $X = [X_1 \ X_2]$.
$$
\begin{aligned}
P_1 &= X(X^T X)^{-1}X^T & (\text{Projection Matrix}) \\
M_1 &= I - P_1 & (\text{Annihilator Matrix}) \\
\hat{y}_1 &= P_1 y & (\text{Full Estimator})
\end{aligned}
$$
...
**The Truth:** The full model is correct.
$$
y = X_1\beta_1 + X_2\beta_2 + e, \quad \beta_2 \neq 0
$$
The true mean is $\mu = X_1\beta_1 + X_2\beta_2$.

We analyze the properties of the **Reduced Estimator** $\hat{y}_0$ (which "underfits" the data) compared to the Full Estimator $\hat{y}_1$.

::: {#thm-underfitting name="Bias-Variance Tradeoff in Underfitting"}
When the full model is true:

1.  **Bias:** The reduced estimator $\hat{y}_0$ is **biased**, while $\hat{y}_1$ is unbiased.
    $$ \text{Bias}(\hat{y}_0) = -M_0 X_2 \beta_2 $$
2.  **Variance:** The reduced estimator $\hat{y}_0$ has **smaller variance** (matrix difference is positive semidefinite).
...
The bias is:
$$
\text{Bias} = E[\hat{y}_0] - \mu = (X_1\beta_1 + P_0 X_2 \beta_2) - (X_1\beta_1 + X_2\beta_2) = -(I - P_0)X_2\beta_2 = -M_0 X_2 \beta_2
$$
...
**Conclusion:** Underfitting introduces bias (proportional to $M_0 X_2$) but reduces variance.

### Case 2: Overfitting

**The Truth:** The reduced model is correct.
$$
y = X_1\beta_1 + e \quad (\text{i.e., } \beta_2 = 0)
$$
The true mean is $\mu = X_1\beta_1$.

We analyze the properties of the **Full Estimator** $\hat{y}_1$ (which "overfits" the data) compared to the Reduced Estimator $\hat{y}_0$.

::: {#thm-overfitting name="Variance Inflation in Overfitting"}
When the reduced model is true:

1.  **Bias:** Both estimators are **unbiased**.
    $$ E[\hat{y}_1] = \mu \quad \text{and} \quad E[\hat{y}_0] = \mu $$
2.  **Variance:** The full estimator $\hat{y}_1$ has **unnecessarily higher variance**.
...
Consider the **Full Model** matrix $X = [X_1 \ X_2]$.
We define two estimators for the fitted values:

1.  **Reduced Estimator ($\hat{y}_0$):** Based on $X_1$ only.
    $$ \hat{y}_0 = P_1 y \quad \text{where} \quad P_1 = X_1(X_1^T X_1)^{-1}X_1^T $$
2.  **Full Estimator ($\hat{y}_1$):** Based on the full $X$.
    $$ \hat{y}_1 = P y \quad \text{where} \quad P = X(X^T X)^{-1}X^T $$
...
1.  **Bias:** The reduced estimator $\hat{y}_0$ is **biased**, while $\hat{y}_1$ is unbiased.
    $$ \text{Bias}(\hat{y}_0) = -(I - P_1)X_2\beta_2 $$
2.  **Variance:** The reduced estimator $\hat{y}_0$ has **smaller variance** (element-wise or matrix difference).
    $$ \text{Var}(\hat{y}_1) - \text{Var}(\hat{y}_0) = \sigma^2 (P - P_1) \ge 0 $$
:::
...
\end{aligned}
$$
The bias is $E[\hat{y}_0] - \mu = (X_1\beta_1 + P_1 X_2 \beta_2) - (X_1\beta_1 + X_2\beta_2) = -(I - P_1)X_2\beta_2$.
This is non-zero unless $X_2$ is already in the span of $X_1$ or orthogonal to the residual space.
...
**Part 1 (Bias):**
Since $\mu = X_1\beta_1$:
$$ E[\hat{y}_1] = P X_1\beta_1 = X_1\beta_1 = \mu \quad (\text{Since } X_1 \in \mathcal{C}(X)) $$
$$ E[\hat{y}_0] = P_1 X_1\beta_1 = X_1\beta_1 = \mu \quad (\text{Since } X_1 \in \mathcal{C}(X_1)) $$

**Part 2 (Variance):**
As shown in Case 1, $\text{Var}(\hat{y}_1) - \text{Var}(\hat{y}_0) = \sigma^2(P - P_1)$.
This matrix is positive semidefinite.
Specifically, the total variance (trace) increases by the number of extra parameters ($p_2$):
$$ \text{tr}(\text{Var}(\hat{y}_1)) - \text{tr}(\text{Var}(\hat{y}_0)) = \sigma^2 (\text{tr}(P) - \text{tr}(P_1)) = \sigma^2 (p_{full} - p_{reduced}) = \sigma^2 p_2 $$
:::
