...
generate_animation <- function(scenario = "H0", filename) {
  ...
  # Define beta_vec upfront so it dictates BOTH theoretical and empirical data
  ...
  # Since X are independent standard normals, Var(X_i) = 1.
  # Total Variance of Y = sigma^2 + sum(beta^2)
  total_var <- sigma^2 + sum(beta_vec^2)
  
  # Unified function: Expected Residual Variance = sigma^2 + variance of omitted variables
  get_expected_residual_var <- function(k) {
    if (k >= max_k) {
      omitted_var <- 0
    } else {
      # Sum of squared betas for variables NOT included in the model (k+1 to max_k)
  ...
  # Theoretical R^2 = 1 - (Expected Residual Variance / Total Variance)
  ...
  for (j in 1:n_frames) {
    ...
    # Generate y using the unified beta_vec
    ...
    g1 <- ggplot(df, aes(x = k)) +
      # Theoretical curve drawn with geom_line to match empirical
      ...
      # Dynamically scale upper limit so empirical spikes are never clipped
    ...
    g2 <- ggplot(df, aes(x = k)) +
      ...
      scale_x_continuous(breaks = model_steps) +
      # Use coord_cartesian to prevent dropping data points and triggering warnings
