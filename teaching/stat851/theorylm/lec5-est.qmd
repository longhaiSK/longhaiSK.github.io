...
generate_animation <- function(scenario = "H0", filename) {
  ...
  var_x <- 1 
  signal_var <- if(scenario == "H1") (2^2 * var_x) else 0
  ...
  for (j in 1:n_frames) {
    ...
    # Generate exactly as many columns as the max step requires
    ...
    stats_list <- lapply(model_steps, function(k) {
      if (k == 0) {
        ...
        # Dynamically build formula based on current k
        # checks if k exceeds available columns (safety)
        k_safe <- min(k, ncol(X))
      ...
      # Handle potential division by zero if p == n
