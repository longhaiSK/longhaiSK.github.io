...
library(plotly)
library(MASS)
# Ensure webshot and htmlwidgets are available for PDF generation
if (!requireNamespace("webshot", quietly = TRUE)) {
  warning("The 'webshot' package is required for PDF output of plotly figures.")
}
if (!requireNamespace("htmlwidgets", quietly = TRUE)) {
  warning("The 'htmlwidgets' package is required for PDF output of plotly figures.")
}
...
# --- 5. Output Logic (PDF vs HTML) ---
if (knitr::is_html_output()) {
  fig
} else {
  # 1. Define paths
  dir.create("figs", showWarnings = FALSE)
  img_path <- "figs/geometry-3d-static.png"
  tmp_html <- "temp_plotly.html"
  
  # 2. Save interactive widget to temporary HTML file
  htmlwidgets::saveWidget(as_widget(fig), tmp_html)
  
  # 3. Take screenshot using webshot
  # 'delay' is CRITICAL for 3D plots to allow WebGL to render
  webshot::webshot(
    url = tmp_html, 
    file = img_path, 
    delay = 2, 
  ...
  if (file.exists(tmp_html)) file.remove(tmp_html)
  
  # 5. Insert static image
