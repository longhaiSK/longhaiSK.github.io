...
linear_outcomes <- data.frame(
  ...
  Competencies = c(
    "<b>Understanding</b> the geometric interpretation of subspaces, orthogonal projections with the Hat matrix (H) and the Partial Residual matrix. <b>Numerically and symbolically calculating</b> orthogonal projections onto these spaces using various approaches",
  ...
  Percentages = c("15%", "10%", "10%", "10%", "40%", "15%")
)

# Render as a gt table
linear_outcomes %>%
  gt() %>%
  # Handle Markdown inside the cells
  fmt_markdown(columns = c(Competencies)) %>%
  # Set column widths to sum to 100%
  cols_width(
    Topic       ~ pct(30),
    Competencies ~ pct(60),
    Percentages ~ pct(10)
  ...
  cols_label(
    Topic = "Topic",
    Competencies = "Learning Outcomes (Knowledge & Skills)",
    Percentages = "Perc"
  ) %>%
  # Alignment
...
schedule_data %>%
  gt() %>%
  # Handle HTML formatting in Remarks
  fmt_markdown(columns = c(Remarks)) %>%
  # Set column widths to 100% total
  cols_width(
    Date      ~ pct(12),
    Acad_Week ~ pct(8),
    Topic     ~ pct(45),
