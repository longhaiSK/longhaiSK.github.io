...
# 2. Create a sample data frame named bond.data 
# (replace this with your actual data loading)
set.seed(42) # for reproducible results
bond.data <- data.frame(
  strength = rnorm(150, mean = 60, sd = 15),
  length = runif(150, min = 1.0, max = 5.0),
  height = runif(150, min = 0.5, max = 3.0)
)
# Introduce a relationship for a more interesting plot
bond.data$strength <- bond.data$strength - (7 * bond.data$length) + (4 * bond.data$height)

# 3. Create the 3D scatterplot and store its object
...
# 5. Add the regression plane to the plot
s3d$plane3d(fit, lty.box = "solid")
```

<b>Dynamic Visualization of 3D Data</b>

```{r}
# 1. Load the plotly library
library(plotly)

# 2. Create the initial plot object
fig <- plot_ly(
  data = bond.data, 
  x = ~length, 
  y = ~height, 
...
# 3. Add layout and labels to the 'fig' object
# This replaces the line that used the %>% pipe
fig <- layout(fig, 
  title = "Interactive 3D Plot of Bond Strength",
  scene = list(
...
# 4. Display the final, updated plot
fig
```
