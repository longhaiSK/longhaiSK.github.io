...
```{r include=FALSE,  eval=FALSE}
# --- 1. Clear the environment to prevent variable mix-ups ---
rm(list = ls())
...
# --- 4. Fit the model ONLY on the perfect data ---
fit_perfect <- lm(strength ~ length + height, data = perfect_data)

# --- 5. Prepare the surface grid from the perfect fit ---
axis_x <- seq(min(perfect_data$length), max(perfect_data$length), length.out = 10)
axis_y <- seq(min(perfect_data$height), max(perfect_data$height), length.out = 10)

z_matrix <- outer(
  axis_x,
  axis_y,
  function(len, ht) {
    predict(fit_perfect, newdata = data.frame(length = len, height = ht))
  }
)

# --- 6. Build the plot ---
fig <- plot_ly(data = perfect_data, x = ~length, y = ~height, z = ~strength,
               type = "scatter3d", mode = "markers", 
               marker = list(size=8, color="red"), name = "Data Points") %>%
  add_surface(x = ~axis_x, y = ~axis_y, z = ~z_matrix, 
              opacity = 0.7, name = "Fitted Surface") %>%
  layout(title = "Fit on a Perfect, Noise-Free Dataset")
...
```{r include=FALSE,  eval=FALSE}
par(mfrow = c(1,1))
s3d <- with(perfect_data, scatterplot3d(
  x = length,
  y = height,
  z = strength,
  pch = 19,
...
fit <- lm(strength ~ length + height, data = perfect_data)
s3d$plane3d(fit, lty.box = "solid")
```
