...
print(anova(fit.coin,fit.para,fit.full))
```


### Varying Slope, Equal Intercept Model
```{r}
#| echo: true
# Make sure sex is a factor (for colors/legend)
sbpdata$sex <- factor(sbpdata$sex)
...
# Fit (interaction: different slopes by sex)
fit.eqint <- lm(sbp ~ age + age:sex, data = sbpdata)
...
for (sx in lev) {
  nd <- data.frame(age = ages, sex = factor(sx, levels = lev))
  yhat <- predict(fit.eqint, newdata = nd)
  lines(ages, yhat, col = cols[sx], lwd = 2)
}
...
## Orders of Terms Matters in ANOVA and Warnings in Interpreting t-test
```{r}
#| echo: true
#| message: true
...
print(anova(fit.coin,fit.para, fit.full))
print(anova(fit.coin,fit.eqint, fit.full))
