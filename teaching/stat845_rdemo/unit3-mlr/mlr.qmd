...
| Source | Sum of Squares | $R^2$ | df | Mean Squares | $F$ | SS$_\mathrm{adj}$ | $\hat{\sigma}^2$ (variance estimates) | $R^2_{\mathrm{adj}}$ |
|:--|:--|:--|:---|:--|:--|:--|:--|:--|
| $X\beta$ | $\mathrm{SSR} = \displaystyle \sum_{i=1}^n (\hat y_i - \bar y)^2$ | $\displaystyle \frac{\mathrm{SSR}}{\mathrm{SST}}$ | $k$ | $\displaystyle \mathrm{MSR} = \frac{\mathrm{SSR}}{k}$ | $\displaystyle \frac{\mathrm{MSR}}{\mathrm{MSE}}$ | $\mathrm{SSR}_{\mathrm{adj}}$ | $\displaystyle \frac{\mathrm{SSR}_{\mathrm{adj}}}{\,n-1\,}$ | $\displaystyle \frac{\mathrm{SSR}_{\mathrm{adj}}}{\mathrm{SST}} = 1 - \frac{\mathrm{MSE}}{\mathrm{MST}}$ |
| $\epsilon$ | $\mathrm{SSE} = \displaystyle \sum_{i=1}^n (y_i - \hat y_i)^2$ | — | $n-p$ | $\displaystyle \mathrm{MSE} = \frac{\mathrm{SSE}}{n-p}$ | — | $\mathrm{SSE}$ | $\mathrm{MSE}$ | — |
| $y$ | $\mathrm{SST} = \displaystyle \sum_{i=1}^n (y_i - \bar y)^2$ | — | $n-1$ | $\displaystyle \mathrm{MST} = \frac{\mathrm{SST}}{n-1}$ | — | $\mathrm{SST}$ | $\mathrm{MST}$ | — |
...
= \frac{\hat{\sigma}^2_{X\beta}}{\hat{\sigma}^2_y}.
$$
...
$$
\mathrm{SSR}_{\mathrm{adj}} = \mathrm{SSR} - k\,\mathrm{MSE}.
$$

---

### Interpretation of $\hat{\sigma}^2$ and the Role of MSR

The quantity $\hat{\sigma}^2$ represents the **estimated variance** associated with each component of the model.  
However, the **Mean Square for Regression (MSR)** is *not* an estimate of the variance of $X\beta$.  
Its name “Mean Square” reflects that it is the **mean of the regression sum of squares under the null hypothesis** $H_0\!:\,\beta = 0$.
...
which justifies the use of the statistic

$$
F = \frac{\mathrm{MSR}}{\mathrm{MSE}}
$$

to test whether any regression coefficients (except the intercept) differ significantly from zero.

---

### Mean of $\mathrm{SSR}_{\text{adj}}$ (with $k = p-1$)
...
For fixed $X$, $\mathrm{SSR}_{\text{adj}}/(n-1)$ equals the **sample variance** of the true means $\{\mu_i\}$ over the observed design points.  
If the rows of $X$ are independently sampled with covariance matrix $\Sigma_X$ (the random-$X$ model), then
...
so $\mathrm{SSR}_{\text{adj}}/(n-1)$ serves as an **unbiased estimator** of the variance of $x^\top\beta$.
