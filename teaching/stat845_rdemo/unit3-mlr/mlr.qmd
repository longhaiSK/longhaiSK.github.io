...
```{r, cache=TRUE, fig.show="animate", class.source="fold-hide", fig.height=8}
library(ggplot2)
library(patchwork)
...
for (j in 1:N_sim) {
  ...
  stats_list <- lapply(ks, function(k) {
    ...
    if (k == 0) {
      ...
      fml <- as.formula(paste("y ~", paste(paste0("x", 1:k), collapse = " + ")))
  ...
  g2 <- ggplot(df, aes(x = p)) +
    ...
    labs(title = "Model Fit Statistics vs. Number of Parameters",
         x = "Number of parameters (p)") +
    theme_minimal(base_size = 13) +
