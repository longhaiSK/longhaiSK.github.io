...
| Source |  | $R^2$ | df | Mean Square | $F$ | Adjusted SS | $\hat{\sigma}^2$ | $R^2_{\mathrm{adj}}$ |
|:--|:--|---:|---:|:--|:--:|:--|:--:|:--:|
| $X\beta$ | $\mathrm{SSR} = \displaystyle \sum_{i=1}^n (\hat y_i - \bar y)^2$ | $\displaystyle \frac{\mathrm{SSR}}{\mathrm{SST}}$ | $p-1$ | $\displaystyle \mathrm{MSR} = \frac{\mathrm{SSR}}{p-1}$ | $\displaystyle \frac{\mathrm{MSR}}{\mathrm{MSE}}$ | $\mathrm{SSR}_{\mathrm{adj}}$ | $\displaystyle \frac{\mathrm{SSR}_{\mathrm{adj}}}{\,n-1\,}$ | $\displaystyle \frac{\mathrm{SSR}_{\mathrm{adj}}}{\mathrm{SST}} = 1 - \frac{\mathrm{MSE}}{\mathrm{MST}}$ |
...
where 
$$
\mathrm{SSR}_{\mathrm{adj}} = \mathrm{SSR} - (p-1)\,\mathrm{MSE}.
$$



- Interpretation Remark: The quantity $\hat{\sigma}^2$ represents the **estimated variance of each source**. The **MSR (Mean Square for Regression)** is *not* an estimate of the variance of $X\beta$.  
Its name comes from the **mean of the regression sum of squares under the null hypothesis** $H_0\!:\,\beta=0$.  In that case,
$$
E[\mathrm{MSR}\mid H_0] = \sigma^2, 
\qquad 
E[\mathrm{MSR}\mid H_1] > \sigma^2,
$$
which justifies the use of the ratio
$$
F = \frac{\mathrm{MSR}}{\mathrm{MSE}}
$$
as a test statistic for detecting whether any $\beta_j$ (except the intercept) differ from zero.
