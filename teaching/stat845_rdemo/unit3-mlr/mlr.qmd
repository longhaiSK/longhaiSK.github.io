...
# --- 1. Create Reproducible Sample Data ---
# This part is just to make the example runnable.
# You can replace this with your own `bond.data` and `fit` objects.
set.seed(123)
bond.data <- data.frame(
  length = runif(50, 1, 2),
  height = runif(50, 0.5, 1.5),
  strength = 5 * runif(50, 1, 2) + 2 * runif(50, 0.5, 1.5) + rnorm(50, 0, 0.5)
) %>%
  mutate(case = row_number())

# Create a sample linear model fit
fit <- lm(strength ~ length + height, data = bond.data)


# --- 2. Build the Plot (Corrected Version) ---

# Initialize the base scatter plot
fig <- plot_ly(
  data = bond.data,
  x = ~length, y = ~height, z = ~strength,
  ...
  hovertemplate = paste(
    ...
    "Length: %{x:.2f}<br>",
    "Height: %{y:.2f}<br>",
    "Strength: %{z:.2f}<extra></extra>"
  )
)

# Add text labels on top of the markers
# inherit = FALSE prevents warnings about 'marker' and 'textfont.color'
fig <- fig %>% add_trace(
  type = "scatter3d",
  mode = "text",
  x = ~length, y = ~height, z = ~strength,
  ...
  showlegend = FALSE,
  hoverinfo = "skip",
  inherit = FALSE  # <-- Key correction
)

# Prepare the data for the regression surface
resolution <- 20
...
# inherit = FALSE prevents warnings about invalid 'surface' attributes
fig <- fig %>% add_surface(
  x = ~axis_x,
  y = ~axis_y,
  z = ~z_matrix,
...
bond_data = pd.DataFrame({
    'length': np.random.uniform(1, 2, 50),
    'height': np.random.uniform(0.5, 1.5, 50)
})
# Calculate strength based on a linear relationship with some noise
bond_data['strength'] = 5 * bond_data['length'] + 2 * bond_data['height'] + np.random.normal(0, 0.5, 50)
bond_data['case'] = np.arange(1, len(bond_data) + 1)

# --- 2. Fit a Multiple Linear Regression Model ---
# Prepare the features (X) and target (y) for the model
X = bond_data[['length', 'height']]
y = bond_data['strength']

# Create and fit the model
model = LinearRegression()
model.fit(X, y)

# --- 3. Prepare Data for the Regression Surface ---
# Create a grid of x (length) and y (height) values
resolution = 20
axis_x = np.linspace(bond_data['length'].min(), bond_data['length'].max(), resolution)
axis_y = np.linspace(bond_data['height'].min(), bond_data['height'].max(), resolution)
grid_x, grid_y = np.meshgrid(axis_x, axis_y)

# Predict the z (strength) values for every point on the grid
# The grid points must be flattened into a 2D array for prediction
grid_features = np.vstack([grid_x.ravel(), grid_y.ravel()]).T
z_matrix = model.predict(grid_features).reshape(grid_x.shape)


# --- 4. Create the Plotly Figure ---

# a. Create the main scatter plot trace
scatter_trace = go.Scatter3d(
    x=bond_data['length'],
    y=bond_data['height'],
    z=bond_data['strength'],
    mode='markers',
    marker=dict(
        size=5,
        color=bond_data['strength'],  # Color points by strength value
...
# b. Create the text labels trace
text_trace = go.Scatter3d(
    ...
    hoverinfo='skip', # Disable hover events for the text labels
    showlegend=False
)

# c. Create the regression surface trace
...
# d. Combine all traces into a single figure
fig = go.Figure(data=[scatter_trace, text_trace, surface_trace])

# e. Update the layout with titles and axis labels
fig.update_layout(
    title_text='Interactive 3D Scatter Plot of Bond Strength',
    scene=dict(
        xaxis_title='Length',
        yaxis_title='Height',
...
- $\mathrm{Var}\!\big(E[y \mid x]\big)$ corresponds to the **explained variation** due to the regression component $X\beta$, and  
- $E\!\big(\mathrm{Var}[y \mid x]\big)$ corresponds to the **residual variation** due to $\epsilon$.


Hence, $F$ equals one plus the **ratio of the estimated variance of $X\beta$ per predictor** 
to the **estimated variance of the residuals per total degree of freedom**.  
Under $H_0\!:\beta=0$, both variance estimates coincide in expectation, 
so \( F \sim F_{k,n-p} \); under $H_1$, the numerator contains signal variance, 
making \( F > 1 \) on average.
...
```{r}
#| echo: true
data.frame(model.matrix(fit.coin)) 
anova(fit.coin)
```
...
```{r}
#| echo: true
data.frame(model.matrix(fit.para))
anova(fit.coin, fit.para)
```

### Interaction Effect (Varying Slope) Model
```{r}
#| echo: true
...
```{r}
#| echo: true
data.frame(model.matrix(fit.full))
...
## Another way to draw the lines with direct calculation

```{r}
colors <- ifelse(sbpdata$sex == 1, "red", "blue")
