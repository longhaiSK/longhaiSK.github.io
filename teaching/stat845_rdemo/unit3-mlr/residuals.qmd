...
```{r}
#| label: build-residual-variants-ascii-old
#| echo: true
#| message: false
#| include: false
#| warning: false
...
residuals_df_print <- residuals_df
...
x <- 1:n
y <- 2 + 3 * x + rnorm(n, mean = 0, sd = 5)
y[1] <- y[1] + 25  # add an outlier at x = 6
full_data  <- data.frame(x = x, y = y)
...
# Define line break style based on output format
brk <- if (knitr::is_html_output()) "<br/>" else "\\\\"

# --- THIS IS THE CORRECTED PART ---
# Create a vector of 10 names, one for each column in the data frame
display_names <- c(
  "$x_i$", 
  "$h_{ii}$", 
  "$e_i$", 
  ...
  "$e_{i,-i}$", 
  "$\\hat{\\sigma}_{-i}$",
  paste0("NS-Full", brk, "$\\frac{e_i}{\\hat{\\sigma}}$"),
  paste0("NS-LOO", brk, "$\\frac{e_{i,-i}}{\\hat{\\sigma}_{-i}}$"),
  paste0("ST-LOO", brk, "$\\frac{e_{i,-i}}{\\hat{\\sigma}_{-i}/\\sqrt{1-h_{ii}}}$"),
  paste0("ST-Full", brk, "$\\frac{e_i}{\\hat{\\sigma}_{-i}\\sqrt{1-h_{ii}}}$")
)

# --- THIS IS THE CONVERTED PART ---
# Use DT::datatable() instead of knitr::kable()
DT::datatable(
  residuals_df,
  caption = "Residual variants and related quantities (Interactive).",
