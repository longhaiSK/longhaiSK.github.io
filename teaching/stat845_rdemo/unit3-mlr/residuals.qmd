...
This means we have a true intercept of 2, a true slope of 3, and a random error term, $\epsilon_i$, drawn from a normal distribution with a mean of 0 and a standard deviation of 5. Two artificial outliers are added to this data to test the behavior of the different residual types.
```{r}
#| message: false
#| warning: false
# Load libraries
...
* **Identity Verification:** The numerical results validated the key algebraic identity that connects the full-data residual ($e_i$) to the more honest leave-one-out (deleted) residual ($e_{i,-i}$). This demonstrates that the LOOCV error can be calculated efficiently from a single model fit.
