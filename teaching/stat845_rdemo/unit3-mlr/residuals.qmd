...
### 1) Definition from the change in coefficients
...
### 2) Express $D_i$ via the **deleted (external) studentized residual** $t_i$
...
### 3) Exact null distribution (normal, homoskedastic errors; fixed $X$)
...
= \mathrm{BetaCCDF}\!\left(\frac{D_i}{c_i};\,\tfrac12,\,\tfrac{n-p-1}{2}\right),
$$

$$
d_{i,\alpha}=c_i\ \mathrm{BetaQuantile}\!\Big(1-\alpha;\tfrac12,\tfrac{n-p-1}{2}\Big).
$$

(Here $\mathrm{BetaCCDF}$ and $\mathrm{BetaQuantile}$ denote the Beta tail CDF and quantile; in R use `pbeta(..., lower.tail = FALSE)` and `qbeta(...)`.)

---

### 4) The rough $4/n$ rule (average-leverage simplification)
...
```{r}
# Exact p-value and critical value for Cook's D (given h_ii, n, p)
cook_pval <- function(D, h, n, p) {
  nu <- n - p - 1
  c  <- ((n - p)/p) * (h/(1 - h))
  x  <- pmin(D / c, 1)  # support is [0,1]
  pbeta(x, 0.5, nu/2, lower.tail = FALSE)
}

cook_crit <- function(h, n, p, alpha = 0.05) {
  nu <- n - p - 1
  c  <- ((n - p)/p) * (h/(1 - h))
  c * qbeta(1 - alpha, 0.5, nu/2)
}
```
...
### Finding the LOOCV Standard Error ($\hat{\sigma}_{-i}$) from the Full-Model Standard Error ($\hat{\sigma}$) {.unnumbered}
