...
library(biostat3)
k_all_levels <- c(
  "Level_1_Effect = power1",
  "Level_2_Effect = power2",
  "Level_3_Effect = power3",
  "Level_4_Effect = -power1 - power2 - power3"
)
...
Estimated Treatment Means ($mu_i$):
...
##### 95% Confidence Intervals for Treatment (Cell) Means
...
## Display CIs
data.frame(Power_Level = power_levels, Mean = mui.hat, SE = se, Lower_CI = CI.lower, Upper_CI = CI.upper, row.names = NULL)
```


#### Baseline Contrast Model (Default for R)
...
**An important trick for handling factor in R**

If we want to treat power200 as the baseline, we do this:
```{r}
etching_df_200 <- etching_df
etching_df_200$power <- factor(etching_df$power, levels=c("200","160", "180", "220"))
etching_df_200
levels(etching_df_200$power)
```
...
fit1_200 <- lm(rate ~ power, data = etching_df_200)
