...
# This chunk creates the video file for the HTML output
library(av)
pngs <- list.files("frames0", "\\.png$", full.names = TRUE)
pngs <- pngs[order(nchar(pngs), pngs)] # A more robust sort for frame numbers
av_encode_video(pngs, "rss-h0.mp4", framerate = 2, verbose = FALSE)
```
...
#| context: pdf
#| echo: false
#| fig.cap: "A single representative frame from the simulation."

# This chunk grabs the first frame to display in the PDF
pngs <- list.files("frames0", "\\.png$", full.names = TRUE)
pngs <- pngs[order(nchar(pngs), pngs)] # Sort frames
first_frame <- pngs[1]
knitr::include_graphics(first_frame)
```
