...
### An Animation to Visualize the Distributions of RSS
...
#| include: false
#| cache: true
#| label: build-h0-assets

# --- knobs (override in your setup) ---
n_frames <- getOption("sim_frames", 50L)
rebuild  <- isTRUE(getOption("sim_rebuild"))
fps      <- getOption("sim_fps", 1)           # frames per second for MP4
...
video_h0 <- "figs/rss-h0.mp4"
png_h0   <- "figs/rss-h0.png"
...
# --- frame generator (returns a patchwork object) ---
make_one_frame <- function() {
...
# --- build assets by format ---
if (knitr::is_html_output()) {
  if (!file.exists(video_h0) || rebuild) {
    # render frames, encode MP4
    frm <- file.path("frames0", sprintf("f%03d.png", seq_len(n_frames)))
    for (j in seq_len(n_frames)) {
      g <- make_one_frame()
      ggsave(frm[j], g, width = 8, height = 8, dpi = 150)
    }
    if (!requireNamespace("av", quietly = TRUE)) {
      stop("Package 'av' is required to create the MP4. Install it with install.packages('av').")
    }
    pngs <- list.files("frames0", "\\.png$", full.names = TRUE)
    pngs <- pngs[order(nchar(pngs), pngs)]
    av::av_encode_video(pngs, video_h0, framerate = fps, verbose = FALSE)
  }
} else if (knitr::is_latex_output()) {
  if (!file.exists(png_h0) || rebuild) {
    # single representative frame for PDF
    g <- make_one_frame()
    ggsave(png_h0, g, width = 8, height = 8, dpi = 300)
  }
}
```
...
#| cache: true
#| fig.align: center
#| fig.cap: "When $H_0$ is true: MP4 animation (HTML) or a representative static frame (PDF)."

if (knitr::is_html_output()) {
  htmltools::tags$video(
    controls = "controls", width = "100%", htmltools::tags$source(src = "figs/rss-h0.mp4", type = "video/mp4")
  )
} else if (knitr::is_latex_output()) {
  knitr::include_graphics("figs/rss-h0.png")
}
```
...
#| include: false
#| cache: true
#| label: build-h1-assets

# --- knobs (reuse) ---
n_frames <- getOption("sim_frames", 50L)
rebuild  <- isTRUE(getOption("sim_rebuild"))
fps      <- getOption("sim_fps", 1)
...
dir.create("figs",     showWarnings = FALSE)
video_h1 <- "figs/rss-h1.mp4"
png_h1   <- "figs/rss-h1.png"

# --- model params (H1: beta_1 = 1) ---
n <- 30; p_max <- 20
beta <- rep(0, p_max); beta[1] <- 1
sigma <- 1
x_lim <- c(0, n); ks <- seq(0, p_max, by = 2)

make_one_frame_h1 <- function() {
  X <- as.data.frame(replicate(p_max, rnorm(n)))
  names(X) <- paste0("x", 1:p_max)
  y <- as.numeric(as.matrix(X) %*% beta + rnorm(n, sd = sigma))
...
if (knitr::is_html_output()) {
  if (!file.exists(video_h1) || rebuild) {
    frm <- file.path("frames1", sprintf("f%03d.png", seq_len(n_frames)))
    for (j in seq_len(n_frames)) {
      g <- make_one_frame_h1()
      ggsave(frm[j], g, width = 8, height = 8, dpi = 150)
    }
    ...
    pngs <- list.files("frames1", "\\.png$", full.names = TRUE)
    pngs <- pngs[order(nchar(pngs), pngs)]
    av::av_encode_video(pngs, video_h1, framerate = fps, verbose = FALSE)
  }
} else if (knitr::is_latex_output()) {
  if (!file.exists(png_h1) || rebuild) {
    g <- make_one_frame_h1()
    ggsave(png_h1, g, width = 8, height = 8, dpi = 300)
  }
}
```
...
#| cache: true
#| fig.align: center
#| fig.cap: "When $H_1$ is true: MP4 animation (HTML) or a representative static frame (PDF)."

if (knitr::is_html_output()) {
  htmltools::tags$video(
    controls = "controls", width = "100%", htmltools::tags$source(src = "figs/rss-h1.mp4", type = "video/mp4")
  )
} else if (knitr::is_latex_output()) {
  knitr::include_graphics("figs/rss-h1.png")
}
```
