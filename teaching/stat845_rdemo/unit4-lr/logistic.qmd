...
## Assessing Statistical Regression Significance with Wilks' Theorem
...
# Row 1: Null model
# Row 2: Full model (fit1), showing the test against the null
lrt_table <- data.frame(
  "Resid. Df" = c(fit1$df.null, fit1$df.residual),
  "Resid. Dev" = c(round(fit1$null.deviance, 4), round(fit1$deviance, 4)),
  "Test Df" = c(NA, lrt_df),
  "Test Statistic (D)" = c(NA, round(lrt_statistic, 4)),
  "p-value" = c(NA, format.pval(p_value, digits = 4)),
  row.names = c("Null Model", "Full Model (fit1)"),
...
## Assessing Prediction Effect-size (anolouge to $R^2_\mathrm{adj}$)
...
*Confusion Matrix with threhold=0.5*
```{r}
# --- 4. Assess accuracy ---

# 4a. Misclassification Error Rate (using 0.5 threshold)
threhold <- 0.5
pred_class <- ifelse(pred_probs > threhold, 1, 0)
conf_matrix <- table(Actual = test_data$chd, Predicted = pred_class)
print(conf_matrix)
...
*Confusion Matrix with threhold=0.1*
```{r}
threhold <- 0.1
pred_class <- ifelse(pred_probs > threhold, 1, 0)
conf_matrix <- table(Actual = test_data$chd, Predicted = pred_class)
