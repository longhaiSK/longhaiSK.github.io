...
Here’s the same **description text** rewritten using `$...$` and `$$...$$` for math formatting:
...
```{r}
#| label: chd-univariate-2x2-jitter001
#| echo: true
...
for (v in vars) {
  ...
  # Univariate logistic regression: chd ~ x
  fit <- glm(y ~ x, family = binomial())
  
  print(summary(fit))

  # Base scatter of y with small jitter (left axis: probability scale)
  plot(
    x, jitter(y, amount = jit),
    pch = 16, col = rgb(0, 0, 0, 0.45),
  ...
  if (length(unique(x)) == 2) {
    xcat <- sort(unique(x))
    pcat <- predict(fit, newdata = data.frame(x = xcat), type = "response")
    points(xcat, pcat, pch = 19, col = "red")
    lines(xcat, pcat, col = "red", lwd = 2)

    # Right-axis: logit{π(x)}
    logit_p <- log(pcat / (1 - pcat))
    par(new = TRUE)
    plot(
      xcat, logit_p, type = "l", lwd = 2, col = "blue",
      axes = FALSE, xlab = "", ylab = "",
      xlim = range(x), ylim = range(logit_p)
    )
    axis(4)
    mtext("logit{π(x)}", side = 4, line = 3)
    par(new = FALSE)

  } else {
    # Continuous predictor
    xg <- seq(min(x), max(x), length.out = 400)
    pg <- predict(fit, newdata = data.frame(x = xg), type = "response")
    ...
    plot(
      ...
      axes = FALSE, xlab = "", ylab = "",
      xlim = range(xg), ylim = range(logit_pg)
    )
    axis(4)
    mtext("logit{π(x)}", side = 4, line = 3)
    par(new = FALSE)
