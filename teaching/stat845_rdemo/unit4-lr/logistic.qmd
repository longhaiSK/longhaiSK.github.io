...
## Assessing Statistical Regression Significance with Wilks' Theorem (Analogue of F-test for OLS)
...
where $\log L_0$ and $\log L_1$ are the log-likelihoods of the null and full models, respectively. In logistic regression, this is equivalent to the difference in the deviances: $D = \text{Deviance}_0 - \text{Deviance}_1$. For assessing the overall significance of a regression model (fit1_chd), this involves comparing it to its corresponding intercept-only (null) model. The degrees of freedom for the $\chi^2$ test is the difference in the number of parameters between the two models, $df = df_0 - df_1$, which equals the number of predictors in the full model.Here is an R code chunk demonstrating how to compute this p-value directly from a glm fit object, assuming it is named fit1_chd.
```{r}
...
## Assessing Predictive Effect-Size (Anolouge to $R^2_\mathrm{adj}$)
...
From this matrix, we derive several key performance metrics:
  ...
  * **ROC Curve and AUC:** An **ROC Curve** plots the model's performance across *all possible* thresholds by plotting the **True Positive Rate (Recall)** against the **False Positive Rate** (FPR = $\frac{FP}{FP + TN}$). The **Area Under the Curve (AUC)** summarizes this plot into a single number from 0 to 1, where 0.5 is a random guess and 1.0 is a perfect model.
...
legend(
  "topleft",
  legend = c("Actual 0 (o)", "Actual 1 (x)", "True p(x)"),
  pch = c(1, 4, NA),
  lty = c(NA, NA, 1),
  lwd = c(NA, NA, 2),
...
```{r}
library(PRROC)
# 4c. Precision-Recall (PR) Curve
# Scores for positive class (y == 1)
scores_pos_sim <- pred_probs_sim[test_data_sim$y == 1]
# Scores for negative class (y == 0)
scores_neg_sim <- pred_probs_sim[test_data_sim$y == 0]

# Calculate PR curve
pr_curve_sim <- pr.curve(scores.class0 = scores_pos_sim, scores.class1 = scores_neg_sim, curve = TRUE)

# Plot the PR curve
plot(pr_curve_sim, main = "Precision-Recall Curve (Simulated Test Data)", print.auc.approx = TRUE,
     xlim = c(0,1))

# Calculate and add the 'no-skill' baseline
baseline_precision_sim <- sum(test_data_sim$y == 1) / length(test_data_sim$y)
abline(h = baseline_precision_sim, col = "blue", lty = 2)
legend("bottomleft", 
       legend = c("Model", paste("Baseline (", round(baseline_precision_sim, 3), ")")), 
       col = c("black", "blue"), 
       lty = c(1, 2), 
...
library(PRROC)
# 4c. Precision-Recall (PR) Curve
# Scores for positive class (chd == 1)
scores_pos <- pred_probs[test_data$chd == 1]
# Scores for negative class (chd == 0)
scores_neg <- pred_probs[test_data$chd == 0]

# Calculate PR curve
pr_curve <- pr.curve(scores.class0 = scores_pos, scores.class1 = scores_neg, curve = TRUE)

# Plot the PR curve
plot(pr_curve, main = "Precision-Recall Curve (Test Data)", print.auc.approx = TRUE,
     xlim = c(0,1))
...
legend("bottomleft", 
       legend = c("Model", paste("Baseline (", round(baseline_precision, 3), ")")), 
       col = c("black", "blue"), 
       lty = c(1, 2), 
       bty = "n") # bty="n" removes the box
